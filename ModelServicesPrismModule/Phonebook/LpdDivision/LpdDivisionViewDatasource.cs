...
/*
    In the file of IModule-class of the project for the current service the following lines of code must be inserted:
        ...
        {
            ...
            containerRegistry.Register<ILpdDivisionViewDatasource, LpdDivisionViewDatasource>();
            ...
        }

*/
namespace ModelServicesPrismModule.Phonebook.LpdDivision {
    public class LpdDivisionViewDatasource: ILpdDivisionViewDatasource
    {
        protected readonly string _CurrentViewName   = "LpdDivisionView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            {"DivisionNameId", null},  // System.Int32
            {"DivisionName", null},  // System.String
        };
        protected ILpdDivisionViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public LpdDivisionViewDatasource(ILpdDivisionViewService frs,
        ...
        public bool Interface2Values(ILpdDivisionView data, bool doNotify = true) {
            if(data == null) {
                return this.Clear(doNotify);
            }
            bool hasChanged  = false;
            bool aftrMstrChngd = false;
            if(!this.isEqual(this.getValue("DivisionNameId"), data.DivisionNameId, "int32")) {
        ...
        public ILpdDivisionView Values2Interface() {
            object locobjval;

            LpdDivisionView rslt = new LpdDivisionView(); 
            locobjval = this.getValue("DivisionNameId");
            rslt.DivisionNameId = Convert.ToInt32(locobjval);
        ...
        public async Task DoSubmitOnDetailChanged(object sender)  {
            ...
            if (this._ClientViewName == "LprDivision01View") {
                if (this._DirectNavigation == "DivisionNameDict") {
                    ...
                    System.Int32  locDivisionNameId;
                    locobjval = this.getValue("DivisionNameId"); 
                    if(locobjval is null) {
                        appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                        return;
                    } else {
                        locDivisionNameId = Convert.ToInt32(locobjval);
                    }
                        ILpdDivisionView data = await this.frmRootSrv.getone(
                         locDivisionNameId
                        );
                        if(data == null) {
                            this.Clear(true);
        ...
        protected ILpdDivisionViewFilter DefineFilterOneProp(ILpdDivisionViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new LpdDivisionViewFilter(); 
            ...
            switch(propNm) {
                case "DivisionNameId":
                    ...
                    flt.divisionNameIdOprtr.Add(Oprtr);
                    flt.DivisionNameId.Add(Convert.ToInt32(value));
                    break;
                case "DivisionName":
        ...
        protected ILpdDivisionViewFilter GetFilterByCurrDirMstrs() {
            ILpdDivisionViewFilter flt = new LpdDivisionViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<ILpdDivisionView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<ILpdDivisionView>();
            }
            ILpdDivisionViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<ILpdDivisionView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<ILpdDivisionView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            ILpdDivisionViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            ILpdDivisionViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<ILpdDivisionView>();
            }
            if(prslt.items == null) {
        ...
        public async Task getonebyLpdDivisionNameUk(
                  System.String DivisionName
        ) {
            ILpdDivisionView data = await this.frmRootSrv.getonebyLpdDivisionNameUk(
                  DivisionName
            );
            if(data != null) {
                if(!this.Interface2Values(data, true)) {
        ...
        public async Task Refresh() {
            ...
            if (
                ...
                System.String  locDivisionName;
                locobjval = this.getValue("DivisionName"); 
                if(locobjval is null) {
                    appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                    return;
                } else {
                    locDivisionName = locobjval.ToString();
                }
                await this.getonebyLpdDivisionNameUk(
                      locDivisionName
                );
                return;
            }
        ...
        public async Task updateone() {
            ...
            ILpdDivisionView itm = this.Values2Interface();
            ILpdDivisionView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            ILpdDivisionView itm = this.Values2Interface();
            ILpdDivisionView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            ILpdDivisionView data = await this.frmRootSrv.deleteone(  locDivisionNameId );
            if(data != null) {
                this.Interface2Values(data, false);
