...
namespace ModelServicesPrismModule.Phonebook.PhbkDivision.ViewModels {
    public class PhbkDivisionViewRaddPageViewModel: INotifyPropertyChanged, INavigationAware, IDestructible  
    {
        ...
        public PhbkDivisionViewRaddPageViewModel(IPhbkDivisionViewService _FrmSrvPhbkDivisionView, IAppGlblSettingsService GlblSettingsSrv, INavigationService navigationService) {
        ...
        public void OnNavigatedTo(INavigationParameters prms) {
            ...
            if ((modeToCheck == EformModeEnum.DeleteMode) && ((GlblSettingsSrv.GetViewModelMask("PhbkDivisionView") & 2) != 2 )) {
                throw new Exception("Access denied to delete PhbkDivisionView");
            } else if ((modeToCheck == EformModeEnum.UpdateMode) && ((GlblSettingsSrv.GetViewModelMask("PhbkDivisionView") & 4) != 4 )) {
                throw new Exception("Access denied to update PhbkDivisionView");
            } else if ((modeToCheck == EformModeEnum.AddMode) && ((GlblSettingsSrv.GetViewModelMask("PhbkDivisionView") & 8) != 8 )) {
            ...
            if ((!resetHF) && (hf.Count > 0)) {
                ...
                Caption = "Add item for Division";
                if (resetHF) { HiddenFilters = hf; }
                FormControlModel = FrmSrvPhbkDivisionView.CopyToModelNotify(null,null); // this is correct: cleanup twice
                EformMode = EformModeEnum.AddMode;
                FormControlModel = null; // since DateTime-issue this is correct: FrmSrvPhbkDivisionView.CopyToModelNotify(null,null);
                IsParentLoaded = true;
        }
        #endregion
