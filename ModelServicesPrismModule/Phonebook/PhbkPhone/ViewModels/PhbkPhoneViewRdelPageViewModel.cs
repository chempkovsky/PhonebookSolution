...
namespace ModelServicesPrismModule.Phonebook.PhbkPhone.ViewModels {
    public class PhbkPhoneViewRdelPageViewModel: INotifyPropertyChanged, INavigationAware, IDestructible  
    {
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IPhbkPhoneViewService FrmSrvPhbkPhoneView = null;
        protected INavigationService _navigationService = null;

        public PhbkPhoneViewRdelPageViewModel(IPhbkPhoneViewService _FrmSrvPhbkPhoneView, IAppGlblSettingsService GlblSettingsSrv, INavigationService navigationService) {
        ...
        public void OnNavigatedTo(INavigationParameters prms) {
            ...
            if ((modeToCheck == EformModeEnum.DeleteMode) && ((GlblSettingsSrv.GetViewModelMask("PhbkPhoneView") & 2) != 2 )) {
                throw new Exception("Access denied to delete PhbkPhoneView");
            } else if ((modeToCheck == EformModeEnum.UpdateMode) && ((GlblSettingsSrv.GetViewModelMask("PhbkPhoneView") & 4) != 4 )) {
                throw new Exception("Access denied to update PhbkPhoneView");
            } else if ((modeToCheck == EformModeEnum.AddMode) && ((GlblSettingsSrv.GetViewModelMask("PhbkPhoneView") & 8) != 8 )) {
            ...
            if ((!resetHF) && (hf.Count > 0)) {
                ...
                string locCaption = "Delete item for Phone";
