...
/*
    In the file of IModule-class of the project for the current service the following lines of code must be inserted:
        ...
        {
            ...
            containerRegistry.Register<ILpdEmpFirstNameViewDatasource, LpdEmpFirstNameViewDatasource>();
            ...
        }

*/
namespace ModelServicesPrismModule.Phonebook.LpdEmpFirstName {
    public class LpdEmpFirstNameViewDatasource: ILpdEmpFirstNameViewDatasource
    {
        protected readonly string _CurrentViewName   = "LpdEmpFirstNameView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            {"EmpFirstNameId", null},  // System.Int32
            {"EmpFirstName", null},  // System.String
        };
        protected ILpdEmpFirstNameViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public LpdEmpFirstNameViewDatasource(ILpdEmpFirstNameViewService frs,
        ...
        public bool Interface2Values(ILpdEmpFirstNameView data, bool doNotify = true) {
            if(data == null) {
                return this.Clear(doNotify);
            }
            bool hasChanged  = false;
            bool aftrMstrChngd = false;
            if(!this.isEqual(this.getValue("EmpFirstNameId"), data.EmpFirstNameId, "int32")) {
        ...
        public ILpdEmpFirstNameView Values2Interface() {
            object locobjval;

            LpdEmpFirstNameView rslt = new LpdEmpFirstNameView(); 
            locobjval = this.getValue("EmpFirstNameId");
            rslt.EmpFirstNameId = Convert.ToInt32(locobjval);
        ...
        public async Task DoSubmitOnDetailChanged(object sender)  {
            ...
            if (this._ClientViewName == "LprEmployee01View") {
                if (this._DirectNavigation == "EmpFirstNameDict") {
                    ...
                    System.Int32  locEmpFirstNameId;
                    locobjval = this.getValue("EmpFirstNameId"); 
                    if(locobjval is null) {
                        appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                        return;
                    } else {
                        locEmpFirstNameId = Convert.ToInt32(locobjval);
                    }
                        ILpdEmpFirstNameView data = await this.frmRootSrv.getone(
                         locEmpFirstNameId
                        );
                        if(data == null) {
                            this.Clear(true);
        ...
        protected ILpdEmpFirstNameViewFilter DefineFilterOneProp(ILpdEmpFirstNameViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new LpdEmpFirstNameViewFilter(); 
            ...
            switch(propNm) {
                case "EmpFirstNameId":
                    ...
                    flt.empFirstNameIdOprtr.Add(Oprtr);
                    flt.EmpFirstNameId.Add(Convert.ToInt32(value));
                    break;
                case "EmpFirstName":
        ...
        protected ILpdEmpFirstNameViewFilter GetFilterByCurrDirMstrs() {
            ILpdEmpFirstNameViewFilter flt = new LpdEmpFirstNameViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<ILpdEmpFirstNameView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<ILpdEmpFirstNameView>();
            }
            ILpdEmpFirstNameViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<ILpdEmpFirstNameView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<ILpdEmpFirstNameView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            ILpdEmpFirstNameViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            ILpdEmpFirstNameViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<ILpdEmpFirstNameView>();
            }
            if(prslt.items == null) {
        ...
        public async Task getonebyLpdEmpFirstNameUK(
                  System.String EmpFirstName
        ) {
            ILpdEmpFirstNameView data = await this.frmRootSrv.getonebyLpdEmpFirstNameUK(
                  EmpFirstName
            );
            if(data != null) {
                if(!this.Interface2Values(data, true)) {
        ...
        public async Task Refresh() {
            ...
            if (
                ...
                System.String  locEmpFirstName;
                locobjval = this.getValue("EmpFirstName"); 
                if(locobjval is null) {
                    appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                    return;
                } else {
                    locEmpFirstName = locobjval.ToString();
                }
                await this.getonebyLpdEmpFirstNameUK(
                      locEmpFirstName
                );
                return;
            }
        ...
        public async Task updateone() {
            ...
            ILpdEmpFirstNameView itm = this.Values2Interface();
            ILpdEmpFirstNameView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            ILpdEmpFirstNameView itm = this.Values2Interface();
            ILpdEmpFirstNameView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            ILpdEmpFirstNameView data = await this.frmRootSrv.deleteone(  locEmpFirstNameId );
            if(data != null) {
                this.Interface2Values(data, false);
