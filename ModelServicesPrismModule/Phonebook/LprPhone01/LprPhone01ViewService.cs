...
namespace ModelServicesPrismModule.Phonebook.LprPhone01 {
    public class LprPhone01ViewService: ILprPhone01ViewService
    {
        ...
        public LprPhone01ViewService(IAppGlblSettingsService agstt) {
            this.appGlblSettings = agstt;
            this.serviceUrl = this.appGlblSettings.GetWebApiPrefix("LprPhone01View") + "lprphone01viewwebapi";
    ...
    public Dictionary<(string viewNm, string navNm, string propNm), object> getHiddenFilterByRow(ILprPhone01View rw, string nvNm) {
        ...
        foreach(string i in this._Values.Keys) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                rslt[("LprPhone01View",nvNm,i)] = rw.GetType().GetProperty(i).GetValue(rw);
            }
        }
        return rslt;
    }
    ...
    public Dictionary<(string viewNm, string navNm, string propNm), string> getC2mfm() {
        ...
        public async Task<IList<ILprPhone01View>> getall() {
            try
            {
                HttpResponseMessage response = await client.GetAsync(serviceUrl + "/" + "getall");
                response.EnsureSuccessStatusCode();
                string responseBody = await response.Content.ReadAsStringAsync();
                return new List<ILprPhone01View>( JsonConvert.DeserializeObject<IList<LprPhone01View>>(responseBody) );
            } 
            catch(Exception e)
            {
                string exceptionMsg = "LprPhone01ViewService.getall : " + e.Message;
        ...
        public async Task<ILprPhone01ViewPage> getwithfilter(ILprPhone01ViewFilter filter) {
            string prms = null;
            if(filter != null) {
            ...
            try
            {
                ...
                response.EnsureSuccessStatusCode();
                string responseBody = await response.Content.ReadAsStringAsync();
                return  JsonConvert.DeserializeObject<LprPhone01ViewPage>(responseBody) ;
            } 
            catch(Exception e)
            {
                string exceptionMsg = "LprPhone01ViewService.getwithfilter : " + e.Message;
                Exception inner = e.InnerException;
                while (inner != null)
                {
                    exceptionMsg = exceptionMsg + ": " + inner.Message;
    ...
    //    // handling value of type ILprPhone01View
    // },
    // error => {
    //    // handling error 
    // });
    // 
    public async Task<ILprPhone01View> getone(
        ...
        , System.Int32 PhoneId 
            ...
            try
            {
                ...
                response.EnsureSuccessStatusCode();
                string responseBody = await response.Content.ReadAsStringAsync();
                return  JsonConvert.DeserializeObject<LprPhone01View>(responseBody) ;
            } 
            catch(Exception e) 
            {
                string exceptionMsg = "LprPhone01ViewService.getone : " + e.Message;
                Exception inner = e.InnerException;
                while (inner != null)
                {
                    exceptionMsg = exceptionMsg + ": " + inner.Message;
    ...
    public async Task<ILprPhone01ViewPage> getmanybyrepprim(ILprPhone01ViewFilter filter) {
        string prms = null;
        if(filter != null) {
                if (filter.PhoneId != null) {
                    foreach(var val in filter.PhoneId) {
        ...
        {
            string exceptionMsg = "LprPhone01ViewService.getmanybyrepprim : " + e.Message;
            Exception inner = e.InnerException;
            while (inner != null)
            {
                exceptionMsg = exceptionMsg + ": " + inner.Message;
        ...
        public async Task<ILprPhone01View> updateone(ILprPhone01View item) {
            if(item == null) {
            ...
            {
                string exceptionMsg = "LprPhone01ViewService.updateone : " + e.Message;
                Exception inner = e.InnerException;
                while (inner != null)
                {
                    exceptionMsg = exceptionMsg + ": " + inner.Message;
        ...
        public async Task<ILprPhone01View> addone(ILprPhone01View item) {
            if(item == null) {
                appGlblSettings.ShowErrorMessage("http", "Input item is not defined");
                return null;
            }
            ...
            {
                string exceptionMsg = "LprPhone01ViewService.addone : " + e.Message;
                Exception inner = e.InnerException;
                while (inner != null)
                {
                    exceptionMsg = exceptionMsg + ": " + inner.Message;
        ...
        public async Task<ILprPhone01View> deleteone(System.Int32 LpdPhoneIdRef , System.Int32 PhoneId ) {
            string prms = null;
                if(prms == null)
                    prms = "?" + Uri.EscapeDataString("phoneId") + "=" +  Uri.EscapeDataString(PhoneId.ToString());
                else 
            ...
            {
                string exceptionMsg = "LprPhone01ViewService.deleteone : " + e.Message;
                Exception inner = e.InnerException;
                while (inner != null)
                {
                    exceptionMsg = exceptionMsg + ": " + inner.Message;
        ...
        public ILprPhone01ViewNotify CopyToModelNotify(ILprPhone01View  src, ILprPhone01ViewNotify dest = null) {
            if(dest == null) dest = new LprPhone01ViewNotify();
            if(src == null) return dest;
            dest.PhoneId = src.PhoneId;
            dest.LpdPhoneIdRef = src.LpdPhoneIdRef;
            return dest;
        }
        public ILprPhone01View CopyToModel(ILprPhone01View  src, ILprPhone01View dest = null) {
            if(dest == null) dest = new LprPhone01View();
            if(src == null) return dest;
            dest.PhoneId = src.PhoneId;
            dest.LpdPhoneIdRef = src.LpdPhoneIdRef;
            return dest;
        ...
        public ILprPhone01ViewFilter GetFilter() {
            return new LprPhone01ViewFilter();
        }

        public IList<IWebServiceFilterRsltInterface> Row2FilterRslt(ILprPhone01View r) {
            IList<IWebServiceFilterRsltInterface> rslt = new List<IWebServiceFilterRsltInterface>();
        ...
        public IList<IWebServiceFilterRsltInterface> RowPrim2FilterRslt(ILprPhone01View r) {
            IList<IWebServiceFilterRsltInterface> rslt = new List<IWebServiceFilterRsltInterface>();
            if (r == null) return rslt;
            foreach(string i in this._Values.Keys) {
        ...
        public ILprPhone01ViewFilter FilterRslt2Filter(IWebServiceFilterRsltInterface e, ILprPhone01ViewFilter dest) {
            ILprPhone01ViewFilter flt = dest;
            if(dest is null) {
                flt = GetFilter();
            }
        ...
        public ILprPhone01ViewFilter FilterRslt2Filter(IList<IWebServiceFilterRsltInterface> src, ILprPhone01ViewFilter dest) {
            ILprPhone01ViewFilter rslt = dest;
            if(dest is null) {
                rslt = GetFilter();
            }
