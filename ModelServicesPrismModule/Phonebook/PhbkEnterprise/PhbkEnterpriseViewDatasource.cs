...
/*
    In the file of IModule-class of the project for the current service the following lines of code must be inserted:
        ...
        {
            ...
            containerRegistry.Register<IPhbkEnterpriseViewDatasource, PhbkEnterpriseViewDatasource>();
            ...
        }

*/
namespace ModelServicesPrismModule.Phonebook.PhbkEnterprise {
    public class PhbkEnterpriseViewDatasource: IPhbkEnterpriseViewDatasource
    {
        protected readonly string _CurrentViewName   = "PhbkEnterpriseView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            {"EntrprsId", null},  // System.Int32
            {"EntrprsName", null},  // System.String
            {"EntrprsDesc", null},  // System.String
        };
        protected IPhbkEnterpriseViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public PhbkEnterpriseViewDatasource(IPhbkEnterpriseViewService frs,
        ...
        public bool Interface2Values(IPhbkEnterpriseView data, bool doNotify = true) {
            if(data == null) {
                return this.Clear(doNotify);
            }
            bool hasChanged  = false;
            bool aftrMstrChngd = false;
            if(!this.isEqual(this.getValue("EntrprsId"), data.EntrprsId, "int32")) {
                this.setValue("EntrprsId", data.EntrprsId);
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("EntrprsName"), data.EntrprsName, "string")) {
                this.setValue("EntrprsName", data.EntrprsName);
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("EntrprsDesc"), data.EntrprsDesc, "string")) {
        ...
        public IPhbkEnterpriseView Values2Interface() {
            object locobjval;

            PhbkEnterpriseView rslt = new PhbkEnterpriseView(); 
            locobjval = this.getValue("EntrprsId");
            rslt.EntrprsId = Convert.ToInt32(locobjval);
            locobjval = this.getValue("EntrprsName");
            if(locobjval is null) {
                rslt.EntrprsName =  null;
            } else {
                rslt.EntrprsName =  locobjval.ToString();
            }
            locobjval = this.getValue("EntrprsDesc");
            if(locobjval is null) {
                rslt.EntrprsDesc =  null;
            } else {
                rslt.EntrprsDesc =  locobjval.ToString();
            }
            return rslt;
        }
        ...
        public async Task DoSubmitOnDetailChanged(object sender)  {
            ...
            if (this._ClientViewName == "PhbkDivisionView") {
                if (this._DirectNavigation == "Enterprise") {
                    ...
                    System.Int32  locEntrprsId;
                    locobjval = this.getValue("EntrprsId"); 
                    if(locobjval is null) {
                        appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                        return;
                    } else {
                        locEntrprsId = Convert.ToInt32(locobjval);
                    }
                        IPhbkEnterpriseView data = await this.frmRootSrv.getone(
                         locEntrprsId
                        );
                        if(data == null) {
                            this.Clear(true);
        ...
        protected IPhbkEnterpriseViewFilter DefineFilterOneProp(IPhbkEnterpriseViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new PhbkEnterpriseViewFilter(); 
            ...
            switch(propNm) {
                case "EntrprsId":
                    ...
                    flt.entrprsIdOprtr.Add(Oprtr);
                    flt.EntrprsId.Add(Convert.ToInt32(value));
                    break;
                case "EntrprsName":
                    ...
                    flt.entrprsNameOprtr.Add(Oprtr);
                    if (value is null) {
                        flt.EntrprsName.Add(null);
                    } else {
                        flt.EntrprsName.Add(value.ToString());
                    }
                    break;
            }
            return flt;
        }
        protected IPhbkEnterpriseViewFilter GetFilterByCurrDirMstrs() {
            IPhbkEnterpriseViewFilter flt = new PhbkEnterpriseViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<IPhbkEnterpriseView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<IPhbkEnterpriseView>();
            }
            IPhbkEnterpriseViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<IPhbkEnterpriseView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<IPhbkEnterpriseView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            IPhbkEnterpriseViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            IPhbkEnterpriseViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<IPhbkEnterpriseView>();
            }
            if(prslt.items == null) {
        ...
        public async Task getonebyEntrprsNameUK(
                  System.String EntrprsName
        ) {
            IPhbkEnterpriseView data = await this.frmRootSrv.getonebyEntrprsNameUK(
                  EntrprsName
            );
            if(data != null) {
                if(!this.Interface2Values(data, true)) {
        ...
        public async Task Refresh() {
            ...
            if (
                    this.isSetValue("EntrprsName")
            ) {
  
                System.String  locEntrprsName;
                locobjval = this.getValue("EntrprsName"); 
                if(locobjval is null) {
                    appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                    return;
                } else {
                    locEntrprsName = locobjval.ToString();
                }
                await this.getonebyEntrprsNameUK(
                      locEntrprsName
                );
                return;
            }
        ...
        public async Task updateone() {
            ...
            IPhbkEnterpriseView itm = this.Values2Interface();
            IPhbkEnterpriseView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            IPhbkEnterpriseView itm = this.Values2Interface();
            IPhbkEnterpriseView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            IPhbkEnterpriseView data = await this.frmRootSrv.deleteone(  locEntrprsId );
            if(data != null) {
                this.Interface2Values(data, false);
