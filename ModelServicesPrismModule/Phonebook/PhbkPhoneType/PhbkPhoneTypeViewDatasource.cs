...
/*
    In the file of IModule-class of the project for the current service the following lines of code must be inserted:
        ...
        {
            ...
            containerRegistry.Register<IPhbkPhoneTypeViewDatasource, PhbkPhoneTypeViewDatasource>();
            ...
        }

*/
namespace ModelServicesPrismModule.Phonebook.PhbkPhoneType {
    public class PhbkPhoneTypeViewDatasource: IPhbkPhoneTypeViewDatasource
    {
        protected readonly string _CurrentViewName   = "PhbkPhoneTypeView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            {"PhoneTypeId", null},  // System.Int32
            {"PhoneTypeName", null},  // System.String
            {"PhoneTypeDesc", null},  // System.String
        };
        protected IPhbkPhoneTypeViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public PhbkPhoneTypeViewDatasource(IPhbkPhoneTypeViewService frs,
        ...
        public bool Interface2Values(IPhbkPhoneTypeView data, bool doNotify = true) {
            if(data == null) {
                return this.Clear(doNotify);
            }
            bool hasChanged  = false;
            bool aftrMstrChngd = false;
            if(!this.isEqual(this.getValue("PhoneTypeId"), data.PhoneTypeId, "int32")) {
                this.setValue("PhoneTypeId", data.PhoneTypeId);
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("PhoneTypeName"), data.PhoneTypeName, "string")) {
                this.setValue("PhoneTypeName", data.PhoneTypeName);
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("PhoneTypeDesc"), data.PhoneTypeDesc, "string")) {
        ...
        public IPhbkPhoneTypeView Values2Interface() {
            object locobjval;

            PhbkPhoneTypeView rslt = new PhbkPhoneTypeView(); 
            locobjval = this.getValue("PhoneTypeId");
            rslt.PhoneTypeId = Convert.ToInt32(locobjval);
            locobjval = this.getValue("PhoneTypeName");
            if(locobjval is null) {
                rslt.PhoneTypeName =  null;
            } else {
                rslt.PhoneTypeName =  locobjval.ToString();
            }
            locobjval = this.getValue("PhoneTypeDesc");
            if(locobjval is null) {
                rslt.PhoneTypeDesc =  null;
            } else {
                rslt.PhoneTypeDesc =  locobjval.ToString();
            }
            return rslt;
        }
        ...
        public async Task DoSubmitOnDetailChanged(object sender)  {
            ...
            if (this._ClientViewName == "PhbkPhoneView") {
                if (this._DirectNavigation == "PhoneType") {
                    bool isKeyCrrct = true;
                    object dtFrTst;
                    if(isKeyCrrct) {
                        dtFrTst = this.getValue("PhoneTypeId");
                        if (dtFrTst == null) {
                            isKeyCrrct = false;
                        }
                    }
                    ...
                    System.Int32  locPhoneTypeId;
                    locobjval = this.getValue("PhoneTypeId"); 
                    if(locobjval is null) {
                        appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                        return;
                    } else {
                        locPhoneTypeId = Convert.ToInt32(locobjval);
                    }
                        IPhbkPhoneTypeView data = await this.frmRootSrv.getone(
                         locPhoneTypeId
                        );
                        if(data == null) {
                            this.Clear(true);
        ...
        protected IPhbkPhoneTypeViewFilter DefineFilterOneProp(IPhbkPhoneTypeViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new PhbkPhoneTypeViewFilter(); 
            ...
            switch(propNm) {
                case "PhoneTypeId":
                    ...
                    flt.phoneTypeIdOprtr.Add(Oprtr);
                    flt.PhoneTypeId.Add(Convert.ToInt32(value));
                    break;
                case "PhoneTypeName":
                    ...
                    flt.phoneTypeNameOprtr.Add(Oprtr);
                    if (value is null) {
                        flt.PhoneTypeName.Add(null);
                    } else {
                        flt.PhoneTypeName.Add(value.ToString());
                    }
                    break;
            }
            return flt;
        }
        protected IPhbkPhoneTypeViewFilter GetFilterByCurrDirMstrs() {
            IPhbkPhoneTypeViewFilter flt = new PhbkPhoneTypeViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<IPhbkPhoneTypeView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<IPhbkPhoneTypeView>();
            }
            IPhbkPhoneTypeViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<IPhbkPhoneTypeView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<IPhbkPhoneTypeView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            IPhbkPhoneTypeViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            IPhbkPhoneTypeViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<IPhbkPhoneTypeView>();
            }
            if(prslt.items == null) {
        ...
        public async Task updateone() {
            ...
            IPhbkPhoneTypeView itm = this.Values2Interface();
            IPhbkPhoneTypeView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            IPhbkPhoneTypeView itm = this.Values2Interface();
            IPhbkPhoneTypeView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            IPhbkPhoneTypeView data = await this.frmRootSrv.deleteone(  locPhoneTypeId );
            if(data != null) {
                this.Interface2Values(data, false);
