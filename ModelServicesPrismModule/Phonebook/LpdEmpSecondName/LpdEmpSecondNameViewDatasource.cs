...
/*
    In the file of IModule-class of the project for the current service the following lines of code must be inserted:
        ...
        {
            ...
            containerRegistry.Register<ILpdEmpSecondNameViewDatasource, LpdEmpSecondNameViewDatasource>();
            ...
        }

*/
namespace ModelServicesPrismModule.Phonebook.LpdEmpSecondName {
    public class LpdEmpSecondNameViewDatasource: ILpdEmpSecondNameViewDatasource
    {
        protected readonly string _CurrentViewName   = "LpdEmpSecondNameView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            {"EmpSecondNameId", null},  // System.Int32
            {"EmpSecondName", null},  // System.String
        };
        protected ILpdEmpSecondNameViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public LpdEmpSecondNameViewDatasource(ILpdEmpSecondNameViewService frs,
        ...
        public bool Interface2Values(ILpdEmpSecondNameView data, bool doNotify = true) {
            if(data == null) {
                return this.Clear(doNotify);
            }
            bool hasChanged  = false;
            bool aftrMstrChngd = false;
            if(!this.isEqual(this.getValue("EmpSecondNameId"), data.EmpSecondNameId, "int32")) {
        ...
        public ILpdEmpSecondNameView Values2Interface() {
            object locobjval;

            LpdEmpSecondNameView rslt = new LpdEmpSecondNameView(); 
            locobjval = this.getValue("EmpSecondNameId");
            rslt.EmpSecondNameId = Convert.ToInt32(locobjval);
        ...
        public async Task DoSubmitOnDetailChanged(object sender)  {
            ...
            if (this._ClientViewName == "LprEmployee01View") {
                if (this._DirectNavigation == "EmpSecondNameDict") {
                    ...
                    System.Int32  locEmpSecondNameId;
                    locobjval = this.getValue("EmpSecondNameId"); 
                    if(locobjval is null) {
                        appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                        return;
                    } else {
                        locEmpSecondNameId = Convert.ToInt32(locobjval);
                    }
                        ILpdEmpSecondNameView data = await this.frmRootSrv.getone(
                         locEmpSecondNameId
                        );
                        if(data == null) {
                            this.Clear(true);
        ...
        protected ILpdEmpSecondNameViewFilter DefineFilterOneProp(ILpdEmpSecondNameViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new LpdEmpSecondNameViewFilter(); 
            ...
            switch(propNm) {
                case "EmpSecondNameId":
                    ...
                    flt.empSecondNameIdOprtr.Add(Oprtr);
                    flt.EmpSecondNameId.Add(Convert.ToInt32(value));
                    break;
                case "EmpSecondName":
                    ...
                    flt.empSecondNameOprtr.Add(Oprtr);
                    if (value is null) {
                        flt.EmpSecondName.Add(null);
                    } else {
                        flt.EmpSecondName.Add(value.ToString());
                    }
                    break;
            }
            return flt;
        }
        protected ILpdEmpSecondNameViewFilter GetFilterByCurrDirMstrs() {
            ILpdEmpSecondNameViewFilter flt = new LpdEmpSecondNameViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<ILpdEmpSecondNameView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<ILpdEmpSecondNameView>();
            }
            ILpdEmpSecondNameViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<ILpdEmpSecondNameView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<ILpdEmpSecondNameView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            ILpdEmpSecondNameViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            ILpdEmpSecondNameViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<ILpdEmpSecondNameView>();
            }
            if(prslt.items == null) {
        ...
        public async Task getonebyLpdEmpSecondNameUK(
                  System.String EmpSecondName
        ) {
            ILpdEmpSecondNameView data = await this.frmRootSrv.getonebyLpdEmpSecondNameUK(
                  EmpSecondName
            );
            if(data != null) {
                if(!this.Interface2Values(data, true)) {
        ...
        public async Task Refresh() {
            ...
            if (
                ...
                System.String  locEmpSecondName;
                locobjval = this.getValue("EmpSecondName"); 
                if(locobjval is null) {
                    locEmpSecondName = null;
                } else {
                    locEmpSecondName = locobjval.ToString();
                }
                await this.getonebyLpdEmpSecondNameUK(
                      locEmpSecondName
                );
                return;
            }
        ...
        public async Task updateone() {
            ...
            ILpdEmpSecondNameView itm = this.Values2Interface();
            ILpdEmpSecondNameView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            ILpdEmpSecondNameView itm = this.Values2Interface();
            ILpdEmpSecondNameView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            ILpdEmpSecondNameView data = await this.frmRootSrv.deleteone(  locEmpSecondNameId );
            if(data != null) {
                this.Interface2Values(data, false);
