...
namespace ModelServicesPrismModule.Phonebook.LprPhone02 {
    public class LprPhone02ViewDatasource: ILprPhone02ViewDatasource
    {
        protected readonly string _CurrentViewName   = "LprPhone02View";
        ...
        protected ILprPhone02ViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public LprPhone02ViewDatasource(ILprPhone02ViewService frs,
        ...
        public bool Interface2Values(ILprPhone02View data, bool doNotify = true) {
            if(data == null) {
        ...
        public ILprPhone02View Values2Interface() {
            object locobjval;

            LprPhone02View rslt = new LprPhone02View(); 
        ...
        protected ILprPhone02ViewFilter DefineFilterOneProp(ILprPhone02ViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new LprPhone02ViewFilter(); 
        ...
        protected ILprPhone02ViewFilter GetFilterByCurrDirMstrs() {
            ILprPhone02ViewFilter flt = new LprPhone02ViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<ILprPhone02View>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<ILprPhone02View>();
            }
            ILprPhone02ViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<ILprPhone02View>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<ILprPhone02View>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            ILprPhone02ViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            ILprPhone02ViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<ILprPhone02View>();
            }
            if(prslt.items == null) {
        ...
        public async Task getone(
            ...
            ILprPhone02View data = await this.frmRootSrv.getone(
                  EmployeeIdRef
                , LpdPhoneIdRef
                , PhoneId
            );
        ...
        public async Task updateone() {
            ...
            ILprPhone02View itm = this.Values2Interface();
            ILprPhone02View data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            ILprPhone02View itm = this.Values2Interface();
            ILprPhone02View data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            ILprPhone02View data = await this.frmRootSrv.deleteone(  locEmployeeIdRef ,   locLpdPhoneIdRef ,   locPhoneId );
            if(data != null) {
                this.Interface2Values(data, false);
