...
/*
    In the file of IModule-class of the project for the current service the following lines of code must be inserted:
        ...
        {
            ...
            containerRegistry.Register<IPhbkEmployeeViewDatasource, PhbkEmployeeViewDatasource>();
            ...
        }

*/
namespace ModelServicesPrismModule.Phonebook.PhbkEmployee {
    public class PhbkEmployeeViewDatasource: IPhbkEmployeeViewDatasource
    {
        protected readonly string _CurrentViewName   = "PhbkEmployeeView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            ...
            {"EmpFirstName", null},  // System.String
            {"EmpLastName", null},  // System.String
            {"EmpSecondName", null},  // System.String
            {"DivisionIdRef", null},  // System.Int32
            {"DDivisionName", null},  // System.String
            {"DEEntrprsName", null},  // System.String
        };
        protected IPhbkEmployeeViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public PhbkEmployeeViewDatasource(IPhbkEmployeeViewService frs,
        ...
        public bool Interface2Values(IPhbkEmployeeView data, bool doNotify = true) {
            if(data == null) {
            ...
            if(!this.isEqual(this.getValue("EmpFirstName"), data.EmpFirstName, "string")) {
            ...
            if(!this.isEqual(this.getValue("EmpSecondName"), data.EmpSecondName, "string")) {
            ...
            if(!this.isEqual(this.getValue("DDivisionName"), data.DDivisionName, "string")) {
                this.setValue("DDivisionName", data.DDivisionName);
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("DEEntrprsName"), data.DEEntrprsName, "string")) {
        ...
        public IPhbkEmployeeView Values2Interface() {
            object locobjval;

            PhbkEmployeeView rslt = new PhbkEmployeeView(); 
            locobjval = this.getValue("EmployeeId");
            rslt.EmployeeId = Convert.ToInt32(locobjval);
            locobjval = this.getValue("EmpFirstName");
            if(locobjval is null) {
                rslt.EmpFirstName =  null;
            } else {
                rslt.EmpFirstName =  locobjval.ToString();
            }
            ...
            locobjval = this.getValue("EmpSecondName");
            if(locobjval is null) {
                rslt.EmpSecondName =  null;
            } else {
                rslt.EmpSecondName =  locobjval.ToString();
            }
            locobjval = this.getValue("DivisionIdRef");
            rslt.DivisionIdRef = Convert.ToInt32(locobjval);
            locobjval = this.getValue("DDivisionName");
            if(locobjval is null) {
                rslt.DDivisionName =  null;
            } else {
                rslt.DDivisionName =  locobjval.ToString();
            }
            locobjval = this.getValue("DEEntrprsName");
            if(locobjval is null) {
                rslt.DEEntrprsName =  null;
            } else {
                rslt.DEEntrprsName =  locobjval.ToString();
            }
            return rslt;
        }
            ...
            if (this._ClientViewName == "LprEmployee01View") {
                if (this._DirectNavigation == "Employee") {
                    ...
                    if(locobjval is null) {
                        ...
                        IPhbkEmployeeView data = await this.frmRootSrv.getone(
                         locEmployeeId
                        );
                        if(data == null) {
                            this.Clear(true);
        ...
        protected IPhbkEmployeeViewFilter DefineFilterOneProp(IPhbkEmployeeViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new PhbkEmployeeViewFilter(); 
            ...
            switch(propNm) {
                ...
                case "EmpFirstName":
                    ...
                    flt.empFirstNameOprtr.Add(Oprtr);
                    if (value is null) {
                        flt.EmpFirstName.Add(null);
                    } else {
                        flt.EmpFirstName.Add(value.ToString());
                    }
                    break;
                case "EmpLastName":
        ...
        protected IPhbkEmployeeViewFilter GetFilterByCurrDirMstrs() {
            IPhbkEmployeeViewFilter flt = new PhbkEmployeeViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<IPhbkEmployeeView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<IPhbkEmployeeView>();
            }
            IPhbkEmployeeViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<IPhbkEmployeeView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<IPhbkEmployeeView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            IPhbkEmployeeViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            IPhbkEmployeeViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<IPhbkEmployeeView>();
            }
            if(prslt.items == null) {
        ...
        public async Task updateone() {
            ...
            IPhbkEmployeeView itm = this.Values2Interface();
            IPhbkEmployeeView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            IPhbkEmployeeView itm = this.Values2Interface();
            IPhbkEmployeeView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            IPhbkEmployeeView data = await this.frmRootSrv.deleteone(  locEmployeeId );
            if(data != null) {
                this.Interface2Values(data, false);
