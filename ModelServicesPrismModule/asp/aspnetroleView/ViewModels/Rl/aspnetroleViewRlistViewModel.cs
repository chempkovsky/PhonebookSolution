...
namespace ModelServicesPrismModule.asp.aspnetroleView.ViewModels.Rl {

    public class AspnetroleViewRlistViewModel: RegionAwareViewModelBase, IRegionAware 
    {
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IAspnetroleViewService FrmSrvaspnetroleView = null;
        public AspnetroleViewRlistViewModel(IAspnetroleViewService _FrmSrvaspnetroleView, 
        ...
        public void NavigateToDetailCommand(IAspnetroleView selected, string detailVwNm, string fkNvNm, string detailVwClNm) { 
        ...
        protected ObservableCollection<IWebServiceFilterMenuInterface> GetDefaultRowMenuItems() {
            return new ObservableCollection<IWebServiceFilterMenuInterface>()  {
                ...
                new WebServiceFilterMenuViewModel() { Id = "RowMIAspnetrolemaskViewRlistUserControlAspNetRole", Caption="Role Masks: AspNetRole", IconName="ArrowRightBold", IconColor=Color.Default, Enabled=true, Data=null, FeedbackData=null, Command = RowMenuItemsCommand},
            };
        }
        ...
        public void SformAddItemCommand() {
            ...
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetroleViewRaddUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        ...
        public void SformUpdItemCommand(IAspnetroleView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if (!CanUpdate) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
            ...
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetroleViewRupdUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion

        #region SformDelItemCommand
        public void SformDelItemCommand(IAspnetroleView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if(!CanDelete) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
            ...
            navigationParameters.Add("pkpId", selected.Id);
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetroleViewRdelUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        
        #region SformViewItemCommand
        public void SformViewItemCommand(IAspnetroleView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            NavigationParameters navigationParameters = new NavigationParameters();
            foreach(IWebServiceFilterRsltInterface hf in HiddenFilters) {
            ...
            navigationParameters.Add("pkpId", selected.Id);
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetroleViewRviewUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        ...
        protected void RowMenuItemsCommandExecute(object prm)
        {
            ...
            switch(mi.Id) {
                ...
                case "RowViewMI":
                    ...
                    break;
                    case "RowMIAspnetrolemaskViewRlistUserControlAspNetRole":
                        NavigateToDetailCommand(arow, "aspnetrolemaskView", "AspNetRole", "AspnetrolemaskViewRlistUserControl");
                        break;
                default:
                    break;
            }         
