...
namespace ModelServicesPrismModule.asp.aspnetroleView.ViewModels.Sform {
    public class AspnetroleViewSformViewModel: INotifyPropertyChanged, ISformViewModelInterface, IBindingContextChanged, IDestructible
    {
        protected IAspnetroleViewService FrmRootSrv=null;
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IDialogService _dialogService=null;
    

        bool CnFllscn = false;
        public AspnetroleViewSformViewModel(IAppGlblSettingsService GlblSettingsSrv, IAspnetroleViewService FrmRootSrv, 
            ...
            CnFllscn = (GlblSettingsSrv.GetViewModelMask("aspnetroleView") & 16) == 16;
        }
        ...
        #region DataSource
        protected IEnumerable<IAspnetroleView> _DataSource = new ObservableCollection<IAspnetroleView>();
        public IEnumerable<IAspnetroleView> DataSource
        { 
            get
            {
                return _DataSource;
        ...
        public async Task HiddenFiltersPropertyChanged(object Sender, object OldValue, object NewValue)
        {
            ...
            ObservableCollection<IWebServiceFilterDefInterface> fltDf = 
                new ObservableCollection<IWebServiceFilterDefInterface>() {
                    new WebServiceFilterDefViewModel() {fltrName="Id", fltrCaption="Role Id",  fltrDataType="string", fltrMaxLen=128, fltrMin=null, fltrMax=null },
                    new WebServiceFilterDefViewModel() {fltrName="Name", fltrCaption="Role Name",  fltrDataType="string", fltrMaxLen=128, fltrMin=null, fltrMax=null },
                };
            ActualCurrentPage = 0;
            defineSearchMethod();
            await onFilter();
        ...
        protected async Task onFilter() {
            ...
            if(SearchMethod == "NoSearchMethod") {
                ClearDataSource();
                DataSource = new ObservableCollection<IAspnetroleView>();
                SelectedRow = null;
                IsInQuery = false;
                return;
            }
            ...
            if ((SearchMethod == "FullScan")
                || (SearchMethod == "ScanByUkUnqName")
                || (SearchMethod == "ScanByUkPrimary")
            ) {

                IAspnetroleViewFilter flt  = FrmRootSrv.GetFilter();
                    ...
                    foreach(IWebServiceFilterRsltInterface e in CurrentFilter) {
                        ...
                        switch(e.fltrName) {
                            case "Id":
                                ...
                                break;
                            case "Name":
                                if (flt.Name == null) flt.Name = new List<System.String>();
                                flt.Name.Add((System.String)e.fltrValue);
                                if (flt.nameOprtr == null) flt.nameOprtr = new List<string>();
                                flt.nameOprtr.Add(e.fltrOperator);
                                break;
                            default: break;
                        }
                    }
                }
                IaspnetroleViewPage rslt = await this.FrmRootSrv.getwithfilter(flt);
                if(rslt != null) {
                    ...
                    SelectedRow = null;
                    ObservableCollection<IAspnetroleView> ds = DataSource as ObservableCollection<IAspnetroleView>;
                    IsDsDestroyed = true;
                    if (ds == null) { ds = new ObservableCollection<IAspnetroleView>(); } else { ds.Clear(); }
                    IsDsDestroyed = false;
                    DataSource = null; 
                    if(rslt.items != null) {
                        foreach(IAspnetroleView itm in rslt.items) {
        ...
        public void SformAfterAddItemCommand(object sender, object item) {
            IAspnetroleView prm = item as  IAspnetroleView;
            if(prm == null) return;
            ObservableCollection<IAspnetroleView> ds = DataSource as ObservableCollection<IAspnetroleView>;
        ...
        #region ScanByUkUnqNameFilterDefinitions
        protected IList<IUniqServiceFilterDefInterface> _ScanByUkUnqNameFilterDefinitions =
            new List<IUniqServiceFilterDefInterface>() {
                new UniqServiceFilterDef() { fltrName = "Name", fltrDispMemb = "Name", fltrTextMemb = "Name", fltrCaption = "Role Name", fltrDataType = "string", fltrMaxLen = 128, fltrMin = null, fltrMax = null },
            };
        public IList<IUniqServiceFilterDefInterface> ScanByUkUnqNameFilterDefinitions {
            get {
                return _ScanByUkUnqNameFilterDefinitions;
            }
        }

        protected IAspnetroleView _ScanByUkUnqNameNameItem = null;
        public IAspnetroleView ScanByUkUnqNameNameItem {
            get {
                return _ScanByUkUnqNameNameItem;
            }
            set {
                if (_ScanByUkUnqNameNameItem != value) {
                    _ScanByUkUnqNameNameItem = value;
                    OnPropertyChanged();
                    OnPropertyChanged("ScanByUkUnqNameNameText");
                }
            }
        }
        
        protected IList<IAspnetroleView> _ScanByUkUnqNameItemsSource = null;
        public IList<IAspnetroleView> ScanByUkUnqNameItemsSource {
            get {
                return _ScanByUkUnqNameItemsSource;
            }
            set {
                if(_ScanByUkUnqNameItemsSource != value) {
                    _ScanByUkUnqNameItemsSource = value;
                    OnPropertyChanged();
                }
            }
        }

        protected string _ScanByUkUnqNameNameText = "";
        public string ScanByUkUnqNameNameText {
            get {
                IAspnetroleView dmObj = ScanByUkUnqNameNameItem;
                if(dmObj != null) {
                    return dmObj.Name;
                } else {
                    return _ScanByUkUnqNameNameText;
                }

            }
            set {
                if(_ScanByUkUnqNameNameText != value) {
                    if(value is null)
                        _ScanByUkUnqNameNameText = "";
                    else
                        _ScanByUkUnqNameNameText = value;
                    OnPropertyChanged();
                }
            }
        }
        protected ICommand _ScanByUkUnqNameTextChangedCommand = null;
        public ICommand ScanByUkUnqNameTextChangedCommand
        {
            get
            {
                return _ScanByUkUnqNameTextChangedCommand ?? (_ScanByUkUnqNameTextChangedCommand = new Command((prm) => ScanByUkUnqNameTextChangedCommandExecute(prm), (prm) => ScanByUkUnqNameTextChangedCommandCanExecute(prm)));
            }
        }
        // Reason: UserInput = 0, ProgrammaticChange = 1, SuggestionChosen = 2
        protected async void ScanByUkUnqNameTextChangedCommandExecute(object prm)
        {
            ...
            IAspnetroleView itm = null;
            IAspnetroleViewFilter fltr = FrmRootSrv.GetFilter();
            fltr.page = 0;
            fltr.pagesize = 15;
            bool runQuery = true;
            itm = ScanByUkUnqNameNameItem;
            if (k > 0)  { 
                if(itm is null) {
                    runQuery = false;
                } else {
                    fltr.Name = new List<System.String>() { itm.Name }; 
                    fltr.nameOprtr = new List<string>() { "eq" };
                }
             } else if(k == 0) { 
                ScanByUkUnqNameNameItem = null;
                ScanByUkUnqNameNameText = tprm.QueryText;
                string qstr = tprm.QueryText;
                if(qstr is null) qstr = "";
                if(_ScanByUkUnqNameNameText == qstr) return;
                _ScanByUkUnqNameNameText = qstr;


                    fltr.Name = new List<string>() { qstr };
                fltr.nameOprtr = new List<string>() { "lk" };
             }
            if (runQuery) {
                IaspnetroleViewPage rslt = await FrmRootSrv.getmanybyrepunqUnqName(fltr);
                if(rslt is null) {
                    ScanByUkUnqNameItemsSource = null;
                } else {
                    ScanByUkUnqNameItemsSource = rslt.items;
                }
            } else {
                ScanByUkUnqNameItemsSource = null;
            }
        }
        protected bool ScanByUkUnqNameTextChangedCommandCanExecute(object prm)
        {
            return true;
        }
        protected ICommand _ScanByUkUnqNameQuerySubmitted = null;
        public ICommand ScanByUkUnqNameQuerySubmitted
        {
            get
            {
                return _ScanByUkUnqNameQuerySubmitted ?? (_ScanByUkUnqNameQuerySubmitted = new Command((prm) => ScanByUkUnqNameQuerySubmittedExecute(prm), (prm) => ScanByUkUnqNameQuerySubmittedCanExecute(prm)));
            }
        }
        protected void ScanByUkUnqNameQuerySubmittedExecute(object prm)
        {
            if ((IsDsDestroyed) || (prm is null)) return;
            ...
            switch(tprm.FltDef.fltrName) {
                case "Name":
                    ScanByUkUnqNameNameItem = tprm.ChosenSuggestion as IAspnetroleView;
                    break;
                default:
                    return;
            }
        ...
        protected bool ScanByUkUnqNameQuerySubmittedCanExecute(object prm) {
            return true;
        }
        protected ICommand _ScanByUkUnqNameUnfocusedCommand = null;
        public ICommand ScanByUkUnqNameUnfocusedCommand
        {
            get
            {
                return _ScanByUkUnqNameUnfocusedCommand ?? (_ScanByUkUnqNameUnfocusedCommand = new Command((prm) => ScanByUkUnqNameUnfocusedCommandExecute(prm), (prm) => ScanByUkUnqNameUnfocusedCommandCanExecute(prm)));
            }
        }
        protected void ScanByUkUnqNameUnfocusedCommandExecute(object prm)
        {
            if ((IsDsDestroyed) || (prm is null)) return;
            ScanByUkUnqNameItemsSource = null;
        }
        protected bool ScanByUkUnqNameUnfocusedCommandCanExecute(object prm) {
            return true;
        }
        #endregion
        ...
        protected IList<IUniqServiceFilterDefInterface> _ScanByUkPrimaryFilterDefinitions =
            new List<IUniqServiceFilterDefInterface>() {
                new UniqServiceFilterDef() { fltrName = "Id", fltrDispMemb = "Id", fltrTextMemb = "Id", fltrCaption = "Role Id", fltrDataType = "string", fltrMaxLen = 128, fltrMin = null, fltrMax = null },
            };
        ...
        protected IAspnetroleView _ScanByUkPrimaryIdItem = null;
        public IAspnetroleView ScanByUkPrimaryIdItem {
            get {
                return _ScanByUkPrimaryIdItem;
            }
            set {
                if (_ScanByUkPrimaryIdItem != value) {
                    _ScanByUkPrimaryIdItem = value;
                    OnPropertyChanged();
                    OnPropertyChanged("ScanByUkPrimaryIdText");
                }
            }
        }
        
        protected IList<IAspnetroleView> _ScanByUkPrimaryItemsSource = null;
        public IList<IAspnetroleView> ScanByUkPrimaryItemsSource {
            get {
        ...
        protected string _ScanByUkPrimaryIdText = "";
        public string ScanByUkPrimaryIdText {
            get {
                IAspnetroleView dmObj = ScanByUkPrimaryIdItem;
                if(dmObj != null) {
                    return dmObj.Id;
                } else {
                    return _ScanByUkPrimaryIdText;
                }

            }
            set {
                if(_ScanByUkPrimaryIdText != value) {
                    if(value is null)
                        _ScanByUkPrimaryIdText = "";
                    else
                        _ScanByUkPrimaryIdText = value;
                    OnPropertyChanged();
                }
            }
        }
        ...
        protected async void ScanByUkPrimaryTextChangedCommandExecute(object prm)
        {
            ...
            fltr.pagesize = 15;
            bool runQuery = true;
            itm = ScanByUkPrimaryIdItem;
            if (k > 0)  { 
                if(itm is null) {
                    runQuery = false;
                } else {
                    fltr.Id = new List<System.String>() { itm.Id }; 
                    fltr.idOprtr = new List<string>() { "eq" };
                }
             } else if(k == 0) { 
                ScanByUkPrimaryIdItem = null;
                ScanByUkPrimaryIdText = tprm.QueryText;
                string qstr = tprm.QueryText;
                if(qstr is null) qstr = "";
                if(_ScanByUkPrimaryIdText == qstr) return;
                _ScanByUkPrimaryIdText = qstr;


                    fltr.Id = new List<string>() { qstr };
                fltr.idOprtr = new List<string>() { "lk" };
             }
            if (runQuery) {
                IaspnetroleViewPage rslt = await FrmRootSrv.getmanybyrepprim(fltr);
                if(rslt is null) {
        ...
        protected void ScanByUkPrimaryQuerySubmittedExecute(object prm)
        {
            ...
            switch(tprm.FltDef.fltrName) {
                case "Id":
                    ScanByUkPrimaryIdItem = tprm.ChosenSuggestion as IAspnetroleView;
                    break;
                default:
                    return;
            }
        ...
        public IList<IWebServiceFilterMenuInterface> defineSearchMethodMenuItemsData() {
            ...
            if(hasNtExternal) {
                ...
                if(this.HiddenFiltersVM.Count() == reqFkPrps.Count) {
                    ...
                    if(insmi) {
                        rslt.Add( 
                            new WebServiceFilterMenuViewModel() { Id = "ScanByUkUnqName", Caption="filter by UnqName", IconName=IconFont.Refresh, IconColor=Color.Default, Enabled=true, Data=null, Command = TableMenuItemsCommand}
                        );
                    }
                }
            }
