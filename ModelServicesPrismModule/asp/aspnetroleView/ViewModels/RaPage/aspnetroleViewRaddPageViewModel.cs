...
namespace ModelServicesPrismModule.asp.aspnetroleView.ViewModels.RaPage {
    public class AspnetroleViewRaddPageViewModel: INotifyPropertyChanged, INavigationAware, IDestructible  
    {
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IAspnetroleViewService FrmSrvaspnetroleView = null;
        protected INavigationService _navigationService = null;

        public AspnetroleViewRaddPageViewModel(IAspnetroleViewService _FrmSrvaspnetroleView, IAppGlblSettingsService GlblSettingsSrv, INavigationService navigationService) {
        ...
        public void OnNavigatedTo(INavigationParameters prms) {
            ...
            if ((!resetHF) && (hf.Count > 0)) {
                ...
                Caption = "Add item for Role";
                if (resetHF) { HiddenFilters = hf; }
                FormControlModel = FrmSrvaspnetroleView.CopyToModelNotify(null,null); // this is correct: cleanup twice
                EformMode = EformModeEnum.AddMode;
                FormControlModel = null; // since DateTime-issue this is correct: FrmSrvaspnetroleView.CopyToModelNotify(null,null);
                IsParentLoaded = true;
        }
        #endregion
