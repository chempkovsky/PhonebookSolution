...
namespace ModelServicesPrismModule.asp.aspnetrolemaskView.ViewModels.Rl {

    public class AspnetrolemaskViewRlistViewModel: RegionAwareViewModelBase, IRegionAware 
    {
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IAspnetrolemaskViewService FrmSrvaspnetrolemaskView = null;
        public AspnetrolemaskViewRlistViewModel(IAspnetrolemaskViewService _FrmSrvaspnetrolemaskView, 
        ...
        public void NavigateToDetailCommand(IAspnetrolemaskView selected, string detailVwNm, string fkNvNm, string detailVwClNm) { 
            ...
            navigationParameters.Add("HiddenFilter", this.FrmSrvaspnetrolemaskView.getHiddenFilterByRow(selected, fkNvNm));  
        ...
        public void SformAddItemCommand() {
            ...
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetrolemaskViewRaddUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        ...
        public void SformUpdItemCommand(IAspnetrolemaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if (!CanUpdate) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
            ...
            navigationParameters.Add("pkpRName", selected.RName);
            navigationParameters.Add("pkpModelPkRef", selected.ModelPkRef);
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetrolemaskViewRupdUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion

        #region SformDelItemCommand
        public void SformDelItemCommand(IAspnetrolemaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if(!CanDelete) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
            ...
            navigationParameters.Add("pkpRName", selected.RName);
            navigationParameters.Add("pkpModelPkRef", selected.ModelPkRef);
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetrolemaskViewRdelUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        
        #region SformViewItemCommand
        public void SformViewItemCommand(IAspnetrolemaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            NavigationParameters navigationParameters = new NavigationParameters();
            foreach(IWebServiceFilterRsltInterface hf in HiddenFilters) {
            ...
            navigationParameters.Add("pkpRName", selected.RName);
            navigationParameters.Add("pkpModelPkRef", selected.ModelPkRef);
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetrolemaskViewRviewUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        ...
        protected void RowMenuItemsCommandExecute(object prm)
        {
            ...
            IWebServiceFilterMenuInterface mi = prm as IWebServiceFilterMenuInterface;
            if (mi == null) return;
            IAspnetrolemaskView arow = mi.FeedbackData as IAspnetrolemaskView;
            if (arow == null) return;
            switch(mi.Id) {
                case "RowUpdMI":
       ...
       public async void OnNavigatedTo(INavigationContext navigationContext) {
            ...
            if ((PermissionMask & 1) != 1) {
                if (navigationContext.NavigationService.Journal.CurrentEntry.Uri.OriginalString == "AccessDeniedUserControl")
                {
                    ...
                    throw new Exception("Access Denied to navigate aspnetrolemaskView");
                } else {
            ...
            if ((!IsParentLoaded) || (IsNavigatedBack)) {
                ...
                if(prms.ContainsKey("HiddenFilter")) {
                    IList<IWebServiceFilterRsltInterface> hflts = 
                        FrmSrvaspnetrolemaskView.getHiddenFilterAsFltRslt(
            ...
            if (prms.ContainsKey("EformModeEnum")) {
                ...
                if ((mode == EformModeEnum.AddMode) || (mode == EformModeEnum.UpdateMode)) {
                    ...
                    IAspnetrolemaskView data = FrmSrvaspnetrolemaskView.CopyToModel(null,null);
                    data.RName = pkpRName;
                    data.ModelPkRef = pkpModelPkRef;
