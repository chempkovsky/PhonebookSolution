...
namespace ModelServicesPrismModule.asp.aspnetrolemaskView.ViewModels.RlPage {

    public class AspnetrolemaskViewRlistPageViewModel: INotifyPropertyChanged, INavigationAware, IDestructible 
    {
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IAspnetrolemaskViewService FrmSrvaspnetrolemaskView = null;
        protected INavigationService _navigationService;
        public AspnetrolemaskViewRlistPageViewModel(IAspnetrolemaskViewService _FrmSrvaspnetrolemaskView, IAppGlblSettingsService GlblSettingsSrv, INavigationService navigationService) {
        ...
        public async Task NavigateToDetailCommand(IAspnetrolemaskView selected, string detailVwNm, string fkNvNm, string detailVwClNm) { 
            if(IsDestroyed) return;
        ...
        public async Task SformAddItemCommand() {
            ...
            INavigationResult nr2 = await _navigationService.NavigateAsync("AspnetrolemaskViewRaddPage", navigationParameters);
            OnNavigationResult(nr2);
        }
        #endregion
        ...
        public async Task SformUpdItemCommand(IAspnetrolemaskView selected) {
            if(IsDestroyed) return;
            if (selected == null) return;
            ...
            INavigationResult nr2 = await _navigationService.NavigateAsync("AspnetrolemaskViewRupdPage", navigationParameters);
            OnNavigationResult(nr2);
        }
        #endregion

        #region SformDelItemCommand
        public async Task SformDelItemCommand(IAspnetrolemaskView selected) {
            if(IsDestroyed) return;
            if (selected == null) return;
            ...
            navigationParameters.Add("pkpRName", selected.RName);
            navigationParameters.Add("pkpModelPkRef", selected.ModelPkRef);
            INavigationResult nr2 = await _navigationService.NavigateAsync("AspnetrolemaskViewRdelPage", navigationParameters);
            OnNavigationResult(nr2);
        }
        #endregion

        #region SformViewItemCommand
        public async Task SformViewItemCommand(IAspnetrolemaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            NavigationParameters navigationParameters = new NavigationParameters();
            foreach(IWebServiceFilterRsltInterface hf in HiddenFilters) {
            ...
            navigationParameters.Add("pkpRName", selected.RName);
            navigationParameters.Add("pkpModelPkRef", selected.ModelPkRef);
            INavigationResult nr2 = await _navigationService.NavigateAsync("AspnetrolemaskViewRviewPage", navigationParameters);
            OnNavigationResult(nr2);
        }
        #endregion
