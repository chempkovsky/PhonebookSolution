...
namespace ModelServicesPrismModule.asp.aspnetusermaskView {
    public class AspnetusermaskViewService: IAspnetusermaskViewService
    {
        ...
        public AspnetusermaskViewService(IAppGlblSettingsService agstt) {
      ...
      {"UserId", ("UserId", "", "", true, false, true, false, "string")},  // System.String
      ...
      {"ModelPkRef", ("ModelPkRef", "", "", false, false, true, false, "int32")},  // System.Int32
      {"UEmail", ("Email", "AspNetUser", "AspNetUser", false, false, false, false, "string")},  // System.String
    ...
    protected Dictionary<(string viewNm, string navNm, string propNm), string> _M2cfm = 
        ...
        {("aspnetuserView", "AspNetUser", "Email"), "UEmail"},
    ...
    public Dictionary<(string viewNm, string navNm, string propNm), object> getHiddenFilterByRow(IAspnetusermaskView rw, string nvNm) {
        ...
        foreach(string i in this._Values.Keys) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                rslt[("aspnetusermaskView",nvNm,i)] = rw.GetType().GetProperty(i).GetValue(rw);
            }
        }
        return rslt;
    }
    ...
    public Dictionary<(string viewNm, string navNm, string propNm), string> getC2mfm() {
        ...
        public async Task<IaspnetusermaskViewPage> getwithfilter(IAspnetusermaskViewFilter filter) {
            string prms = null;
            if(filter != null) {
            ...
            try
            {
                ...
                return  JsonConvert.DeserializeObject<AspnetusermaskViewPage>(responseBody) ;
            } 
            catch(Exception e)
            {
                string exceptionMsg = "AspnetusermaskViewService.getwithfilter : " + e.Message;
        ...
        public IAspnetusermaskViewNotify CopyToModelNotify(IAspnetusermaskView  src, IAspnetusermaskViewNotify dest = null) {
            if(dest == null) dest = new AspnetusermaskViewNotify();
            if(src == null) return dest;
        ...
        public IAspnetusermaskView CopyToModel(IAspnetusermaskView  src, IAspnetusermaskView dest = null) {
            if(dest == null) dest = new AspnetusermaskView();
            if(src == null) return dest;
            dest.UserId = src.UserId;
            dest.Mask1 = src.Mask1;
            dest.Mask2 = src.Mask2;
        ...
        public IAspnetusermaskViewFilter GetFilter() {
            return new AspnetusermaskViewFilter();
        }

        public IList<IWebServiceFilterRsltInterface> Row2FilterRslt(IAspnetusermaskView r) {
            IList<IWebServiceFilterRsltInterface> rslt = new List<IWebServiceFilterRsltInterface>();
        ...
        public IList<IWebServiceFilterRsltInterface> RowPrim2FilterRslt(IAspnetusermaskView r) {
            IList<IWebServiceFilterRsltInterface> rslt = new List<IWebServiceFilterRsltInterface>();
            if (r == null) return rslt;
            foreach(string i in this._Values.Keys) {
        ...
        public IAspnetusermaskViewFilter FilterRslt2Filter(IWebServiceFilterRsltInterface e, IAspnetusermaskViewFilter dest) {
            IAspnetusermaskViewFilter flt = dest;
            if(dest is null) {
                flt = GetFilter();
            }
        ...
        public IAspnetusermaskViewFilter FilterRslt2Filter(IList<IWebServiceFilterRsltInterface> src, IAspnetusermaskViewFilter dest) {
            IAspnetusermaskViewFilter rslt = dest;
            if(dest is null) {
                rslt = GetFilter();
            }
