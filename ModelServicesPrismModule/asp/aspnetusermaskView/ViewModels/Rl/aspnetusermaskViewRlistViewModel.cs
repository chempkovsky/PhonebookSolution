...
namespace ModelServicesPrismModule.asp.aspnetusermaskView.ViewModels.Rl {

    public class AspnetusermaskViewRlistViewModel: RegionAwareViewModelBase, IRegionAware 
    {
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IAspnetusermaskViewService FrmSrvaspnetusermaskView = null;
        public AspnetusermaskViewRlistViewModel(IAspnetusermaskViewService _FrmSrvaspnetusermaskView, 
            ...
            PermissionMask = GlblSettingsSrv.GetViewModelMask("aspnetusermaskView");
        ...
        public void NavigateToDetailCommand(IAspnetusermaskView selected, string detailVwNm, string fkNvNm, string detailVwClNm) { 
            ...
            navigationParameters.Add("HiddenFilter", this.FrmSrvaspnetusermaskView.getHiddenFilterByRow(selected, fkNvNm));  
        ...
        public void SformAddItemCommand() {
            ...
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        ...
        public void SformUpdItemCommand(IAspnetusermaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if (!CanUpdate) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
        ...
        public void SformDelItemCommand(IAspnetusermaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if(!CanDelete) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
        ...
        public void SformViewItemCommand(IAspnetusermaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            NavigationParameters navigationParameters = new NavigationParameters();
            foreach(IWebServiceFilterRsltInterface hf in HiddenFilters) {
        ...
        protected void RowMenuItemsCommandExecute(object prm)
        {
            ...
            IWebServiceFilterMenuInterface mi = prm as IWebServiceFilterMenuInterface;
            if (mi == null) return;
            IAspnetusermaskView arow = mi.FeedbackData as IAspnetusermaskView;
            if (arow == null) return;
            switch(mi.Id) {
                case "RowUpdMI":
       ...
       public async void OnNavigatedTo(INavigationContext navigationContext) {
            ...
            if ((PermissionMask & 1) != 1) {
                if (navigationContext.NavigationService.Journal.CurrentEntry.Uri.OriginalString == "AccessDeniedUserControl")
                {
                    ...
                    throw new Exception("Access Denied to navigate aspnetusermaskView");
                } else {
            ...
            if ((!IsParentLoaded) || (IsNavigatedBack)) {
                ...
                if(prms.ContainsKey("HiddenFilter")) {
                    IList<IWebServiceFilterRsltInterface> hflts = 
                        FrmSrvaspnetusermaskView.getHiddenFilterAsFltRslt(
...
// "select one" method is not defined for the aspnetusermaskView
          
                } else {
                    IAspnetusermaskView data = FrmSrvaspnetusermaskView.CopyToModel(null,null);
