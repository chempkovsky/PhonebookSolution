...
namespace ModelServicesPrismModule.asp.aspnetusermaskView.ViewModels.RlPage {

    public class AspnetusermaskViewRlistPageViewModel: INotifyPropertyChanged, INavigationAware, IDestructible 
    {
        ...
        public async Task NavigateToDetailCommand(IAspnetusermaskView selected, string detailVwNm, string fkNvNm, string detailVwClNm) { 
            if(IsDestroyed) return;
        ...
        public async Task SformAddItemCommand() {
            ...
            INavigationResult nr2 = await _navigationService.NavigateAsync("", navigationParameters);
            OnNavigationResult(nr2);
        }
        #endregion
        ...
        public async Task SformUpdItemCommand(IAspnetusermaskView selected) {
            if(IsDestroyed) return;
            if (selected == null) return;
        ...
        public async Task SformDelItemCommand(IAspnetusermaskView selected) {
            if(IsDestroyed) return;
            if (selected == null) return;
        ...
        public async Task SformViewItemCommand(IAspnetusermaskView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            NavigationParameters navigationParameters = new NavigationParameters();
            foreach(IWebServiceFilterRsltInterface hf in HiddenFilters) {
