...
/*
    In the file of IModule-class of the project for the current service the following lines of code must be inserted:
        ...
        {
            ...
            containerRegistry.Register<IAspnetmodelViewDatasource, AspnetmodelViewDatasource>();
            ...
        }

*/
namespace ModelServicesPrismModule.asp.aspnetmodelView {
    public class AspnetmodelViewDatasource: IAspnetmodelViewDatasource
    {
        protected readonly string _CurrentViewName   = "aspnetmodelView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            {"ModelPk", null},  // System.Int32
            {"ModelName", null},  // System.String
            {"ModelDescription", null},  // System.String
        };
        protected IAspnetmodelViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public AspnetmodelViewDatasource(IAspnetmodelViewService frs,
        ...
        public bool Interface2Values(IAspnetmodelView data, bool doNotify = true) {
            if(data == null) {
                return this.Clear(doNotify);
            }
            bool hasChanged  = false;
            bool aftrMstrChngd = false;
            if(!this.isEqual(this.getValue("ModelPk"), data.ModelPk, "int32")) {
                this.setValue("ModelPk", data.ModelPk);
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("ModelName"), data.ModelName, "string")) {
                this.setValue("ModelName", data.ModelName);
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("ModelDescription"), data.ModelDescription, "string")) {
        ...
        public IAspnetmodelView Values2Interface() {
            object locobjval;

            AspnetmodelView rslt = new AspnetmodelView(); 
            locobjval = this.getValue("ModelPk");
            rslt.ModelPk = Convert.ToInt32(locobjval);
            locobjval = this.getValue("ModelName");
            if(locobjval is null) {
                rslt.ModelName =  null;
            } else {
                rslt.ModelName =  locobjval.ToString();
            }
            locobjval = this.getValue("ModelDescription");
            if(locobjval is null) {
                rslt.ModelDescription =  null;
            } else {
                rslt.ModelDescription =  locobjval.ToString();
            }
            return rslt;
        }
        ...
        public async Task DoSubmitOnDetailChanged(object sender)  {
            ...
            if (this._ClientViewName == "aspnetrolemaskView") {
                if (this._DirectNavigation == "AspNetModel") {
                    ...
                    System.Int32  locModelPk;
                    locobjval = this.getValue("ModelPk"); 
                    if(locobjval is null) {
                        appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                        return;
                    } else {
                        locModelPk = Convert.ToInt32(locobjval);
                    }
                        IAspnetmodelView data = await this.frmRootSrv.getone(
                         locModelPk
                        );
                        if(data == null) {
                            this.Clear(true);
        ...
        protected IAspnetmodelViewFilter DefineFilterOneProp(IAspnetmodelViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new AspnetmodelViewFilter(); 
            ...
            switch(propNm) {
                case "ModelPk":
                    ...
                    flt.modelPkOprtr.Add(Oprtr);
                    flt.ModelPk.Add(Convert.ToInt32(value));
                    break;
                case "ModelName":
                    if(flt.ModelName == null) flt.ModelName = new List<System.String>();
                    if(flt.modelNameOprtr == null) flt.modelNameOprtr = new List< string > ();
                    flt.modelNameOprtr.Add(Oprtr);
                    if (value is null) {
                        flt.ModelName.Add(null);
                    } else {
                        flt.ModelName.Add(value.ToString());
                    }
                    break;
            }
            return flt;
        }
        protected IAspnetmodelViewFilter GetFilterByCurrDirMstrs() {
            IAspnetmodelViewFilter flt = new AspnetmodelViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<IAspnetmodelView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<IAspnetmodelView>();
            }
            IaspnetmodelViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<IAspnetmodelView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<IAspnetmodelView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            IAspnetmodelViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            IaspnetmodelViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<IAspnetmodelView>();
            }
            if(prslt.items == null) {
        ...
        public async Task updateone() {
            ...
            IAspnetmodelView itm = this.Values2Interface();
            IAspnetmodelView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            IAspnetmodelView itm = this.Values2Interface();
            IAspnetmodelView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            IAspnetmodelView data = await this.frmRootSrv.deleteone(  locModelPk );
            if(data != null) {
                this.Interface2Values(data, false);
