...
namespace ModelServicesPrismModule.asp.aspnetmodelView.ViewModels.Sform {
    public class AspnetmodelViewSformViewModel: INotifyPropertyChanged, ISformViewModelInterface, IBindingContextChanged, IDestructible
    {
        protected IAspnetmodelViewService FrmRootSrv=null;
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IDialogService _dialogService=null;
    

        bool CnFllscn = false;
        public AspnetmodelViewSformViewModel(IAppGlblSettingsService GlblSettingsSrv, IAspnetmodelViewService FrmRootSrv, 
            ...
            CnFllscn = (GlblSettingsSrv.GetViewModelMask("aspnetmodelView") & 16) == 16;
        }
        ...
        #region DataSource
        protected IEnumerable<IAspnetmodelView> _DataSource = new ObservableCollection<IAspnetmodelView>();
        public IEnumerable<IAspnetmodelView> DataSource
        { 
            get
            {
                return _DataSource;
        ...
        public async Task HiddenFiltersPropertyChanged(object Sender, object OldValue, object NewValue)
        {
            ...
            ObservableCollection<IWebServiceFilterDefInterface> fltDf = 
                new ObservableCollection<IWebServiceFilterDefInterface>() {
                    new WebServiceFilterDefViewModel() {fltrName="ModelPk", fltrCaption="Model Id",  fltrDataType="int32", fltrMaxLen=null, fltrMin=null, fltrMax=null },
                    new WebServiceFilterDefViewModel() {fltrName="ModelName", fltrCaption="Model Name",  fltrDataType="string", fltrMaxLen=50, fltrMin=null, fltrMax=null },
                };
            ActualCurrentPage = 0;
            defineSearchMethod();
            await onFilter();
        ...
        protected async Task onFilter() {
            ...
            if(SearchMethod == "NoSearchMethod") {
                ClearDataSource();
                DataSource = new ObservableCollection<IAspnetmodelView>();
                SelectedRow = null;
                IsInQuery = false;
                return;
            }
            ...
            if ((SearchMethod == "FullScan")
                ...
                IAspnetmodelViewFilter flt  = FrmRootSrv.GetFilter();
                ...
                IaspnetmodelViewPage rslt = await this.FrmRootSrv.getwithfilter(flt);
                if(rslt != null) {
                    ...
                    SelectedRow = null;
                    ObservableCollection<IAspnetmodelView> ds = DataSource as ObservableCollection<IAspnetmodelView>;
                    IsDsDestroyed = true;
                    if (ds == null) { ds = new ObservableCollection<IAspnetmodelView>(); } else { ds.Clear(); }
                    IsDsDestroyed = false;
                    DataSource = null; 
                    if(rslt.items != null) {
                        foreach(IAspnetmodelView itm in rslt.items) {
        ...
        public void SformAfterAddItemCommand(object sender, object item) {
            IAspnetmodelView prm = item as  IAspnetmodelView;
            if(prm == null) return;
            ObservableCollection<IAspnetmodelView> ds = DataSource as ObservableCollection<IAspnetmodelView>;
        ...
        public void SformAfterUpdItemCommand(object sender, object item) {
            ...
            IAspnetmodelView rw = ds.Where(d => 
                        (d.ModelPk == prm.ModelPk)
                    ).FirstOrDefault();
            if (rw != null) {
        ...
        protected IList<IUniqServiceFilterDefInterface> _ScanByUkPrimaryFilterDefinitions =
            new List<IUniqServiceFilterDefInterface>() {
                new UniqServiceFilterDef() { fltrName = "ModelPk", fltrDispMemb = "ModelPk", fltrTextMemb = "ModelPk", fltrCaption = "Model Id", fltrDataType = "int32", fltrMaxLen = null, fltrMin = null, fltrMax = null },
            };
        ...
        protected IAspnetmodelView _ScanByUkPrimaryModelPkItem = null;
        public IAspnetmodelView ScanByUkPrimaryModelPkItem {
            get {
                return _ScanByUkPrimaryModelPkItem;
            }
            set {
                if (_ScanByUkPrimaryModelPkItem != value) {
                    _ScanByUkPrimaryModelPkItem = value;
                    OnPropertyChanged();
                    OnPropertyChanged("ScanByUkPrimaryModelPkText");
                }
            }
        }
        
        protected IList<IAspnetmodelView> _ScanByUkPrimaryItemsSource = null;
        public IList<IAspnetmodelView> ScanByUkPrimaryItemsSource {
            get {
        ...
        protected string _ScanByUkPrimaryModelPkText = "";
        public string ScanByUkPrimaryModelPkText {
            get {
                IAspnetmodelView dmObj = ScanByUkPrimaryModelPkItem;
                if(dmObj != null) {
                    return dmObj.ModelPk.ToString();
                } else {
                    return _ScanByUkPrimaryModelPkText;
                }

            }
            set {
                if(_ScanByUkPrimaryModelPkText != value) {
                    if(value is null)
                        _ScanByUkPrimaryModelPkText = "";
                    else
                        _ScanByUkPrimaryModelPkText = value;
                    OnPropertyChanged();
                }
            }
        }
        ...
        protected async void ScanByUkPrimaryTextChangedCommandExecute(object prm)
        {
            ...
            IAspnetmodelView itm = null;
            IAspnetmodelViewFilter fltr = FrmRootSrv.GetFilter();
            fltr.page = 0;
            fltr.pagesize = 15;
            bool runQuery = true;
            itm = ScanByUkPrimaryModelPkItem;
            if (k > 0)  { 
                if(itm is null) {
                    runQuery = false;
                } else {
                    fltr.ModelPk = new List<System.Int32>() { itm.ModelPk }; 
                    fltr.modelPkOprtr = new List<string>() { "eq" };
                }
             } else if(k == 0) { 
                ScanByUkPrimaryModelPkItem = null;
                ScanByUkPrimaryModelPkText = tprm.QueryText;
                string qstr = tprm.QueryText;
                if(qstr is null) qstr = "";
                if(_ScanByUkPrimaryModelPkText == qstr) return;
                _ScanByUkPrimaryModelPkText = qstr;


                {
                    System.Int32 val;
                    if (System.Int32.TryParse(qstr, out val)) {
                        fltr.ModelPk = new List<System.Int32>() { val };
                    }
                }
                fltr.modelPkOprtr = new List<string>() { "lk" };
             }
            if (runQuery) {
                IaspnetmodelViewPage rslt = await FrmRootSrv.getmanybyrepprim(fltr);
                if(rslt is null) {
        ...
        protected void ScanByUkPrimaryQuerySubmittedExecute(object prm)
        {
            ...
            switch(tprm.FltDef.fltrName) {
                case "ModelPk":
                    ScanByUkPrimaryModelPkItem = tprm.ChosenSuggestion as IAspnetmodelView;
                    break;
                default:
                    return;
            }
