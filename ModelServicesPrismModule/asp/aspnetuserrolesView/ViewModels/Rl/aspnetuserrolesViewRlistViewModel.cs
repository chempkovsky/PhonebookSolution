...
namespace ModelServicesPrismModule.asp.aspnetuserrolesView.ViewModels.Rl {

    public class AspnetuserrolesViewRlistViewModel: RegionAwareViewModelBase, IRegionAware 
    {
        protected IAppGlblSettingsService GlblSettingsSrv=null;
        protected IAspnetuserrolesViewService FrmSrvaspnetuserrolesView = null;
        public AspnetuserrolesViewRlistViewModel(IAspnetuserrolesViewService _FrmSrvaspnetuserrolesView, 
        ...
        public void NavigateToDetailCommand(IAspnetuserrolesView selected, string detailVwNm, string fkNvNm, string detailVwClNm) { 
        ...
        public void SformAddItemCommand() {
            ...
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetuserrolesViewRaddUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        ...
        public void SformUpdItemCommand(IAspnetuserrolesView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if (!CanUpdate) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
            ...
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetuserrolesViewRupdUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion

        #region SformDelItemCommand
        public void SformDelItemCommand(IAspnetuserrolesView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            if(!CanDelete) {
                CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AccessDeniedUserControl", UriKind.Relative), OnNavigationResult);
            ...
            navigationParameters.Add("pkpUserId", selected.UserId);
            navigationParameters.Add("pkpRoleId", selected.RoleId);
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetuserrolesViewRdelUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
        
        #region SformViewItemCommand
        public void SformViewItemCommand(IAspnetuserrolesView selected) {
            if (IsDestroyed) return;
            if (selected == null) return;
            NavigationParameters navigationParameters = new NavigationParameters();
            foreach(IWebServiceFilterRsltInterface hf in HiddenFilters) {
            ...
            navigationParameters.Add("pkpUserId", selected.UserId);
            navigationParameters.Add("pkpRoleId", selected.RoleId);
            CurrentNavigationContext.NavigationService.RequestNavigate(new Uri("AspnetuserrolesViewRviewUserControl", UriKind.Relative), OnNavigationResult, navigationParameters);
        }
        #endregion
