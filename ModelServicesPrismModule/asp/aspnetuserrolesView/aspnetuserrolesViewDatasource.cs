...
namespace ModelServicesPrismModule.asp.aspnetuserrolesView {
    public class AspnetuserrolesViewDatasource: IAspnetuserrolesViewDatasource
    {
        protected readonly string _CurrentViewName   = "aspnetuserrolesView";
        ...
        protected Dictionary<string,object> _Values = new Dictionary<string,object> {
            ...
            {"RoleId", null},  // System.String
            {"ULockoutEnd", null},  // System.DateTimeOffset ?
            {"UUserName", null},  // System.String
            {"RName", null},  // System.String
        };
        protected IAspnetuserrolesViewService frmRootSrv;
        protected IAppGlblSettingsService appGlblSettings;

        public AspnetuserrolesViewDatasource(IAspnetuserrolesViewService frs,
        ...
        public bool Interface2Values(IAspnetuserrolesView data, bool doNotify = true) {
            if(data == null) {
            ...
            if(!this.isEqual(this.getValue("RoleId"), data.RoleId, "string")) {
                this.setValue("RoleId", data.RoleId);
                aftrMstrChngd = (this._CurrentlyDirectMasterNavs.IndexOf("AspNetRole") >= 0) || aftrMstrChngd;
                hasChanged = true;
            }
            if(!this.isEqual(this.getValue("ULockoutEnd"), data.ULockoutEnd, "datetimeoffset")) {
        ...
        public IAspnetuserrolesView Values2Interface() {
            object locobjval;

            DateTimeOffset dtofst;
            AspnetuserrolesView rslt = new AspnetuserrolesView(); 
            ...
            locobjval = this.getValue("RoleId");
            if(locobjval is null) {
                rslt.RoleId =  null;
            } else {
                rslt.RoleId =  locobjval.ToString();
            }
            locobjval = this.getValue("ULockoutEnd");
            if(locobjval is null) {
                rslt.ULockoutEnd = null;
            } else {
                if(DateTimeOffset.TryParse(locobjval.ToString(), out dtofst)) {
                    rslt.ULockoutEnd = dtofst;
                } else {
                    rslt.ULockoutEnd = null;
                }
            }
        ...
        protected IAspnetuserrolesViewFilter DefineFilterOneProp(IAspnetuserrolesViewFilter flt, string propNm, object value, string Oprtr) {
            if(flt == null) flt = new AspnetuserrolesViewFilter(); 
            ...
            switch(propNm) {
                ...
                case "RoleId":
                    ...
                    flt.roleIdOprtr.Add(Oprtr);
                    if (value is null) {
                        flt.RoleId.Add(null);
                    } else {
                        flt.RoleId.Add(value.ToString());
                    }
                    break;
                case "UUserName":
                    if(flt.UUserName == null) flt.UUserName = new List<System.String>();
                    if(flt.uUserNameOprtr == null) flt.uUserNameOprtr = new List< string > ();
                    flt.uUserNameOprtr.Add(Oprtr);
                    if (value is null) {
                        flt.UUserName.Add(null);
                    } else {
                        flt.UUserName.Add(value.ToString());
                    }
                    break;
                case "RName":
        ...
        protected IAspnetuserrolesViewFilter GetFilterByCurrDirMstrs() {
            IAspnetuserrolesViewFilter flt = new AspnetuserrolesViewFilter(); 
            Dictionary<(string viewNm, string navNm, string propNm), (bool isMstrReq, string propNm)> M2cKeyfm = this.frmRootSrv.getM2cKeyfm();
            foreach(var lkey in M2cKeyfm.Keys) {
                if (this.isSetValue(M2cKeyfm[lkey].propNm)) {
        ...
        public async Task<IList<IAspnetuserrolesView>> GetClActionByCurrDirMstrs() {
            if (!this.IsSetFilterByCurrDirMstrs()) {
                return new List<IAspnetuserrolesView>();
            }
            IaspnetuserrolesViewPage prslt = await this.frmRootSrv.getwithfilter(this.GetFilterByCurrDirMstrs());
            if(prslt == null) {
                return new List<IAspnetuserrolesView>();
            }
            if(prslt.items == null) {
        ...
        public async Task<IList<IAspnetuserrolesView>> GetClActionByFldFilter(string fldName, object fldVal) {
            ...
            IAspnetuserrolesViewFilter flt = this.GetFilterByCurrDirMstrs();
            this.DefineFilterOneProp(flt, fldName, fldVal, "lk");
            if (flt.orderby == null) flt.orderby = new List<string>();
            flt.orderby.Add(fldName);
            IaspnetuserrolesViewPage prslt = await this.frmRootSrv.getwithfilter(flt);
            if(prslt == null) {
                return new List<IAspnetuserrolesView>();
            }
            if(prslt.items == null) {
        ...
        public async Task getone(
            ...
            IAspnetuserrolesView data = await this.frmRootSrv.getone(
                  UserId
                , RoleId
            );
            if(data != null) {
        ...
        public async Task Refresh() {
            ...
            if (
                ...
                System.String  locUserId;
                locobjval = this.getValue("UserId"); 
                if(locobjval is null) {
                    appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                    return;
                } else {
                    locUserId = locobjval.ToString();
                }
  
                System.String  locRoleId;
                locobjval = this.getValue("RoleId"); 
                if(locobjval is null) {
                    appGlblSettings.ShowErrorMessage("http", "Not all propertes are correctly defined");
                    return;
                } else {
                    locRoleId = locobjval.ToString();
                }
                await this.getone(
                      locUserId
                    , locRoleId
        ...
        public async Task updateone() {
            ...
            IAspnetuserrolesView itm = this.Values2Interface();
            IAspnetuserrolesView data = await this.frmRootSrv.updateone(itm);
            if(data != null) {
        ...
        public async Task addone() {
            ...
            IAspnetuserrolesView itm = this.Values2Interface();
            IAspnetuserrolesView data = await this.frmRootSrv.addone(itm);
            if(data != null) {
                this.Interface2Values(data, false);
        ...
        public async Task deleteone() {
            ...
            IAspnetuserrolesView data = await this.frmRootSrv.deleteone(  locUserId ,   locRoleId );
            if(data != null) {
                this.Interface2Values(data, false);
