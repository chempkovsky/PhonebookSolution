...
namespace LpPhBkControllers.Consumers {
    ...
    public class PhbkPhoneViewExtForLkUpMsgConsumer: IConsumer<IPhbkPhoneViewExtForLkUpMsg> {
        private readonly ILogger<PhbkPhoneViewExtForLkUpMsgConsumer> _logger;
        // private readonly PhbkDbContext db;
        IServiceScopeFactory _serviceScopeFactory;

        // public PhbkPhoneViewExtForLkUpMsgConsumer(PhbkDbContext dbcontext, ILogger<PhbkPhoneViewExtForLkUpMsgConsumer> logger)
        public PhbkPhoneViewExtForLkUpMsgConsumer(IServiceScopeFactory serviceScopeFactory, ILogger<PhbkPhoneViewExtForLkUpMsgConsumer> logger)
        {
            // db = dbcontext;
        ...
        public async Task Consume(ConsumeContext<IPhbkPhoneViewExtForLkUpMsg> context)
        {
            // _logger.LogInformation("Order Submitted: {OrderId}", context.Message.OrderId);
            using (IServiceScope scope = _serviceScopeFactory.CreateScope()) {
                LpPhnPhBkContext db = scope.ServiceProvider.GetRequiredService<LpPhnPhBkContext>();
                await M2mUpdaterPhbkPhoneView.UpdateForPhbkPhoneView(db, context.Message.Action, context.Message.OldVals, context.Message.NewVals);
            }
        }
    }

    public class PhbkPhoneViewExtForLkUpMsgConsumerDefinition: ConsumerDefinition<PhbkPhoneViewExtForLkUpMsgConsumer> {
        public PhbkPhoneViewExtForLkUpMsgConsumerDefinition()
        {
            // override the default endpoint name
        ...
        protected override void ConfigureConsumer(IReceiveEndpointConfigurator endpointConfigurator,
            IConsumerConfigurator<PhbkPhoneViewExtForLkUpMsgConsumer> consumerConfigurator)
        {
