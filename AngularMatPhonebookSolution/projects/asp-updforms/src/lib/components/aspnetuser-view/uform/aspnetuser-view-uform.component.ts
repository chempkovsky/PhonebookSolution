...
@Component({
  selector: 'app-aspnetuser-view-uform',
  templateUrl: './aspnetuser-view-uform.component.html',
  styleUrls: ['./aspnetuser-view-uform.component.css']
})
export class AspnetuserViewUformComponent implements OnInit, OnDestroy, IEventEmitterPub {
    frases: {[key:string]: string}  = {
        'Not-all-props': $localize`:Not all properties are set@@AspnetuserViewUformComponent.Not-all-props:Not all properties are set`,
        'caption': $localize`:Update User@@AspnetuserViewUformComponent.Update-Item:Update User`,
        'title': $localize`:Select Item@@AspnetuserViewUformComponent.SelItem:Select Item`,
        'Not-all-masters': $localize`:Not all masters have been set@@AspnetuserViewUformComponent.Not-all-masters:Not all masters have been set`,
    ...
    constructor(private frmSrvaspnetuserView: AspnetuserViewService, public dialog: MatDialog, protected appGlblSettings: AppGlblSettingsService ) {
        ...
        this.mainFormGroup.addControl('userName', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('userName', val);
        }});
        locValidators = [ Validators.maxLength(256),Validators.minLength(0) ];
        frmcntrl = new FormControl({ value: null, disabled: false}, locValidators);
        this.mainFormGroup.addControl('email', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('email', val);
        }});
        locValidators = [ ];
        frmcntrl = new FormControl({ value: false, disabled: false}, locValidators);
        this.mainFormGroup.addControl('emailConfirmed', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('emailConfirmed', val);
        }});
        locValidators = [  ];
        frmcntrl = new FormControl({ value: null, disabled: false}, locValidators);
        this.mainFormGroup.addControl('phoneNumber', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('phoneNumber', val);
        }});
        locValidators = [ ];
        frmcntrl = new FormControl({ value: false, disabled: false}, locValidators);
        this.mainFormGroup.addControl('phoneNumberConfirmed', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('phoneNumberConfirmed', val);
        }});
        locValidators = [ ];
        frmcntrl = new FormControl({ value: false, disabled: false}, locValidators);
        this.mainFormGroup.addControl('twoFactorEnabled', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('twoFactorEnabled', val);
        }});
        locValidators = [ ];
        frmcntrl = new FormControl({ value: false, disabled: false}, locValidators);
        this.mainFormGroup.addControl('lockoutEnabled', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('lockoutEnabled', val);
        }});
        locValidators = [ Validators.required,Validators.pattern(/^[-+]?\d+$/),Validators.max(2147483640),Validators.min(-2147483640) ];
        frmcntrl = new FormControl({ value: null, disabled: false}, locValidators);
        this.mainFormGroup.addControl('accessFailedCount', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('accessFailedCount', val);
        }});
...
} // the end of the AspnetuserViewUformComponent class body
