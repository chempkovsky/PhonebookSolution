...
@Component({
  selector: 'app-aspnetrole-view-uform',
  templateUrl: './aspnetrole-view-uform.component.html',
  styleUrls: ['./aspnetrole-view-uform.component.css']
})
export class AspnetroleViewUformComponent implements OnInit, OnDestroy, IEventEmitterPub {
    frases: {[key:string]: string}  = {
        'Not-all-props': $localize`:Not all properties are set@@AspnetroleViewUformComponent.Not-all-props:Not all properties are set`,
        'caption': $localize`:Update Role@@AspnetroleViewUformComponent.Update-Item:Update Role`,
        'title': $localize`:Select Item@@AspnetroleViewUformComponent.SelItem:Select Item`,
        'Not-all-masters': $localize`:Not all masters have been set@@AspnetroleViewUformComponent.Not-all-masters:Not all masters have been set`,
    ...
    constructor(private frmSrvaspnetroleView: AspnetroleViewService, public dialog: MatDialog, protected appGlblSettings: AppGlblSettingsService ) {
        ...
        this.mainFormGroup.addControl('name', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('name', val);
        }});
...
} // the end of the AspnetroleViewUformComponent class body
