...
import { AspnetmodelViewService } from './aspnetmodel-view.service';

export class AspnetmodelViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'aspnetmodelView';
    ...
    protected _Values: {[key: string]: any} = {
       "modelPk": undefined ,  //  System.Int32
       "modelName": undefined ,  //  System.String
       "modelDescription": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: AspnetmodelViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public getHiddenFilterByRow(rw: IaspnetmodelView|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
    ...
    public interface2Values(data: IaspnetmodelView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('modelPk'), data.modelPk)) {
            this.setValue('modelPk', data.modelPk);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('modelName'), data.modelName)) {
            this.setValue('modelName', data.modelName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('modelDescription'), data.modelDescription)) {
            this.setValue('modelDescription', data.modelDescription);
            hasChanged = true;
        }
        if(!hasChanged) return hasChanged;
    ...
    public values2Interface(): IaspnetmodelView | any {
        return {
                modelPk: this.getValue('modelPk'),
                modelName: this.getValue('modelName'),
                modelDescription: this.getValue('modelDescription'),
        };
    }
    ...
    public submitOnDetailChanged(v: IViewModelDatasource): void {
        ...
        if (this._ClientViewName === 'aspnetrolemaskView') {
            if (this._DirectNavigation === 'AspNetModel') {
                let isKeyCrrct = true;
                let dtFrTst: any;
                if(isKeyCrrct) {
                    dtFrTst = this.getValue('modelPk');
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('modelPk')
                    ).subscribe({
                        next: (data: IaspnetmodelView ) => {
        ...
        else if (this._ClientViewName === 'aspnetusermaskView') {
            if (this._DirectNavigation === 'AspNetModel') {
                let isKeyCrrct = true;
                let dtFrTst: any;
                if(isKeyCrrct) {
    ...
    public clearPartially(doNotify: boolean): boolean {
        let hasChanged: boolean = false;
        if(!this.isUnderHiddenFilterFields('modelPk')) {
            if(this.isSetValue('modelPk')) {
                this.clearValue('modelPk');
                hasChanged = true;
            }
        }
        if(!this.isUnderHiddenFilterFields('modelName')) {
            if(this.isSetValue('modelName')) {
                this.clearValue('modelName');
                hasChanged = true;
            }
        }
        if(!this.isUnderHiddenFilterFields('modelDescription')) {
            if(this.isSetValue('modelDescription')) {
                this.clearValue('modelDescription');
                hasChanged = true;
            }
        }
        if(!hasChanged) return hasChanged;
    ...
    protected getFilterByCurrDirMstrs(): IaspnetmodelViewFilter {
        let flt: IaspnetmodelViewFilter | any = {}; // IaspnetmodelViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IaspnetmodelViewFilter] === 'undefined') { 
                                flt[nm as keyof IaspnetmodelViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IaspnetmodelViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IaspnetmodelViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IaspnetmodelViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IaspnetmodelView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IaspnetmodelViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IaspnetmodelViewPage) => {
                     ...
                     catchError(this.handleError<Array<IaspnetmodelView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IaspnetmodelView>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@AspnetmodelViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetmodelViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetmodelView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetmodelViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetmodelView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetmodelViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('modelPk') ).subscribe({
                next: (data: IaspnetmodelView ) => { 
                    this.interface2Values(data, false);
