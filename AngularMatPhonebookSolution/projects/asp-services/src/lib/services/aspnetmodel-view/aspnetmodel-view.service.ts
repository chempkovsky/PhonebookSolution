...
export class AspnetmodelViewService {
    ...
    protected _Values: {[key: string]: {org: string, fk: string, fkchain: string, isinprimkey: boolean, isinunqkey: boolean, required: boolean, dbgenerated: boolean, dttp: string}} = {
      'modelPk': {org: 'ModelPk', fk: '', fkchain: '', isinprimkey: true, isinunqkey: false, required: true, dbgenerated: false, dttp: 'int32'},  // number, System.Int32
      'modelName': {org: 'ModelName', fk: '', fkchain: '', isinprimkey: false, isinunqkey: false, required: true, dbgenerated: false, dttp: 'string'},  // string, System.String
      'modelDescription': {org: 'ModelDescription', fk: '', fkchain: '', isinprimkey: false, isinunqkey: false, required: false, dbgenerated: false, dttp: 'string'},  // string | null, System.String
    };
    ...
    public getHiddenFilterByRow(rw: IaspnetmodelView|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
        ...
        for(let i in this._Values) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                if(typeof rslt['aspnetmodelView'] === 'undefined') rslt['aspnetmodelView'] = {};
                if(typeof rslt['aspnetmodelView'][nvNm] === 'undefined') rslt['aspnetmodelView'][nvNm] = {};
                rslt['aspnetmodelView'][nvNm][i] = rw[i];
            }
        }
        return rslt;
    }
...
*/
    ...
    getmanybyrepprim(filter: IaspnetmodelViewFilter): Observable<IaspnetmodelViewPage> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<IaspnetmodelViewPage>(this.serviceUrl+'/getmanybyrepprim', options);
    }
    ...
    // });
    // 
    deleteone(modelPk: number|any ): Observable<IaspnetmodelView> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof modelPk === 'undefined')) {
            if (!(modelPk === null)) {
    ...
    row2FilterRslt(r: IaspnetmodelView| any): Array<IWebServiceFilterRslt> {
        let rslt: Array<IWebServiceFilterRslt> = [];
