...
import { AspnetuserpermsViewService } from './aspnetuserperms-view.service';

export class AspnetuserpermsViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'aspnetuserpermsView';
    ...
    protected _Values: {[key: string]: any} = {
       ...
       "userPerms": undefined ,  //  System.Int32
    };

    constructor(private frmRootSrv: AspnetuserpermsViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IaspnetuserpermsView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('userPerms'), data.userPerms)) {
            this.setValue('userPerms', data.userPerms);
            hasChanged = true;
        }
        if(!hasChanged) return hasChanged;
    ...
    public values2Interface(): IaspnetuserpermsView | any {
        return {
                modelName: this.getValue('modelName'),
                userPerms: this.getValue('userPerms'),
        };
    }
    ...
    public clearPartially(doNotify: boolean): boolean {
        ...
        if(!this.isUnderHiddenFilterFields('userPerms')) {
            if(this.isSetValue('userPerms')) {
                this.clearValue('userPerms');
                hasChanged = true;
            }
        }
        if(!hasChanged) return hasChanged;
    ...
    protected getFilterByCurrDirMstrs(): IaspnetuserpermsViewFilter {
        let flt: IaspnetuserpermsViewFilter | any = {}; // IaspnetuserpermsViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IaspnetuserpermsViewFilter] === 'undefined') { 
                                flt[nm as keyof IaspnetuserpermsViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IaspnetuserpermsViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IaspnetuserpermsViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IaspnetuserpermsViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IaspnetuserpermsView>> {
            return of([]);
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IaspnetuserpermsView>> {
            return of([]);
    }
    public refresh(): void {
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@AspnetuserpermsViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {

            this.appGlblSettings.showError('http', {message: $localize`:Update is not implemeted for the current ViewModel@@AspnetuserpermsViewDatasourceupdateone.Update-not-implemeted:Update is not implemeted for the current ViewModel`});

    }
    public addone(): void {

        this.appGlblSettings.showError('http', {message: $localize`:Insert is not implemeted for the current ViewModel@@AspnetuserpermsViewDatasourceaddone.Insert-not-implemeted:Insert is not implemeted for the current ViewModel`});

    }
    public deleteone(): void {

        this.appGlblSettings.showError('http', {message: $localize`:Delete is not implemeted for the current ViewModel@@AspnetuserpermsViewDatasourcedeleteone.Delete-not-implemeted:Delete is not implemeted for the current ViewModel`});
