...
export class AspnetroleViewService {
    ...
    protected _Values: {[key: string]: {org: string, fk: string, fkchain: string, isinprimkey: boolean, isinunqkey: boolean, required: boolean, dbgenerated: boolean, dttp: string}} = {
      ...
      'name': {org: 'Name', fk: '', fkchain: '', isinprimkey: false, isinunqkey: true, required: true, dbgenerated: false, dttp: 'string'},  // string, System.String
    };
    ...
    public getHiddenFilterByRow(rw: IaspnetroleView|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
        ...
        for(let i in this._Values) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                if(typeof rslt['aspnetroleView'] === 'undefined') rslt['aspnetroleView'] = {};
                if(typeof rslt['aspnetroleView'][nvNm] === 'undefined') rslt['aspnetroleView'][nvNm] = {};
                rslt['aspnetroleView'][nvNm][i] = rw[i];
            }
        }
        return rslt;
    }
...
*/
    ...
    getmanybyrepprim(filter: IaspnetroleViewFilter): Observable<IaspnetroleViewPage> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<IaspnetroleViewPage>(this.serviceUrl+'/getmanybyrepprim', options);
    }
    ...
    getonebyUnqName(
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<IaspnetroleView>(this.serviceUrl+'/getonebyUnqName', options);
          // .pipe(
          //   catchError(this.handleError('getonebyUnqName', []))
          // );
    }

    getmanybyrepunqUnqName(filter: IaspnetroleViewFilter): Observable<IaspnetroleViewPage> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<IaspnetroleViewPage>(this.serviceUrl+'/getmanybyrepunqUnqName', options);
    }
    ...
    // });
    // 
    deleteone(id: string|any ): Observable<IaspnetroleView> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof id === 'undefined')) {
            if (!(id === null)) {
    ...
    row2FilterRslt(r: IaspnetroleView| any): Array<IWebServiceFilterRslt> {
        let rslt: Array<IWebServiceFilterRslt> = [];
