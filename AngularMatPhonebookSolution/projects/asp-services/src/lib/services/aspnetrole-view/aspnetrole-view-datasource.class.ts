...
import { AspnetroleViewService } from './aspnetrole-view.service';

export class AspnetroleViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'aspnetroleView';
    ...
    protected _Values: {[key: string]: any} = {
       ...
       "name": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: AspnetroleViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public getHiddenFilterByRow(rw: IaspnetroleView|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
    ...
    public interface2Values(data: IaspnetroleView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('name'), data.name)) {
    ...
    public values2Interface(): IaspnetroleView | any {
        return {
                id: this.getValue('id'),
    ...
    public submitOnDetailChanged(v: IViewModelDatasource): void {
        ...
        if (this._ClientViewName === 'aspnetrolemaskView') {
            if (this._DirectNavigation === 'AspNetRole') {
                let isKeyCrrct = true;
                let dtFrTst: any;
                if(isKeyCrrct) {
                    dtFrTst = this.getValue('name');
                ...
                if(isKeyCrrct) {
                    this.frmRootSrv.getonebyUnqName(
                    this.getValue('name')
                    ).subscribe({
                        next: (data: IaspnetroleView ) => {
    ...
    public clearPartially(doNotify: boolean): boolean {
        ...
        if(!this.isUnderHiddenFilterFields('name')) {
    ...
    protected getFilterByCurrDirMstrs(): IaspnetroleViewFilter {
        let flt: IaspnetroleViewFilter | any = {}; // IaspnetroleViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IaspnetroleViewFilter] === 'undefined') { 
                                flt[nm as keyof IaspnetroleViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IaspnetroleViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IaspnetroleViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IaspnetroleViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IaspnetroleView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IaspnetroleViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IaspnetroleViewPage) => {
                     ...
                     catchError(this.handleError<Array<IaspnetroleView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IaspnetroleView>> {
    ...
    public refresh(): void {
            ...
            if (
                    this.isSetValue('name')
            ) {
                this.getonebyUnqName(
                      this.getValue('name')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@AspnetroleViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetroleViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetroleView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetroleViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetroleView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetroleViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
