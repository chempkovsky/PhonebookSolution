...
import { AspnetuserViewService } from './aspnetuser-view.service';

export class AspnetuserViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'aspnetuserView';
    ...
    protected _Values: {[key: string]: any} = {
       "id": undefined ,  //  System.String
       "email": undefined ,  //  System.String
       "emailConfirmed": false ,  //  System.Boolean
       "phoneNumber": undefined ,  //  System.String
       "phoneNumberConfirmed": false ,  //  System.Boolean
       "twoFactorEnabled": false ,  //  System.Boolean
       "lockoutEnd": undefined ,  //  System.DateTimeOffset ?
       "lockoutEnabled": false ,  //  System.Boolean
       "accessFailedCount": undefined ,  //  System.Int32
       "userName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: AspnetuserViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IaspnetuserView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('id'), data.id)) {
            this.setValue('id', data.id);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('email'), data.email)) {
            this.setValue('email', data.email);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('emailConfirmed'), data.emailConfirmed)) {
            this.setValue('emailConfirmed', data.emailConfirmed);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('phoneNumber'), data.phoneNumber)) {
            this.setValue('phoneNumber', data.phoneNumber);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('phoneNumberConfirmed'), data.phoneNumberConfirmed)) {
            this.setValue('phoneNumberConfirmed', data.phoneNumberConfirmed);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('twoFactorEnabled'), data.twoFactorEnabled)) {
            this.setValue('twoFactorEnabled', data.twoFactorEnabled);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('lockoutEnd'), data.lockoutEnd)) {
            this.setValue('lockoutEnd', data.lockoutEnd);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('lockoutEnabled'), data.lockoutEnabled)) {
            this.setValue('lockoutEnabled', data.lockoutEnabled);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('accessFailedCount'), data.accessFailedCount)) {
            this.setValue('accessFailedCount', data.accessFailedCount);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('userName'), data.userName)) {
    ...
    public values2Interface(): IaspnetuserView | any {
        return {
                id: this.getValue('id'),
                email: this.getValue('email'),
                emailConfirmed: this.getValue('emailConfirmed'),
                phoneNumber: this.getValue('phoneNumber'),
                phoneNumberConfirmed: this.getValue('phoneNumberConfirmed'),
                twoFactorEnabled: this.getValue('twoFactorEnabled'),
                lockoutEnd: this.getValue('lockoutEnd'),
                lockoutEnabled: this.getValue('lockoutEnabled'),
                accessFailedCount: this.getValue('accessFailedCount'),
    ...
    protected getFilterByCurrDirMstrs(): IaspnetuserViewFilter {
        let flt: IaspnetuserViewFilter | any = {}; // IaspnetuserViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IaspnetuserViewFilter] === 'undefined') { 
                                flt[nm as keyof IaspnetuserViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IaspnetuserViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IaspnetuserViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IaspnetuserViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IaspnetuserView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IaspnetuserViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IaspnetuserViewPage) => {
                     ...
                     catchError(this.handleError<Array<IaspnetuserView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IaspnetuserView>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@AspnetuserViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetuserViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetuserView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetuserViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetuserView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetuserViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('id') ).subscribe({
                next: (data: IaspnetuserView ) => { 
                    this.interface2Values(data, false);
