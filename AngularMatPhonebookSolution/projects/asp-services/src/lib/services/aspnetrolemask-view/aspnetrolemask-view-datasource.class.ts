...
import { AspnetrolemaskViewService } from './aspnetrolemask-view.service';

export class AspnetrolemaskViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'aspnetrolemaskView';
    ...
    protected _Values: {[key: string]: any} = {
       "roleDescription": undefined ,  //  System.String
    ...
    constructor(private frmRootSrv: AspnetrolemaskViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IaspnetrolemaskView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('roleDescription'), data.roleDescription)) {
    ...
    public values2Interface(): IaspnetrolemaskView | any {
        return {
                roleDescription: this.getValue('roleDescription'),
    ...
    protected getFilterByCurrDirMstrs(): IaspnetrolemaskViewFilter {
        let flt: IaspnetrolemaskViewFilter | any = {}; // IaspnetrolemaskViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IaspnetrolemaskViewFilter] === 'undefined') { 
                                flt[nm as keyof IaspnetrolemaskViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IaspnetrolemaskViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IaspnetrolemaskViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IaspnetrolemaskViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IaspnetrolemaskView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IaspnetrolemaskViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IaspnetrolemaskViewPage) => {
                     ...
                     catchError(this.handleError<Array<IaspnetrolemaskView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IaspnetrolemaskView>> {
    ...
    public refresh(): void {
            if (
                ...
                this.getone(
                      this.getValue('rName')
                    , this.getValue('modelPkRef')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@AspnetrolemaskViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetrolemaskViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetrolemaskView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetrolemaskViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetrolemaskView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetrolemaskViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('rName') ,   this.getValue('modelPkRef') ).subscribe({
                next: (data: IaspnetrolemaskView ) => { 
                    this.interface2Values(data, false);
