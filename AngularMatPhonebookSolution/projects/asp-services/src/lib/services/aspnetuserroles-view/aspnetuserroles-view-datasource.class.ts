...
import { AspnetuserrolesViewService } from './aspnetuserroles-view.service';

export class AspnetuserrolesViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'aspnetuserrolesView';
    ...
    protected _Values: {[key: string]: any} = {
       ...
       "roleId": undefined ,  //  System.String
       "uLockoutEnd": undefined ,  //  System.DateTimeOffset ?
       "uUserName": undefined ,  //  System.String
       "rName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: AspnetuserrolesViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IaspnetuserrolesView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('roleId'), data.roleId)) {
            this.setValue('roleId', data.roleId);
            aftrMstrChngd = (this._CurrentlyDirectMasterNavs.indexOf('AspNetRole') >= 0) || aftrMstrChngd;
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('uLockoutEnd'), data.uLockoutEnd)) {
            this.setValue('uLockoutEnd', data.uLockoutEnd);
            hasChanged = true;
        }
        ...
        if(!this.isEqual(this.getValue('rName'), data.rName)) {
    ...
    public values2Interface(): IaspnetuserrolesView | any {
        return {
                userId: this.getValue('userId'),
                roleId: this.getValue('roleId'),
                uLockoutEnd: this.getValue('uLockoutEnd'),
                uUserName: this.getValue('uUserName'),
                rName: this.getValue('rName'),
        };
    }
    ...
    public clearPartially(doNotify: boolean): boolean {
        ...
        if(!this.isUnderHiddenFilterFields('roleId')) {
            if(this._CurrentlyDirectMasterNavs.indexOf('AspNetRole') < 0) {
                if(this.isSetValue('roleId')) {
                    this.clearValue('roleId');
                    hasChanged = true;
                }
            }
        }
        if(!this.isUnderHiddenFilterFields('uLockoutEnd')) {
            if(this._CurrentlyDirectMasterNavs.indexOf(this.frmRootSrv.getFkValue('uLockoutEnd')) < 0) {
                if(this.isSetValue('uLockoutEnd')) {
                    this.clearValue('uLockoutEnd');
                    hasChanged = true;
                }
            }
        }
    ...
    protected getFilterByCurrDirMstrs(): IaspnetuserrolesViewFilter {
        let flt: IaspnetuserrolesViewFilter | any = {}; // IaspnetuserrolesViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IaspnetuserrolesViewFilter] === 'undefined') { 
                                flt[nm as keyof IaspnetuserrolesViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IaspnetuserrolesViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IaspnetuserrolesViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IaspnetuserrolesViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IaspnetuserrolesView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IaspnetuserrolesViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IaspnetuserrolesViewPage) => {
                     ...
                     catchError(this.handleError<Array<IaspnetuserrolesView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IaspnetuserrolesView>> {
    ...
    public getone(
            ...
            this.frmRootSrv.getone(
                ...
                next: (data: IaspnetuserrolesView ) => { 
    ...
    public refresh(): void {
            if (
                    this.isSetValue('userId')
                && this.isSetValue('roleId')
            ) {
                this.getone(
                      this.getValue('userId')
                    , this.getValue('roleId')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@AspnetuserrolesViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetuserrolesViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetuserrolesView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetuserrolesViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IaspnetuserrolesView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@AspnetuserrolesViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('userId') ,   this.getValue('roleId') ).subscribe({
                next: (data: IaspnetuserrolesView ) => { 
                    this.interface2Values(data, false);
