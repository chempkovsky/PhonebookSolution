...
@Component({
  selector: 'app-phbk-phone-type-view-aform',
  templateUrl: './phbk-phone-type-view-aform.component.html',
  styleUrls: ['./phbk-phone-type-view-aform.component.css']
})
export class PhbkPhoneTypeViewAformComponent implements OnInit, OnDestroy, IEventEmitterPub {

    frases: {[key:string]: string}  = {
        'Not-all-props': $localize`:Not all properties are set@@PhbkPhoneTypeViewAformComponent.Not-all-props:Not all properties are set`,
        'caption': $localize`:Add Phone Type@@PhbkPhoneTypeViewAformComponent.Review-Item:Add Phone Type`,
        'title': $localize`:Select Item@@PhbkPhoneTypeViewAformComponent.SelItem:Select Item`,
        'Not-all-masters': $localize`:Not all masters have been set@@PhbkPhoneTypeViewAformComponent.Not-all-masters:Not all masters have been set`,
    ...
    constructor(private frmSrvPhbkPhoneTypeView: PhbkPhoneTypeViewService, public dialog: MatDialog, protected appGlblSettings: AppGlblSettingsService ) {
        ...
        this.mainFormGroup.addControl('phoneTypeId', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('phoneTypeId', val);
        }});
        locValidators = [ Validators.required,Validators.maxLength(20),Validators.minLength(3) ];
        frmcntrl = new FormControl({ value: null, disabled: false}, locValidators);
        this.mainFormGroup.addControl('phoneTypeName', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('phoneTypeName', val);
        }});
        locValidators = [ Validators.maxLength(250) ];
        frmcntrl = new FormControl({ value: null, disabled: false}, locValidators);
        this.mainFormGroup.addControl('phoneTypeDesc', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('phoneTypeDesc', val);
        }});
...
} // the end of the PhbkPhoneTypeViewAformComponent class body
