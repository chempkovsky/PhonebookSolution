...
@Component({
  selector: 'app-phbk-enterprise-view-aform',
  templateUrl: './phbk-enterprise-view-aform.component.html',
  styleUrls: ['./phbk-enterprise-view-aform.component.css']
})
export class PhbkEnterpriseViewAformComponent implements OnInit, OnDestroy, IEventEmitterPub {

    frases: {[key:string]: string}  = {
        'Not-all-props': $localize`:Not all properties are set@@PhbkEnterpriseViewAformComponent.Not-all-props:Not all properties are set`,
        'caption': $localize`:Add Enterprise@@PhbkEnterpriseViewAformComponent.Review-Item:Add Enterprise`,
        'title': $localize`:Select Item@@PhbkEnterpriseViewAformComponent.SelItem:Select Item`,
        'Not-all-masters': $localize`:Not all masters have been set@@PhbkEnterpriseViewAformComponent.Not-all-masters:Not all masters have been set`,
    ...
    constructor(private frmSrvPhbkEnterpriseView: PhbkEnterpriseViewService, public dialog: MatDialog, protected appGlblSettings: AppGlblSettingsService ) {
        ...
        this.mainFormGroup.addControl('entrprsId', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('entrprsId', val);
        }});
        locValidators = [ Validators.required,Validators.maxLength(20),Validators.minLength(3) ];
        frmcntrl = new FormControl({ value: null, disabled: false}, locValidators);
        this.mainFormGroup.addControl('entrprsName', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('entrprsName', val);
        }});
        locValidators = [ Validators.maxLength(250) ];
        frmcntrl = new FormControl({ value: null, disabled: false}, locValidators);
        this.mainFormGroup.addControl('entrprsDesc', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('entrprsDesc', val);
        }});
...
} // the end of the PhbkEnterpriseViewAformComponent class body
