...
export class PhbkDivisionViewAformComponent implements OnInit, OnDestroy, IEventEmitterPub {
    ...
    frases: {[key:string]: string}  = {
        ...
        'caption': $localize`:Add Division@@PhbkDivisionViewAformComponent.Review-Item:Add Division`,
    ...
    public doSubmit(): void {
        if (this.mainFormGroup.invalid || (!this.rootDataSource.refreshIsDefined())) {
            ...
            this.eEntrprsNameCmbCntrl.markAllAsTouched();
    ...
    public eEntrprsNameCmbCntrl: FormControl | any = null;
    public eEntrprsNameCmbCntrlVals : Array<IPhbkEnterpriseView> | any = null;
    ...
    constructor(private frmSrvPhbkEnterpriseView: PhbkEnterpriseViewService, private frmSrvPhbkDivisionView: PhbkDivisionViewService, public dialog: MatDialog, protected appGlblSettings: AppGlblSettingsService ) {
        ...
        this.mainFormGroup.addControl('divisionId', frmcntrl);
        frmcntrl.valueChanges.subscribe({next: (val: any) => { 
            this.rootDataSource.setValue('divisionId', val);
        }});
        ...
        this.eEntrprsNameCmbCntrl = new FormControl({ value: null, disabled: true }, [ 
                (fc)=>{  //if ((typeof fc.value === 'string') || (typeof fc.value === 'undefined') || (fc.value === null)) { 
                        if (!this.eEntrprsNameDtSrc.calcIsDefined()) {
                            this.mainFormGroup.patchValue({'eEntrprsName': null });
                            return  this.mainFormGroup.controls['eEntrprsName'].errors; 
                        }  
                        return null; }]);
        this.eEntrprsNameCmbCntrl.valueChanges
            .subscribe({
                next: (val: any) => { 
                    if(val) {
...
//    public rootDataSourceOnIsDefinedChanged(v: IViewModelDatasource): void { }
    public rootDataSourceAfterPropsChanged(v: IViewModelDatasource): void {
        ...
        if(this.rootDataSource.isUnderHiddenFilterFields('eEntrprsName')) {
            this.eEntrprsNameCmbCntrl.disable({emitEvent: false});
        } else {
            this.eEntrprsNameCmbCntrl.enable({emitEvent: false});
        }
    }
    
    public eEntrprsNameAfterMasterChanged(v: IViewModelDatasource): void {
        this.eEntrprsNameDtSrc.getCllctionByCurrDirMstrs().subscribe({
            next: (data: Array<IPhbkEnterpriseView> ) => {
                this.eEntrprsNameCmbCntrlVals = data;
                let lfc: any = this.eEntrprsNameDtSrc.values2Interface();
                let ind: number = -1;
                if(this.eEntrprsNameDtSrc.calcIsDefined()) {
                    ind = data.findIndex((e: any) => { return  (e.entrprsId === lfc.entrprsId) ; });
                }
                if (ind > -1) {
                    this.eEntrprsNameCmbCntrl.patchValue(data[ind], {emitEvent: false});
                    this.mainFormGroup.patchValue({'eEntrprsName': this.eEntrprsNameCmbCntrlVals[ind].entrprsName}, {emitEvent: false});
                } else {
                    this.eEntrprsNameCmbCntrl.patchValue(undefined, {emitEvent: false});
                    this.mainFormGroup.patchValue({'eEntrprsName': undefined}, {emitEvent: false});
                }
            },
            error: (error: any) => {
                this.eEntrprsNameCmbCntrlVals = [];
                this.eEntrprsNameCmbCntrl.patchValue(undefined, {emitEvent: false});
                this.mainFormGroup.patchValue({'eEntrprsName': undefined}, {emitEvent: false});
...
//    public eEntrprsNameOnIsDefinedChanged(v: IViewModelDatasource): void { }
    public eEntrprsNameAfterPropsChanged(v: IViewModelDatasource): void {
        if(this.eEntrprsNameCmbCntrlVals) {
            let lfc: any = this.eEntrprsNameDtSrc.values2Interface();
            let ind: number = this.eEntrprsNameCmbCntrlVals.findIndex((e: any) => { return  (e.entrprsId === lfc.entrprsId) ; });
            if (ind > -1) {
                this.eEntrprsNameCmbCntrl.patchValue(this.eEntrprsNameCmbCntrlVals[ind], {emitEvent: false});
                this.mainFormGroup.patchValue({'eEntrprsName': this.eEntrprsNameCmbCntrlVals[ind].entrprsName}, {emitEvent: false});
            } else {
                this.eEntrprsNameCmbCntrl.patchValue(undefined, {emitEvent: false});
                this.mainFormGroup.patchValue({'eEntrprsName': undefined}, {emitEvent: false});
...
} // the end of the PhbkDivisionViewAformComponent class body
