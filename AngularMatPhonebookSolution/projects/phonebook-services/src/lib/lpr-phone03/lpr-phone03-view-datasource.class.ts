...
import { LprPhone03ViewService } from './lpr-phone03-view.service';

export class LprPhone03ViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LprPhone03View';
    ...
    constructor(private frmRootSrv: LprPhone03ViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILprPhone03View | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): ILprPhone03View | any {
        return {
    ...
    protected getFilterByCurrDirMstrs(): ILprPhone03ViewFilter {
        let flt: ILprPhone03ViewFilter | any = {}; // ILprPhone03ViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILprPhone03ViewFilter] === 'undefined') { 
                                flt[nm as keyof ILprPhone03ViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILprPhone03ViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILprPhone03ViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILprPhone03ViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILprPhone03View>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILprPhone03ViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILprPhone03ViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILprPhone03View>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILprPhone03View>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LprPhone03ViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone03ViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprPhone03View | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone03ViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprPhone03View | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone03ViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('phoneTypeIdRef') ,   this.getValue('lpdPhoneIdRef') ,   this.getValue('phoneId') ).subscribe({
                next: (data: ILprPhone03View ) => { 
                    this.interface2Values(data, false);
