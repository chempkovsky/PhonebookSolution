...
import { LpdEmpFirstNameViewService } from './lpd-emp-first-name-view.service';

export class LpdEmpFirstNameViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LpdEmpFirstNameView';
    ...
    protected _Values: {[key: string]: any} = {
       "empFirstNameId": undefined ,  //  System.Int32
       "empFirstName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: LpdEmpFirstNameViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILpdEmpFirstNameView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('empFirstNameId'), data.empFirstNameId)) {
            this.setValue('empFirstNameId', data.empFirstNameId);
            hasChanged = true;
        }
    ...
    public values2Interface(): ILpdEmpFirstNameView | any {
        return {
                empFirstNameId: this.getValue('empFirstNameId'),
    ...
    public submitOnDetailChanged(v: IViewModelDatasource): void {
        ...
        if (this._ClientViewName === 'LprEmployee01View') {
            if (this._DirectNavigation === 'EmpFirstNameDict') {
                let isKeyCrrct = true;
                let dtFrTst: any;
                if(isKeyCrrct) {
                    dtFrTst = this.getValue('empFirstNameId');
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('empFirstNameId')
                    ).subscribe({
                        next: (data: ILpdEmpFirstNameView ) => {
    ...
    public clearPartially(doNotify: boolean): boolean {
        let hasChanged: boolean = false;
        if(!this.isUnderHiddenFilterFields('empFirstNameId')) {
            if(this.isSetValue('empFirstNameId')) {
                this.clearValue('empFirstNameId');
                hasChanged = true;
            }
        }
    ...
    protected getFilterByCurrDirMstrs(): ILpdEmpFirstNameViewFilter {
        let flt: ILpdEmpFirstNameViewFilter | any = {}; // ILpdEmpFirstNameViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILpdEmpFirstNameViewFilter] === 'undefined') { 
                                flt[nm as keyof ILpdEmpFirstNameViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILpdEmpFirstNameViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILpdEmpFirstNameViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILpdEmpFirstNameViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILpdEmpFirstNameView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILpdEmpFirstNameViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILpdEmpFirstNameViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILpdEmpFirstNameView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILpdEmpFirstNameView>> {
    ...
    public getonebyLpdEmpFirstNameUK(
                  empFirstName: string 
        ): void {
            this.frmRootSrv.getonebyLpdEmpFirstNameUK(
                  empFirstName
            ).subscribe({
                next: (data: ILpdEmpFirstNameView ) => { 
                    if(!this.interface2Values(data, true)) {
    ...
    public refresh(): void {
            if (
                    this.isSetValue('empFirstNameId')
            ) {
                this.getone(
                      this.getValue('empFirstNameId')
                );
            ...
            if (
                    this.isSetValue('empFirstName')
            ) {
                this.getonebyLpdEmpFirstNameUK(
                      this.getValue('empFirstName')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LpdEmpFirstNameViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpFirstNameViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdEmpFirstNameView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpFirstNameViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdEmpFirstNameView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpFirstNameViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('empFirstNameId') ).subscribe({
                next: (data: ILpdEmpFirstNameView ) => { 
                    this.interface2Values(data, false);
