...
import { PhbkPhoneViewService } from './phbk-phone-view.service';

export class PhbkPhoneViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'PhbkPhoneView';
    ...
    protected _Values: {[key: string]: any} = {
       ...
       "phoneTypeIdRef": undefined ,  //  System.Int32
       "employeeIdRef": undefined ,  //  System.Int32
       "pPhoneTypeName": undefined ,  //  System.String
       "eEmpFirstName": undefined ,  //  System.String
       "eEmpLastName": undefined ,  //  System.String
       "eEmpSecondName": undefined ,  //  System.String
       "eDDivisionName": undefined ,  //  System.String
       "eDEEntrprsName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: PhbkPhoneViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IPhbkPhoneView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('phoneTypeIdRef'), data.phoneTypeIdRef)) {
        ...
        if(!this.isEqual(this.getValue('employeeIdRef'), data.employeeIdRef)) {
        ...
        if(!this.isEqual(this.getValue('pPhoneTypeName'), data.pPhoneTypeName)) {
            this.setValue('pPhoneTypeName', data.pPhoneTypeName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('eEmpFirstName'), data.eEmpFirstName)) {
            this.setValue('eEmpFirstName', data.eEmpFirstName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('eEmpLastName'), data.eEmpLastName)) {
            this.setValue('eEmpLastName', data.eEmpLastName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('eEmpSecondName'), data.eEmpSecondName)) {
            this.setValue('eEmpSecondName', data.eEmpSecondName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('eDDivisionName'), data.eDDivisionName)) {
            this.setValue('eDDivisionName', data.eDDivisionName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('eDEEntrprsName'), data.eDEEntrprsName)) {
    ...
    public values2Interface(): IPhbkPhoneView | any {
        return {
                phoneId: this.getValue('phoneId'),
                phone: this.getValue('phone'),
                phoneTypeIdRef: this.getValue('phoneTypeIdRef'),
                employeeIdRef: this.getValue('employeeIdRef'),
                pPhoneTypeName: this.getValue('pPhoneTypeName'),
                eEmpFirstName: this.getValue('eEmpFirstName'),
                eEmpLastName: this.getValue('eEmpLastName'),
                eEmpSecondName: this.getValue('eEmpSecondName'),
                eDDivisionName: this.getValue('eDDivisionName'),
                eDEEntrprsName: this.getValue('eDEEntrprsName'),
        };
    }
    ...
    protected getFilterByCurrDirMstrs(): IPhbkPhoneViewFilter {
        let flt: IPhbkPhoneViewFilter | any = {}; // IPhbkPhoneViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IPhbkPhoneViewFilter] === 'undefined') { 
                                flt[nm as keyof IPhbkPhoneViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IPhbkPhoneViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IPhbkPhoneViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IPhbkPhoneViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IPhbkPhoneView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IPhbkPhoneViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IPhbkPhoneViewPage) => {
                     ...
                     catchError(this.handleError<Array<IPhbkPhoneView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IPhbkPhoneView>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@PhbkPhoneViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkPhoneViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkPhoneView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkPhoneViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkPhoneView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkPhoneViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('phoneId') ).subscribe({
                next: (data: IPhbkPhoneView ) => { 
                    this.interface2Values(data, false);
