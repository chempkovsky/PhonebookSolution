...
import { LpdEmpLastNameViewService } from './lpd-emp-last-name-view.service';

export class LpdEmpLastNameViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LpdEmpLastNameView';
    ...
    protected _Values: {[key: string]: any} = {
       "empLastNameId": undefined ,  //  System.Int32
       "empLastName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: LpdEmpLastNameViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILpdEmpLastNameView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('empLastNameId'), data.empLastNameId)) {
    ...
    public values2Interface(): ILpdEmpLastNameView | any {
        return {
                empLastNameId: this.getValue('empLastNameId'),
        ...
        if (this._ClientViewName === 'LprEmployee01View') {
            if (this._DirectNavigation === 'EmpLastNameDict') {
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('empLastNameId')
                    ).subscribe({
                        next: (data: ILpdEmpLastNameView ) => {
    ...
    protected getFilterByCurrDirMstrs(): ILpdEmpLastNameViewFilter {
        let flt: ILpdEmpLastNameViewFilter | any = {}; // ILpdEmpLastNameViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILpdEmpLastNameViewFilter] === 'undefined') { 
                                flt[nm as keyof ILpdEmpLastNameViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILpdEmpLastNameViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILpdEmpLastNameViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILpdEmpLastNameViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILpdEmpLastNameView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILpdEmpLastNameViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILpdEmpLastNameViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILpdEmpLastNameView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILpdEmpLastNameView>> {
    ...
    public getonebyLpdEmpLastNameUK(
                  empLastName: string 
        ): void {
            this.frmRootSrv.getonebyLpdEmpLastNameUK(
                  empLastName
            ).subscribe({
                next: (data: ILpdEmpLastNameView ) => { 
                    if(!this.interface2Values(data, true)) {
    ...
    public refresh(): void {
            ...
            if (
                ...
                this.getonebyLpdEmpLastNameUK(
                      this.getValue('empLastName')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LpdEmpLastNameViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpLastNameViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdEmpLastNameView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpLastNameViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdEmpLastNameView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpLastNameViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('empLastNameId') ).subscribe({
                next: (data: ILpdEmpLastNameView ) => { 
                    this.interface2Values(data, false);
