...
export class PhbkDivisionViewService {
    serviceUrl: string;  
    ...
    protected _Values: {[key: string]: {org: string, fk: string, fkchain: string, isinprimkey: boolean, isinunqkey: boolean, required: boolean, dbgenerated: boolean, dttp: string}} = {
      ...
      'divisionName': {org: 'DivisionName', fk: '', fkchain: '', isinprimkey: false, isinunqkey: false, required: true, dbgenerated: false, dttp: 'string'},  // string, System.String
      'divisionDesc': {org: 'DivisionDesc', fk: '', fkchain: '', isinprimkey: false, isinunqkey: false, required: false, dbgenerated: false, dttp: 'string'},  // string | null, System.String
      'entrprsIdRef': {org: 'EntrprsIdRef', fk: '', fkchain: '', isinprimkey: false, isinunqkey: false, required: true, dbgenerated: false, dttp: 'int32'},  // number, System.Int32
      'eEntrprsName': {org: 'EntrprsName', fk: 'Enterprise', fkchain: 'Enterprise', isinprimkey: false, isinunqkey: false, required: true, dbgenerated: false, dttp: 'string'},  // string, System.String
    };
    ...
    public getHiddenFilterByRow(rw: IPhbkDivisionView|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
        ...
        for(let i in this._Values) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                if(typeof rslt['PhbkDivisionView'] === 'undefined') rslt['PhbkDivisionView'] = {};
                if(typeof rslt['PhbkDivisionView'][nvNm] === 'undefined') rslt['PhbkDivisionView'][nvNm] = {};
                rslt['PhbkDivisionView'][nvNm][i] = rw[i];
            }
        }
        return rslt;
    }
...
*/
    ...
    protected _m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = {
                'PhbkEnterpriseView' : {
                    'Enterprise' : {
                        'entrprsId' : {isMstrReq: true, propNm:'entrprsIdRef'},
                    },
                }
    }; 
    ...
    // });
    // 
    getall(): Observable<IPhbkDivisionView[]> {

        return this.http.get<IPhbkDivisionView[]>(this.serviceUrl+'/getall');
        //    .pipe(
    ...
    // 
    // HowTo: flt is of type IPhbkDivisionViewFilter 
    //
    // this.serviceRefInYourCode.getwithfilter(flt).subscibe(value =>{
    //    // handling value of type IPhbkDivisionViewPage
    // },
    // error => {
    //    // handling error 
    // });
    // 
    getwithfilter(filter: IPhbkDivisionViewFilter): Observable<IPhbkDivisionViewPage> {
        ...
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
                ...
                if (!(typeof filter.entrprsIdRef === 'undefined')) {
                    if ( Array.isArray(filter.entrprsIdRef )) {
                        filter.entrprsIdRef.forEach(function (value) {
                            if(!(typeof value === 'undefined')) {
                                if(!(value === null)) {
                                    params = params.append('entrprsIdRef', value.toString());
                                    hasFilter = true;
                                } 
                            }
                        });
                ...
                if (!(typeof  filter.entrprsIdRefOprtr === 'undefined')) {
                    if (Array.isArray(filter.entrprsIdRefOprtr )) {
                        filter.entrprsIdRefOprtr.forEach(function (value) {
                            if(!(typeof value === 'undefined')) {
                                if(value === null) {
                                    params = params.append('entrprsIdRefOprtr', 'eq');
                                } 
                                else {
                                    params = params.append('entrprsIdRefOprtr', value.toString());
                                }
                            }
                        });
                    } // if (Array.isArray(filter.entrprsIdRefOprtr))
        ...
        return this.http.get<IPhbkDivisionViewPage>(this.serviceUrl+'/getwithfilter', options);
          //    .pipe(
    ...
    getone(
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<IPhbkDivisionView>(this.serviceUrl+'/getone', options);
          // .pipe(
          //   catchError(this.handleError('getone', []))
          // );
    }

    getmanybyrepprim(filter: IPhbkDivisionViewFilter): Observable<IPhbkDivisionViewPage> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<IPhbkDivisionViewPage>(this.serviceUrl+'/getmanybyrepprim', options);
    }


    // 
    // HowTo: item is of type IPhbkDivisionView 
    //
    ...
    // });
    // 
    updateone(item: IPhbkDivisionView): Observable<IPhbkDivisionView> {

        return this.http.put<IPhbkDivisionView>(this.serviceUrl+'/updateone', item); //, httpOptions);
        //  .pipe(
    ...
    // });
    // 
    addone(item: IPhbkDivisionView): Observable<IPhbkDivisionView> {
        return this.http.post<IPhbkDivisionView>(this.serviceUrl+'/addone', item); //, httpOptions);
        // .pipe(
    ...
    // });
    // 
    deleteone(divisionId: number|any ): Observable<IPhbkDivisionView> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof divisionId === 'undefined')) {
            if (!(divisionId === null)) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.delete<IPhbkDivisionView>(this.serviceUrl+'/deleteone', options); //, httpOptions);
        // .pipe(
        //      catchError(this.handleError('deleteone', item))
        // );   
    }

    src2dest(src: IPhbkDivisionView, dest: IPhbkDivisionView) {
        ...
        if(typeof src.divisionDesc === 'undefined') {
            dest['divisionDesc'] = null;
        } else {
            dest['divisionDesc'] = src.divisionDesc;
        }
        if(typeof src.entrprsIdRef === 'undefined') {
            dest['entrprsIdRef'] = null;
        } else {
            dest['entrprsIdRef'] = src.entrprsIdRef;
        }
        if(typeof src.eEntrprsName === 'undefined') {
            dest['eEntrprsName'] = null;
        } else {
    ...
    row2FilterRslt(r: IPhbkDivisionView| any): Array<IWebServiceFilterRslt> {
        let rslt: Array<IWebServiceFilterRslt> = [];
