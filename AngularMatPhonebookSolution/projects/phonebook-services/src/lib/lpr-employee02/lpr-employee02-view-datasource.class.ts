...
import { LprEmployee02ViewService } from './lpr-employee02-view.service';

export class LprEmployee02ViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LprEmployee02View';
    ...
    protected _Values: {[key: string]: any} = {
       ...
       "empLastNameIdRef": undefined ,  //  System.Int32
       "empFirstNameIdRef": undefined ,  //  System.Int32
       "empSecondNameIdRef": undefined ,  //  System.Int32
       "divisionIdRef": undefined ,  //  System.Int32
    };

    constructor(private frmRootSrv: LprEmployee02ViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILprEmployee02View | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('empLastNameIdRef'), data.empLastNameIdRef)) {
        ...
        if(!this.isEqual(this.getValue('empFirstNameIdRef'), data.empFirstNameIdRef)) {
        ...
        if(!this.isEqual(this.getValue('empSecondNameIdRef'), data.empSecondNameIdRef)) {
    ...
    public values2Interface(): ILprEmployee02View | any {
        return {
                employeeId: this.getValue('employeeId'),
                empLastNameIdRef: this.getValue('empLastNameIdRef'),
                empFirstNameIdRef: this.getValue('empFirstNameIdRef'),
                empSecondNameIdRef: this.getValue('empSecondNameIdRef'),
    ...
    protected getFilterByCurrDirMstrs(): ILprEmployee02ViewFilter {
        let flt: ILprEmployee02ViewFilter | any = {}; // ILprEmployee02ViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILprEmployee02ViewFilter] === 'undefined') { 
                                flt[nm as keyof ILprEmployee02ViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILprEmployee02ViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILprEmployee02ViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILprEmployee02ViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILprEmployee02View>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILprEmployee02ViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILprEmployee02ViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILprEmployee02View>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILprEmployee02View>> {
    ...
    public getone(
                  divisionIdRef: number 
                , empLastNameIdRef: number 
                , empFirstNameIdRef: number 
                , empSecondNameIdRef: number 
                , employeeId: number 
        ): void {
    ...
    public refresh(): void {
            if (
                ...
                this.getone(
                      this.getValue('divisionIdRef')
                    , this.getValue('empLastNameIdRef')
                    , this.getValue('empFirstNameIdRef')
                    , this.getValue('empSecondNameIdRef')
                    , this.getValue('employeeId')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LprEmployee02ViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprEmployee02ViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprEmployee02View | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprEmployee02ViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprEmployee02View | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprEmployee02ViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('divisionIdRef') ,   this.getValue('empLastNameIdRef') ,   this.getValue('empFirstNameIdRef') ,   this.getValue('empSecondNameIdRef') ,   this.getValue('employeeId') ).subscribe({
                next: (data: ILprEmployee02View ) => { 
                    this.interface2Values(data, false);
