...
import { LprPhone04ViewService } from './lpr-phone04-view.service';

export class LprPhone04ViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LprPhone04View';
    ...
    constructor(private frmRootSrv: LprPhone04ViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILprPhone04View | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): ILprPhone04View | any {
        return {
    ...
    protected getFilterByCurrDirMstrs(): ILprPhone04ViewFilter {
        let flt: ILprPhone04ViewFilter | any = {}; // ILprPhone04ViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILprPhone04ViewFilter] === 'undefined') { 
                                flt[nm as keyof ILprPhone04ViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILprPhone04ViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILprPhone04ViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILprPhone04ViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILprPhone04View>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILprPhone04ViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILprPhone04ViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILprPhone04View>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILprPhone04View>> {
    ...
    public refresh(): void {
            if (
                ...
                this.getone(
                      this.getValue('employeeIdRef')
                    , this.getValue('phoneTypeIdRef')
                    , this.getValue('lpdPhoneIdRef')
                    , this.getValue('phoneId')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LprPhone04ViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone04ViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprPhone04View | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone04ViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprPhone04View | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone04ViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('employeeIdRef') ,   this.getValue('phoneTypeIdRef') ,   this.getValue('lpdPhoneIdRef') ,   this.getValue('phoneId') ).subscribe({
                next: (data: ILprPhone04View ) => { 
                    this.interface2Values(data, false);
