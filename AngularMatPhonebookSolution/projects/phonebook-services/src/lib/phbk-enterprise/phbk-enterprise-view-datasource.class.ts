...
import { PhbkEnterpriseViewService } from './phbk-enterprise-view.service';

export class PhbkEnterpriseViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'PhbkEnterpriseView';
    ...
    protected _Values: {[key: string]: any} = {
       "entrprsId": undefined ,  //  System.Int32
       "entrprsName": undefined ,  //  System.String
       "entrprsDesc": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: PhbkEnterpriseViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IPhbkEnterpriseView | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): IPhbkEnterpriseView | any {
        return {
        ...
        if (this._ClientViewName === 'PhbkDivisionView') {
            if (this._DirectNavigation === 'Enterprise') {
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('entrprsId')
                    ).subscribe({
                        next: (data: IPhbkEnterpriseView ) => {
    ...
    protected getFilterByCurrDirMstrs(): IPhbkEnterpriseViewFilter {
        let flt: IPhbkEnterpriseViewFilter | any = {}; // IPhbkEnterpriseViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IPhbkEnterpriseViewFilter] === 'undefined') { 
                                flt[nm as keyof IPhbkEnterpriseViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IPhbkEnterpriseViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IPhbkEnterpriseViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IPhbkEnterpriseViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IPhbkEnterpriseView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IPhbkEnterpriseViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IPhbkEnterpriseViewPage) => {
                     ...
                     catchError(this.handleError<Array<IPhbkEnterpriseView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IPhbkEnterpriseView>> {
    ...
    public getonebyEntrprsNameUK(
                  entrprsName: string 
        ): void {
            this.frmRootSrv.getonebyEntrprsNameUK(
                  entrprsName
            ).subscribe({
                next: (data: IPhbkEnterpriseView ) => { 
                    if(!this.interface2Values(data, true)) {
    ...
    public refresh(): void {
            ...
            if (
                ...
                this.getonebyEntrprsNameUK(
                      this.getValue('entrprsName')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@PhbkEnterpriseViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkEnterpriseViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkEnterpriseView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkEnterpriseViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkEnterpriseView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkEnterpriseViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
