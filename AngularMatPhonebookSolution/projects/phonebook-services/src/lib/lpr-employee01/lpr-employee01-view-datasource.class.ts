...
import { LprEmployee01ViewService } from './lpr-employee01-view.service';

export class LprEmployee01ViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LprEmployee01View';
    ...
    constructor(private frmRootSrv: LprEmployee01ViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public getHiddenFilterByRow(rw: ILprEmployee01View|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
    ...
    public interface2Values(data: ILprEmployee01View | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): ILprEmployee01View | any {
        return {
    ...
    protected getFilterByCurrDirMstrs(): ILprEmployee01ViewFilter {
        let flt: ILprEmployee01ViewFilter | any = {}; // ILprEmployee01ViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILprEmployee01ViewFilter] === 'undefined') { 
                                flt[nm as keyof ILprEmployee01ViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILprEmployee01ViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILprEmployee01ViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILprEmployee01ViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILprEmployee01View>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILprEmployee01ViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILprEmployee01ViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILprEmployee01View>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILprEmployee01View>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LprEmployee01ViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprEmployee01ViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprEmployee01View | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprEmployee01ViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprEmployee01View | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprEmployee01ViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('empLastNameIdRef') ,   this.getValue('empFirstNameIdRef') ,   this.getValue('empSecondNameIdRef') ,   this.getValue('employeeId') ).subscribe({
                next: (data: ILprEmployee01View ) => { 
                    this.interface2Values(data, false);
