...
import { LpdEmpSecondNameViewService } from './lpd-emp-second-name-view.service';

export class LpdEmpSecondNameViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LpdEmpSecondNameView';
    ...
    protected _Values: {[key: string]: any} = {
       "empSecondNameId": undefined ,  //  System.Int32
       "empSecondName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: LpdEmpSecondNameViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILpdEmpSecondNameView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('empSecondNameId'), data.empSecondNameId)) {
            this.setValue('empSecondNameId', data.empSecondNameId);
            hasChanged = true;
        }
    ...
    public values2Interface(): ILpdEmpSecondNameView | any {
        return {
                empSecondNameId: this.getValue('empSecondNameId'),
    ...
    public submitOnDetailChanged(v: IViewModelDatasource): void {
        ...
        if (this._ClientViewName === 'LprEmployee01View') {
            if (this._DirectNavigation === 'EmpSecondNameDict') {
                let isKeyCrrct = true;
                let dtFrTst: any;
                if(isKeyCrrct) {
                    dtFrTst = this.getValue('empSecondNameId');
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('empSecondNameId')
                    ).subscribe({
                        next: (data: ILpdEmpSecondNameView ) => {
    ...
    public clearPartially(doNotify: boolean): boolean {
        let hasChanged: boolean = false;
        if(!this.isUnderHiddenFilterFields('empSecondNameId')) {
            if(this.isSetValue('empSecondNameId')) {
                this.clearValue('empSecondNameId');
                hasChanged = true;
            }
        }
    ...
    protected getFilterByCurrDirMstrs(): ILpdEmpSecondNameViewFilter {
        let flt: ILpdEmpSecondNameViewFilter | any = {}; // ILpdEmpSecondNameViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILpdEmpSecondNameViewFilter] === 'undefined') { 
                                flt[nm as keyof ILpdEmpSecondNameViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILpdEmpSecondNameViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILpdEmpSecondNameViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILpdEmpSecondNameViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILpdEmpSecondNameView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILpdEmpSecondNameViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILpdEmpSecondNameViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILpdEmpSecondNameView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILpdEmpSecondNameView>> {
    ...
    public getonebyLpdEmpSecondNameUK(
                  empSecondName: string | null 
        ): void {
            this.frmRootSrv.getonebyLpdEmpSecondNameUK(
                  empSecondName
            ).subscribe({
                next: (data: ILpdEmpSecondNameView ) => { 
                    if(!this.interface2Values(data, true)) {
    ...
    public refresh(): void {
            if (
                    this.isSetValue('empSecondNameId')
            ) {
                this.getone(
                      this.getValue('empSecondNameId')
                );
            ...
            if (
                    this.isSetValue('empSecondName')
            ) {
                this.getonebyLpdEmpSecondNameUK(
                      this.getValue('empSecondName')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LpdEmpSecondNameViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpSecondNameViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdEmpSecondNameView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpSecondNameViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdEmpSecondNameView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdEmpSecondNameViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('empSecondNameId') ).subscribe({
                next: (data: ILpdEmpSecondNameView ) => { 
                    this.interface2Values(data, false);
