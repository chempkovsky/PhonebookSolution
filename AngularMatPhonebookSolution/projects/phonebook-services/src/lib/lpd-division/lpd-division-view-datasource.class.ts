...
import { LpdDivisionViewService } from './lpd-division-view.service';

export class LpdDivisionViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LpdDivisionView';
    ...
    protected _Values: {[key: string]: any} = {
       "divisionNameId": undefined ,  //  System.Int32
       "divisionName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: LpdDivisionViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILpdDivisionView | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): ILpdDivisionView | any {
        return {
        ...
        if (this._ClientViewName === 'LprDivision01View') {
            if (this._DirectNavigation === 'DivisionNameDict') {
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('divisionNameId')
                    ).subscribe({
                        next: (data: ILpdDivisionView ) => {
    ...
    protected getFilterByCurrDirMstrs(): ILpdDivisionViewFilter {
        let flt: ILpdDivisionViewFilter | any = {}; // ILpdDivisionViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILpdDivisionViewFilter] === 'undefined') { 
                                flt[nm as keyof ILpdDivisionViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILpdDivisionViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILpdDivisionViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILpdDivisionViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILpdDivisionView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILpdDivisionViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILpdDivisionViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILpdDivisionView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILpdDivisionView>> {
    ...
    public getonebyLpdDivisionNameUk(
                  divisionName: string 
        ): void {
            this.frmRootSrv.getonebyLpdDivisionNameUk(
                  divisionName
            ).subscribe({
                next: (data: ILpdDivisionView ) => { 
                    if(!this.interface2Values(data, true)) {
    ...
    public refresh(): void {
            if (
                    this.isSetValue('divisionNameId')
            ) {
                this.getone(
                      this.getValue('divisionNameId')
                );
            ...
            if (
                    this.isSetValue('divisionName')
            ) {
                this.getonebyLpdDivisionNameUk(
                      this.getValue('divisionName')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LpdDivisionViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdDivisionViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdDivisionView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdDivisionViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILpdDivisionView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LpdDivisionViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
