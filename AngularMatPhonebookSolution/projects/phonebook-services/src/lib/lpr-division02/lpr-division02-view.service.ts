...
export class LprDivision02ViewService {
    serviceUrl: string;  
    ...
    protected _Values: {[key: string]: {org: string, fk: string, fkchain: string, isinprimkey: boolean, isinunqkey: boolean, required: boolean, dbgenerated: boolean, dttp: string}} = {
      ...
      'entrprsIdRef': {org: 'EntrprsIdRef', fk: '', fkchain: '', isinprimkey: true, isinunqkey: false, required: true, dbgenerated: false, dttp: 'int32'},  // number, System.Int32
    ...
    public getHiddenFilterByRow(rw: ILprDivision02View|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
        ...
        for(let i in this._Values) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                if(typeof rslt['LprDivision02View'] === 'undefined') rslt['LprDivision02View'] = {};
                if(typeof rslt['LprDivision02View'][nvNm] === 'undefined') rslt['LprDivision02View'][nvNm] = {};
                rslt['LprDivision02View'][nvNm][i] = rw[i];
            }
        }
        return rslt;
    }
...
*/
    ...
    // });
    // 
    getall(): Observable<ILprDivision02View[]> {

        return this.http.get<ILprDivision02View[]>(this.serviceUrl+'/getall');
        //    .pipe(
    ...
    // 
    // HowTo: flt is of type ILprDivision02ViewFilter 
    //
    // this.serviceRefInYourCode.getwithfilter(flt).subscibe(value =>{
    //    // handling value of type ILprDivision02ViewPage
    // },
    // error => {
    //    // handling error 
    // });
    // 
    getwithfilter(filter: ILprDivision02ViewFilter): Observable<ILprDivision02ViewPage> {
        ...
        return this.http.get<ILprDivision02ViewPage>(this.serviceUrl+'/getwithfilter', options);
          //    .pipe(
    ...
    // 
    getone(
                  entrprsIdRef: number|any 
                , divisionNameIdRef: number|any 
                , divisionId: number|any 
        ): Observable<ILprDivision02View> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        ...
        if (!(typeof entrprsIdRef === 'undefined')) {
            if (!(entrprsIdRef === null)) {
                params = params.append('entrprsIdRef', entrprsIdRef.toString());
                hasFilter = true;
            }
        }
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<ILprDivision02View>(this.serviceUrl+'/getone', options);
          // .pipe(
          //   catchError(this.handleError('getone', []))
          // );
    }

    getmanybyrepprim(filter: ILprDivision02ViewFilter): Observable<ILprDivision02ViewPage> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<ILprDivision02ViewPage>(this.serviceUrl+'/getmanybyrepprim', options);
    }


    // 
    // HowTo: item is of type ILprDivision02View 
    //
    ...
    // });
    // 
    updateone(item: ILprDivision02View): Observable<ILprDivision02View> {

        return this.http.put<ILprDivision02View>(this.serviceUrl+'/updateone', item); //, httpOptions);
        //  .pipe(
    ...
    // });
    // 
    addone(item: ILprDivision02View): Observable<ILprDivision02View> {
        return this.http.post<ILprDivision02View>(this.serviceUrl+'/addone', item); //, httpOptions);
        // .pipe(
    ...
    // });
    // 
    deleteone(entrprsIdRef: number|any , divisionNameIdRef: number|any , divisionId: number|any ): Observable<ILprDivision02View> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof divisionId === 'undefined')) {
            if (!(divisionId === null)) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.delete<ILprDivision02View>(this.serviceUrl+'/deleteone', options); //, httpOptions);
        // .pipe(
        //      catchError(this.handleError('deleteone', item))
        // );   
    }

    src2dest(src: ILprDivision02View, dest: ILprDivision02View) {
    ...
    row2FilterRslt(r: ILprDivision02View| any): Array<IWebServiceFilterRslt> {
        let rslt: Array<IWebServiceFilterRslt> = [];
