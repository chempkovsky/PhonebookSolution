...
import { LprDivision02ViewService } from './lpr-division02-view.service';

export class LprDivision02ViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LprDivision02View';
    ...
    protected _Values: {[key: string]: any} = {
       "divisionId": undefined ,  //  System.Int32
       "entrprsIdRef": undefined ,  //  System.Int32
       "divisionNameIdRef": undefined ,  //  System.Int32
    };

    constructor(private frmRootSrv: LprDivision02ViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILprDivision02View | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): ILprDivision02View | any {
        return {
    ...
    protected getFilterByCurrDirMstrs(): ILprDivision02ViewFilter {
        let flt: ILprDivision02ViewFilter | any = {}; // ILprDivision02ViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILprDivision02ViewFilter] === 'undefined') { 
                                flt[nm as keyof ILprDivision02ViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILprDivision02ViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILprDivision02ViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILprDivision02ViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILprDivision02View>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILprDivision02ViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILprDivision02ViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILprDivision02View>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILprDivision02View>> {
    ...
    public refresh(): void {
            if (
                ...
                this.getone(
                      this.getValue('entrprsIdRef')
                    , this.getValue('divisionNameIdRef')
                    , this.getValue('divisionId')
                );
                return;
            }
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LprDivision02ViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprDivision02ViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprDivision02View | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprDivision02ViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprDivision02View | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprDivision02ViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('entrprsIdRef') ,   this.getValue('divisionNameIdRef') ,   this.getValue('divisionId') ).subscribe({
                next: (data: ILprDivision02View ) => { 
                    this.interface2Values(data, false);
