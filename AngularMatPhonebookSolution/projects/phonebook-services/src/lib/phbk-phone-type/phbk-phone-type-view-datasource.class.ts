...
import { PhbkPhoneTypeViewService } from './phbk-phone-type-view.service';

export class PhbkPhoneTypeViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'PhbkPhoneTypeView';
    ...
    protected _Values: {[key: string]: any} = {
       "phoneTypeId": undefined ,  //  System.Int32
       "phoneTypeName": undefined ,  //  System.String
       "phoneTypeDesc": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: PhbkPhoneTypeViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IPhbkPhoneTypeView | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): IPhbkPhoneTypeView | any {
        return {
        ...
        if (this._ClientViewName === 'PhbkPhoneView') {
            if (this._DirectNavigation === 'PhoneType') {
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('phoneTypeId')
                    ).subscribe({
                        next: (data: IPhbkPhoneTypeView ) => {
    ...
    protected getFilterByCurrDirMstrs(): IPhbkPhoneTypeViewFilter {
        let flt: IPhbkPhoneTypeViewFilter | any = {}; // IPhbkPhoneTypeViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IPhbkPhoneTypeViewFilter] === 'undefined') { 
                                flt[nm as keyof IPhbkPhoneTypeViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IPhbkPhoneTypeViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IPhbkPhoneTypeViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IPhbkPhoneTypeViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IPhbkPhoneTypeView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IPhbkPhoneTypeViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IPhbkPhoneTypeViewPage) => {
                     ...
                     catchError(this.handleError<Array<IPhbkPhoneTypeView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IPhbkPhoneTypeView>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@PhbkPhoneTypeViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkPhoneTypeViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkPhoneTypeView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkPhoneTypeViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkPhoneTypeView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkPhoneTypeViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
