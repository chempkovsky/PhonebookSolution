...
import { PhbkEmployeeViewService } from './phbk-employee-view.service';

export class PhbkEmployeeViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'PhbkEmployeeView';
    ...
    protected _Values: {[key: string]: any} = {
       "employeeId": undefined ,  //  System.Int32
       "empFirstName": undefined ,  //  System.String
       "empLastName": undefined ,  //  System.String
       "empSecondName": undefined ,  //  System.String
       "divisionIdRef": undefined ,  //  System.Int32
       "dDivisionName": undefined ,  //  System.String
       "dEEntrprsName": undefined ,  //  System.String
    };

    constructor(private frmRootSrv: PhbkEmployeeViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: IPhbkEmployeeView | null, doNotify: boolean = true): boolean {
        ...
        if(!this.isEqual(this.getValue('employeeId'), data.employeeId)) {
            this.setValue('employeeId', data.employeeId);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('empFirstName'), data.empFirstName)) {
            this.setValue('empFirstName', data.empFirstName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('empLastName'), data.empLastName)) {
            this.setValue('empLastName', data.empLastName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('empSecondName'), data.empSecondName)) {
            this.setValue('empSecondName', data.empSecondName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('divisionIdRef'), data.divisionIdRef)) {
        ...
        if(!this.isEqual(this.getValue('dDivisionName'), data.dDivisionName)) {
            this.setValue('dDivisionName', data.dDivisionName);
            hasChanged = true;
        }
        if(!this.isEqual(this.getValue('dEEntrprsName'), data.dEEntrprsName)) {
            this.setValue('dEEntrprsName', data.dEEntrprsName);
            hasChanged = true;
        }
        if(!hasChanged) return hasChanged;
    ...
    public values2Interface(): IPhbkEmployeeView | any {
        return {
                employeeId: this.getValue('employeeId'),
                empFirstName: this.getValue('empFirstName'),
                empLastName: this.getValue('empLastName'),
                empSecondName: this.getValue('empSecondName'),
                divisionIdRef: this.getValue('divisionIdRef'),
                dDivisionName: this.getValue('dDivisionName'),
                dEEntrprsName: this.getValue('dEEntrprsName'),
        };
    }
    ...
    public submitOnDetailChanged(v: IViewModelDatasource): void {
        ...
        if (this._ClientViewName === 'LprEmployee01View') {
            if (this._DirectNavigation === 'Employee') {
                let isKeyCrrct = true;
                let dtFrTst: any;
                if(isKeyCrrct) {
                    dtFrTst = this.getValue('employeeId');
                ...
                if(isKeyCrrct) {
                    ...
                    this.getValue('employeeId')
                    ).subscribe({
                        next: (data: IPhbkEmployeeView ) => {
        ...
        else if (this._ClientViewName === 'PhbkPhoneView') {
            if (this._DirectNavigation === 'Employee') {
                let isKeyCrrct = true;
                let dtFrTst: any;
                if(isKeyCrrct) {
    ...
    public clearPartially(doNotify: boolean): boolean {
        ...
        if(!this.isUnderHiddenFilterFields('empFirstName')) {
            if(this.isSetValue('empFirstName')) {
                this.clearValue('empFirstName');
                hasChanged = true;
            }
        }
        ...
        if(!this.isUnderHiddenFilterFields('empSecondName')) {
            if(this.isSetValue('empSecondName')) {
                this.clearValue('empSecondName');
                hasChanged = true;
            }
        }
        ...
        if(!this.isUnderHiddenFilterFields('dDivisionName')) {
            if(this._CurrentlyDirectMasterNavs.indexOf(this.frmRootSrv.getFkValue('dDivisionName')) < 0) {
                if(this.isSetValue('dDivisionName')) {
                    this.clearValue('dDivisionName');
                    hasChanged = true;
                }
            }
        }
        if(!this.isUnderHiddenFilterFields('dEEntrprsName')) {
            if(this._CurrentlyDirectMasterNavs.indexOf(this.frmRootSrv.getFkValue('dEEntrprsName')) < 0) {
                if(this.isSetValue('dEEntrprsName')) {
    ...
    protected getFilterByCurrDirMstrs(): IPhbkEmployeeViewFilter {
        let flt: IPhbkEmployeeViewFilter | any = {}; // IPhbkEmployeeViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof IPhbkEmployeeViewFilter] === 'undefined') { 
                                flt[nm as keyof IPhbkEmployeeViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof IPhbkEmployeeViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof IPhbkEmployeeViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof IPhbkEmployeeViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<IPhbkEmployeeView>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: IPhbkEmployeeViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: IPhbkEmployeeViewPage) => {
                     ...
                     catchError(this.handleError<Array<IPhbkEmployeeView>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<IPhbkEmployeeView>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@PhbkEmployeeViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkEmployeeViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkEmployeeView | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkEmployeeViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: IPhbkEmployeeView | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@PhbkEmployeeViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('employeeId') ).subscribe({
                next: (data: IPhbkEmployeeView ) => { 
                    this.interface2Values(data, false);
