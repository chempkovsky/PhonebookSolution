...
export class LprPhone02ViewService {
    serviceUrl: string;  
    ...
    public getHiddenFilterByRow(rw: ILprPhone02View|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
        ...
        for(let i in this._Values) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                if(typeof rslt['LprPhone02View'] === 'undefined') rslt['LprPhone02View'] = {};
                if(typeof rslt['LprPhone02View'][nvNm] === 'undefined') rslt['LprPhone02View'][nvNm] = {};
                rslt['LprPhone02View'][nvNm][i] = rw[i];
            }
        }
        return rslt;
    }
...
*/
    ...
    // });
    // 
    getall(): Observable<ILprPhone02View[]> {

        return this.http.get<ILprPhone02View[]>(this.serviceUrl+'/getall');
        //    .pipe(
    ...
    // 
    // HowTo: flt is of type ILprPhone02ViewFilter 
    //
    // this.serviceRefInYourCode.getwithfilter(flt).subscibe(value =>{
    //    // handling value of type ILprPhone02ViewPage
    // },
    // error => {
    //    // handling error 
    // });
    // 
    getwithfilter(filter: ILprPhone02ViewFilter): Observable<ILprPhone02ViewPage> {
        ...
        return this.http.get<ILprPhone02ViewPage>(this.serviceUrl+'/getwithfilter', options);
          //    .pipe(
    ...
    getone(
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<ILprPhone02View>(this.serviceUrl+'/getone', options);
          // .pipe(
          //   catchError(this.handleError('getone', []))
          // );
    }

    getmanybyrepprim(filter: ILprPhone02ViewFilter): Observable<ILprPhone02ViewPage> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<ILprPhone02ViewPage>(this.serviceUrl+'/getmanybyrepprim', options);
    }


    // 
    // HowTo: item is of type ILprPhone02View 
    //
    ...
    // });
    // 
    updateone(item: ILprPhone02View): Observable<ILprPhone02View> {

        return this.http.put<ILprPhone02View>(this.serviceUrl+'/updateone', item); //, httpOptions);
        //  .pipe(
    ...
    // });
    // 
    addone(item: ILprPhone02View): Observable<ILprPhone02View> {
        ...
        return this.http.post<ILprPhone02View>(this.serviceUrl+'/addone', item); //, httpOptions);
        // .pipe(
    ...
    // });
    // 
    deleteone(employeeIdRef: number|any , lpdPhoneIdRef: number|any , phoneId: number|any ): Observable<ILprPhone02View> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof phoneId === 'undefined')) {
            if (!(phoneId === null)) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.delete<ILprPhone02View>(this.serviceUrl+'/deleteone', options); //, httpOptions);
        // .pipe(
        //      catchError(this.handleError('deleteone', item))
        // );   
    }

    src2dest(src: ILprPhone02View, dest: ILprPhone02View) {
    ...
    row2FilterRslt(r: ILprPhone02View| any): Array<IWebServiceFilterRslt> {
        let rslt: Array<IWebServiceFilterRslt> = [];
