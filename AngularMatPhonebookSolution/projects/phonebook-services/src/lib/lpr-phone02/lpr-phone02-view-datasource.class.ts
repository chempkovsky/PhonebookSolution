...
import { LprPhone02ViewService } from './lpr-phone02-view.service';

export class LprPhone02ViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LprPhone02View';
    ...
    constructor(private frmRootSrv: LprPhone02ViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public getHiddenFilterByRow(rw: ILprPhone02View|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
    ...
    public interface2Values(data: ILprPhone02View | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): ILprPhone02View | any {
        return {
    ...
    protected getFilterByCurrDirMstrs(): ILprPhone02ViewFilter {
        let flt: ILprPhone02ViewFilter | any = {}; // ILprPhone02ViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILprPhone02ViewFilter] === 'undefined') { 
                                flt[nm as keyof ILprPhone02ViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILprPhone02ViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILprPhone02ViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILprPhone02ViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILprPhone02View>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILprPhone02ViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILprPhone02ViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILprPhone02View>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILprPhone02View>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LprPhone02ViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone02ViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprPhone02View | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone02ViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprPhone02View | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprPhone02ViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        this.frmRootSrv.deleteone(  this.getValue('employeeIdRef') ,   this.getValue('lpdPhoneIdRef') ,   this.getValue('phoneId') ).subscribe({
                next: (data: ILprPhone02View ) => { 
                    this.interface2Values(data, false);
