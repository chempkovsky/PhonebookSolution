...
import { LprDivision01ViewService } from './lpr-division01-view.service';

export class LprDivision01ViewDatasource implements IViewModelDatasource {
    protected readonly _CurrentViewName: string  = 'LprDivision01View';
    ...
    constructor(private frmRootSrv: LprDivision01ViewService, protected appGlblSettings: AppGlblSettingsService,
    ...
    public interface2Values(data: ILprDivision01View | null, doNotify: boolean = true): boolean {
    ...
    public values2Interface(): ILprDivision01View | any {
        return {
    ...
    protected getFilterByCurrDirMstrs(): ILprDivision01ViewFilter {
        let flt: ILprDivision01ViewFilter | any = {}; // ILprDivision01ViewFilter
        let m2cKeyfm: {[key: string]: {[key: string]: {[key: string]: {isMstrReq: boolean ,propNm:string}}}} = this.frmRootSrv.getm2cKeyfm();
        for(let i in m2cKeyfm) {
            for(let j in m2cKeyfm[i]) {
                if(this._CurrentlyDirectMasterNavs.indexOf(j) > -1) {
                    for(let k in m2cKeyfm[i][j]) {
                        if (this.isSetValue(m2cKeyfm[i][j][k].propNm)) {
                            ...
                            if (typeof flt[nm as keyof ILprDivision01ViewFilter] === 'undefined') { 
                                flt[nm as keyof ILprDivision01ViewFilter] = [this.getValue(nm)]; 
                                flt[op as keyof ILprDivision01ViewFilter] = ['eq'];
                            } else { 
                                flt[nm as keyof ILprDivision01ViewFilter].push(this.getValue(nm)); 
                                flt[op as keyof ILprDivision01ViewFilter].push('eq');
                            }
                        }
                    }
                }
    ...
    public getCllctionByCurrDirMstrs(): Observable<Array<ILprDivision01View>> {
        if (!this.isSetFilterByCurrDirMstrs()) {
            return of([]);
        }
        let flt: ILprDivision01ViewFilter | any =  this.getFilterByCurrDirMstrs();
        return this.frmRootSrv.getwithfilter(flt)
                .pipe(
                    switchMap((rslt: ILprDivision01ViewPage) => {
                     ...
                     catchError(this.handleError<Array<ILprDivision01View>>([]))
                );
    }
    public getCllctionByFldFilter(fldName: string|any, fldVal: any): Observable<Array<ILprDivision01View>> {
    ...
    public refresh(): void {
        ...
        this.appGlblSettings.showError('http', {message: $localize`:Not all Unique or Primary key properties are defined to call Refresh-method@@LprDivision01ViewDatasourcerefresh.Not-all-Unique-Primary:Not all Unique or Primary key properties are defined to call Refresh-method`});
    }
    public updateone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprDivision01ViewDatasourceupdateone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprDivision01View | any = this.values2Interface();
    ...
    public addone(): void {
        if(!this.getIsDefined()) {
            ...
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprDivision01ViewDatasourceaddone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
        let itm: ILprDivision01View | any = this.values2Interface();
    ...
    public deleteone(): void {
        if(!this.getIsDefined()) {
            this.appGlblSettings.showError('http', {message: $localize`:Not all propertes are correctly defined@@LprDivision01ViewDatasourcedeleteone.Not-all-propertes:Not all propertes are correctly defined`});
            return;
        }
