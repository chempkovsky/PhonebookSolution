...
export class LprPhone01ViewService {
    serviceUrl: string;  
    ...
    public getHiddenFilterByRow(rw: ILprPhone01View|any, nvNm: string|any): {[key: string]: {[key: string]: {[key: string]: any}}} {
        ...
        for(let i in this._Values) {
            if(this.isInPrimkeyValue(i) || this.IsInUnkKeyValue(i)) {
                if(typeof rslt['LprPhone01View'] === 'undefined') rslt['LprPhone01View'] = {};
                if(typeof rslt['LprPhone01View'][nvNm] === 'undefined') rslt['LprPhone01View'][nvNm] = {};
                rslt['LprPhone01View'][nvNm][i] = rw[i];
            }
        }
        return rslt;
    }
...
*/
    ...
    // });
    // 
    getall(): Observable<ILprPhone01View[]> {

        return this.http.get<ILprPhone01View[]>(this.serviceUrl+'/getall');
        //    .pipe(
    ...
    // 
    // HowTo: flt is of type ILprPhone01ViewFilter 
    //
    // this.serviceRefInYourCode.getwithfilter(flt).subscibe(value =>{
    //    // handling value of type ILprPhone01ViewPage
    // },
    // error => {
    //    // handling error 
    // });
    // 
    getwithfilter(filter: ILprPhone01ViewFilter): Observable<ILprPhone01ViewPage> {
        ...
        return this.http.get<ILprPhone01ViewPage>(this.serviceUrl+'/getwithfilter', options);
          //    .pipe(
    ...
    getone(
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<ILprPhone01View>(this.serviceUrl+'/getone', options);
          // .pipe(
          //   catchError(this.handleError('getone', []))
          // );
    }

    getmanybyrepprim(filter: ILprPhone01ViewFilter): Observable<ILprPhone01ViewPage> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof filter === 'undefined')) {
            if (!(filter === null )) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.get<ILprPhone01ViewPage>(this.serviceUrl+'/getmanybyrepprim', options);
    }


    // 
    // HowTo: item is of type ILprPhone01View 
    //
    ...
    // });
    // 
    updateone(item: ILprPhone01View): Observable<ILprPhone01View> {

        return this.http.put<ILprPhone01View>(this.serviceUrl+'/updateone', item); //, httpOptions);
        //  .pipe(
    ...
    // });
    // 
    addone(item: ILprPhone01View): Observable<ILprPhone01View> {
        ...
        return this.http.post<ILprPhone01View>(this.serviceUrl+'/addone', item); //, httpOptions);
        // .pipe(
    ...
    // });
    // 
    deleteone(lpdPhoneIdRef: number|any , phoneId: number|any ): Observable<ILprPhone01View> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof phoneId === 'undefined')) {
            if (!(phoneId === null)) {
        ...
        const options = hasFilter ? { params } : {};
        return this.http.delete<ILprPhone01View>(this.serviceUrl+'/deleteone', options); //, httpOptions);
        // .pipe(
        //      catchError(this.handleError('deleteone', item))
        // );   
    }

    src2dest(src: ILprPhone01View, dest: ILprPhone01View) {
    ...
    row2FilterRslt(r: ILprPhone01View| any): Array<IWebServiceFilterRslt> {
        let rslt: Array<IWebServiceFilterRslt> = [];
