...
import { PhbkPhoneViewVformComponent } from 'phonebook-updforms';

@Component({
  selector: 'app-phbk-phone-view-r-v',
  templateUrl: './phbk-phone-view-r-v.component.html',
  styleUrls: ['./phbk-phone-view-r-v.component.css']
})

export class PhbkPhoneViewRVComponent implements OnInit, AfterViewInit, IEventEmitterPub {
    frases: {[key:string]: string}  = {
        'title': $localize`:View Phone@@PhbkPhoneViewRVComponent.ViewPhbkPhoneView:View Phone`,
        'NavBackToMaster': $localize`:Navigate back to master@@PhbkPhoneViewRVComponent.NavBackToMaster:Navigate back to master`,
        'Hide-details': $localize`:Hide details@@PhbkPhoneViewRVComponent.Hide-details:Hide details`,
...
*/
    ...
    @ViewChild(PhbkPhoneViewVformComponent) childForm!: PhbkPhoneViewVformComponent;
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'oltnmPhbkPhoneView': null}}], {relativeTo: this.route});
        } else {
            this.router.navigate([{outlets: { 'oltnmPhbkPhoneView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'oltnmPhbkPhoneView2' + this.selectedDetail!.vw );
                    ...
                    this.router.navigate([{ outlets: { 'oltnmPhbkPhoneView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
