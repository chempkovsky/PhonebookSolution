...
@Component({
  selector: 'app-phbk-employee-view-rlist',
  templateUrl: './phbk-employee-view-rlist.component.html',
  styleUrls: ['./phbk-employee-view-rlist.component.css']
})
export class PhbkEmployeeViewRlistComponent implements OnInit, OnDestroy, IEventEmitterPub, IItemHeightData {
    frases: {[key:string]: string}  = {
        'Hide-details': $localize`:Hide details@@PhbkEmployeeViewRlistComponent.Hide-details:Hide details`,
        'nvm2mntallwd':  $localize`:Navigating to detail form is not allowed in One-to-Many mode@@PhbkEmployeeViewRlistComponent.nvm2mntallwd:Navigating to detail form is not allowed in One-to-Many mode`,
        'caption': $localize`:Employees@@PhbkEmployeeViewRlistComponent.caption:Employees`,
        'update': $localize`:Modify Item@@PhbkEmployeeViewRlistComponent.update:Modify Item`,
        'delete': $localize`:Delete Item@@PhbkEmployeeViewRlistComponent.delete:Delete Item`,
        'view': $localize`:View Item@@PhbkEmployeeViewRlistComponent.view:View Item`,
        'add': $localize`:Add Item@@PhbkEmployeeViewRlistComponent.add:Add Item`,
        'deletesel': $localize`:Delete Selected@@PhbkEmployeeViewRlistComponent.deletesel:Delete Selected`,
        'permission': $localize`:Permission@@PhbkEmployeeViewRlistComponent.permission:Permission`,
        'accessdenied': $localize`:Access denied@@PhbkEmployeeViewRlistComponent.accessdenied:Access denied`,
        'PhbkPhoneViewEmployee': $localize`:Phones(Employee)@@PhbkEmployeeViewRlistComponent.PhbkPhoneViewEmployee:Phones(Employee)`,
    ...
    ngOnInit() {
        ...
        let luid = 'PhbkEmployeeViewRlistComponent';
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'loltnmPhbkEmployeeView': null}}], {relativeTo: this.route}).then(()=>{
                this.router.navigate([{outlets: { 'loltnmPhbkEmployeeView': null}}], {relativeTo: this.route});
            });
        } else {
            this.router.navigate([{outlets: { 'loltnmPhbkEmployeeView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'loltnmPhbkEmployeeView2' + this.selectedDetail!.vw );
                    qp.push(JSON.stringify(this.frmRootSrv.getHiddenFilterByRow(this.childForm.currentRow, this.selectedDetail!.nv)));
                    this.router.navigate([{ outlets: { 'loltnmPhbkEmployeeView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
...
*/
    ...
    applyFilter(frm: PhbkEmployeeViewSformComponent | any) {
        ...
        let flt: IPhbkEmployeeView | any = JSON.parse(this.route.snapshot.queryParams['item']);
        if(typeof flt === 'undefined') return;
        if(flt === null) return;

        if ((act === 'Add') ||
            (act === 'Upd')) {
            if(typeof flt['employeeId'] === 'undefined') {
                return;
            }
            let pkpEmployeeId: any = flt['employeeId'];
            this.frmRootSrv.getone(
                  pkpEmployeeId
 
            ).subscribe({
                   next: (data: IPhbkEmployeeView ) => { // success path
