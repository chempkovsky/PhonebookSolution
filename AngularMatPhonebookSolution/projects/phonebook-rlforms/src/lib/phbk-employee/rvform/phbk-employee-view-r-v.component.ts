...
import { PhbkEmployeeViewVformComponent } from 'phonebook-updforms';

@Component({
  selector: 'app-phbk-employee-view-r-v',
  templateUrl: './phbk-employee-view-r-v.component.html',
  styleUrls: ['./phbk-employee-view-r-v.component.css']
})

export class PhbkEmployeeViewRVComponent implements OnInit, AfterViewInit, IEventEmitterPub {
    frases: {[key:string]: string}  = {
        'title': $localize`:View Employee@@PhbkEmployeeViewRVComponent.ViewPhbkEmployeeView:View Employee`,
        'NavBackToMaster': $localize`:Navigate back to master@@PhbkEmployeeViewRVComponent.NavBackToMaster:Navigate back to master`,
        'Hide-details': $localize`:Hide details@@PhbkEmployeeViewRVComponent.Hide-details:Hide details`,
        'PhbkPhoneViewEmployee': $localize`:Phones(Employee)@@PhbkEmployeeViewRVComponent.PhonesEmployee:Phones(Employee)`,
...
*/
    ...
    @ViewChild(PhbkEmployeeViewVformComponent) childForm!: PhbkEmployeeViewVformComponent;
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'oltnmPhbkEmployeeView': null}}], {relativeTo: this.route});
        } else {
            this.router.navigate([{outlets: { 'oltnmPhbkEmployeeView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'oltnmPhbkEmployeeView2' + this.selectedDetail!.vw );
                    ...
                    this.router.navigate([{ outlets: { 'oltnmPhbkEmployeeView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
