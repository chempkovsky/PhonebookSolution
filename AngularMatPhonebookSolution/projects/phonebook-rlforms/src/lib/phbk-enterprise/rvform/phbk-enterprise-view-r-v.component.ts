...
import { PhbkEnterpriseViewVformComponent } from 'phonebook-updforms';

@Component({
  selector: 'app-phbk-enterprise-view-r-v',
  templateUrl: './phbk-enterprise-view-r-v.component.html',
  styleUrls: ['./phbk-enterprise-view-r-v.component.css']
})

export class PhbkEnterpriseViewRVComponent implements OnInit, AfterViewInit, IEventEmitterPub {
    frases: {[key:string]: string}  = {
        'title': $localize`:View Enterprise@@PhbkEnterpriseViewRVComponent.ViewPhbkEnterpriseView:View Enterprise`,
        'NavBackToMaster': $localize`:Navigate back to master@@PhbkEnterpriseViewRVComponent.NavBackToMaster:Navigate back to master`,
        'Hide-details': $localize`:Hide details@@PhbkEnterpriseViewRVComponent.Hide-details:Hide details`,
        'PhbkDivisionViewEnterprise': $localize`:Divisions(Enterprise)@@PhbkEnterpriseViewRVComponent.DivisionsEnterprise:Divisions(Enterprise)`,
    ...
    eformControlModel: IPhbkEnterpriseView | null = null;
    constructor(protected route: ActivatedRoute, protected router: Router, protected appGlblSettings: AppGlblSettingsService, protected frmRootSrv: PhbkEnterpriseViewService) { }
    isdtl: boolean = false;
    ngOnInit() {
...
*/
    ...
    @ViewChild(PhbkEnterpriseViewVformComponent) childForm!: PhbkEnterpriseViewVformComponent;
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'oltnmPhbkEnterpriseView': null}}], {relativeTo: this.route});
        } else {
            this.router.navigate([{outlets: { 'oltnmPhbkEnterpriseView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'oltnmPhbkEnterpriseView2' + this.selectedDetail!.vw );
                    ...
                    this.router.navigate([{ outlets: { 'oltnmPhbkEnterpriseView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
