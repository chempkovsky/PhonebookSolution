...
@Component({
  selector: 'app-phbk-enterprise-view-rlist',
  templateUrl: './phbk-enterprise-view-rlist.component.html',
  styleUrls: ['./phbk-enterprise-view-rlist.component.css']
})
export class PhbkEnterpriseViewRlistComponent implements OnInit, OnDestroy, IEventEmitterPub, IItemHeightData {
    frases: {[key:string]: string}  = {
        'Hide-details': $localize`:Hide details@@PhbkEnterpriseViewRlistComponent.Hide-details:Hide details`,
        'nvm2mntallwd':  $localize`:Navigating to detail form is not allowed in One-to-Many mode@@PhbkEnterpriseViewRlistComponent.nvm2mntallwd:Navigating to detail form is not allowed in One-to-Many mode`,
        'caption': $localize`:Enterprises@@PhbkEnterpriseViewRlistComponent.caption:Enterprises`,
        'update': $localize`:Modify Item@@PhbkEnterpriseViewRlistComponent.update:Modify Item`,
        'delete': $localize`:Delete Item@@PhbkEnterpriseViewRlistComponent.delete:Delete Item`,
        'view': $localize`:View Item@@PhbkEnterpriseViewRlistComponent.view:View Item`,
        'add': $localize`:Add Item@@PhbkEnterpriseViewRlistComponent.add:Add Item`,
        'deletesel': $localize`:Delete Selected@@PhbkEnterpriseViewRlistComponent.deletesel:Delete Selected`,
        'permission': $localize`:Permission@@PhbkEnterpriseViewRlistComponent.permission:Permission`,
        'accessdenied': $localize`:Access denied@@PhbkEnterpriseViewRlistComponent.accessdenied:Access denied`,
        'PhbkDivisionViewEnterprise': $localize`:Divisions(Enterprise)@@PhbkEnterpriseViewRlistComponent.PhbkDivisionViewEnterprise:Divisions(Enterprise)`,
    ...
    ngOnInit() {
        ...
        let luid = 'PhbkEnterpriseViewRlistComponent';
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'loltnmPhbkEnterpriseView': null}}], {relativeTo: this.route}).then(()=>{
                this.router.navigate([{outlets: { 'loltnmPhbkEnterpriseView': null}}], {relativeTo: this.route});
            });
        } else {
            this.router.navigate([{outlets: { 'loltnmPhbkEnterpriseView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'loltnmPhbkEnterpriseView2' + this.selectedDetail!.vw );
                    qp.push(JSON.stringify(this.frmRootSrv.getHiddenFilterByRow(this.childForm.currentRow, this.selectedDetail!.nv)));
                    this.router.navigate([{ outlets: { 'loltnmPhbkEnterpriseView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
    ...
    rowCommand(v: IEventEmitterData) {
        ...
        if(v.id === 'view') {
            qp.push( 'View' + 'PhbkEnterpriseView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
            ...
            qp.push( 'Upd' + 'PhbkEnterpriseView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
                qp.push(this.route.snapshot.params[this.hf]);
            } else {
                qp.push(JSON.stringify({}));
            ...
            qp.push( 'Del' + 'PhbkEnterpriseView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
                qp.push(this.route.snapshot.params[this.hf]);
            } else {
                qp.push(JSON.stringify({}));
...
*/
    ...
    tableCommand(v: IEventEmitterData) {
        ...
        if(v.id === 'deletesel') {
            ...
            let qp: string[] = [];
            qp.push( 'Add' + 'PhbkEnterpriseView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
                qp.push(this.route.snapshot.params[this.hf]);
            } else {
                qp.push(JSON.stringify({}));
...
*/
    ...
    applyFilter(frm: PhbkEnterpriseViewSformComponent | any) {
        ...
        let flt: IPhbkEnterpriseView | any = JSON.parse(this.route.snapshot.queryParams['item']);
        if(typeof flt === 'undefined') return;
        if(flt === null) return;

        if ((act === 'Add') ||
            (act === 'Upd')) {
            if(typeof flt['entrprsId'] === 'undefined') {
                return;
            }
            let pkpEntrprsId: any = flt['entrprsId'];
            this.frmRootSrv.getone(
                  pkpEntrprsId
 
            ).subscribe({
                   next: (data: IPhbkEnterpriseView ) => { // success path
                        ...
                        let i: number = frm.dataSource.findIndex((e:any) => {
                            return  (e.entrprsId === data.entrprsId)                        });
                        if(i < 0) {
