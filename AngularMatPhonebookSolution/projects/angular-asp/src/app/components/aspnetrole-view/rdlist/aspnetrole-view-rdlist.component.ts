...
import { AspnetroleViewSformComponent } from 'asp-sforms';
import { AspnetroleViewService } from 'asp-services';


import { IaspnetroleViewVdlg } from 'asp-updforms';
import { AspnetroleViewVdlgComponent } from 'asp-updforms';

import { IaspnetroleViewAdlg } from 'asp-updforms';
import { AspnetroleViewAdlgComponent } from 'asp-updforms';

import { IaspnetroleViewUdlg } from 'asp-updforms';
import { AspnetroleViewUdlgComponent } from 'asp-updforms';

import { IaspnetroleViewDdlg } from 'asp-updforms';
import { AspnetroleViewDdlgComponent } from 'asp-updforms';


@Component({
  selector: 'app-aspnetrole-view-rdlist',
  templateUrl: './aspnetrole-view-rdlist.component.html',
  styleUrls: ['./aspnetrole-view-rdlist.component.css']
})
export class AspnetroleViewRdlistComponent implements OnInit, OnDestroy, IEventEmitterPub, IItemHeightData {
    frases: {[key:string]: string}  = {
        'nvm2mntallwd':  $localize`:Navigating to detail form is not allowed in One-to-Many mode@@AspnetroleViewRdlistComponent.nvm2mntallwd:Navigating to detail form is not allowed in One-to-Many mode`,
        'caption': $localize`:Roles@@AspnetroleViewRdlistComponent.caption:Roles`,
        'update': $localize`:Modify Item@@AspnetroleViewRdlistComponent.update:Modify Item`,
        'delete': $localize`:Delete Item@@AspnetroleViewRdlistComponent.delete:Delete Item`,
        'view': $localize`:View Item@@AspnetroleViewRdlistComponent.view:View Item`,
        'add': $localize`:Add Item@@AspnetroleViewRdlistComponent.add:Add Item`,
        'deletesel': $localize`:Delete Selected@@AspnetroleViewRdlistComponent.deletesel:Delete Selected`,
        'permission': $localize`:Permission@@AspnetroleViewRdlistComponent.permission:Permission`,
        'accessdenied': $localize`:Access denied@@AspnetroleViewRdlistComponent.accessdenied:Access denied`,
        'aspnetrolemaskViewAspNetRole': $localize`:Role Masks(AspNetRole)@@AspnetroleViewRdlistComponent.aspnetrolemaskViewAspNetRole:Role Masks(AspNetRole)`,
    ...
    onMultiSelectedRow(e: Array<IaspnetroleView>): void {
    ...
    constructor(protected route: ActivatedRoute, protected router: Router, protected  frmRootSrv: AspnetroleViewService, protected appGlblSettings: AppGlblSettingsService, 
    ...
    ngOnInit() {
        ...
        let luid = 'AspnetroleViewRdlistComponent';
        this.permMask = this.appGlblSettings.getViewModelMask('aspnetroleView');
...
*/
        ...
        if(!this.isdtl) {
            ...
            if((msk & 1) === 1) {
                this.rowMenuItemsData.push(
                 { id: '1', caption: this.frases['aspnetrolemaskViewAspNetRole'], iconName: 'arrow_forward', iconColor: 'primary', enabled: true, 
                    data: {
    ...
    @Output('selected-row') selectedRow: EventEmitter<IaspnetroleView> = new EventEmitter<IaspnetroleView>();
    onSelectRow(e: IaspnetroleView|any) {
        this.toDetail();
        this.selectedRow.emit(e);
    }
    @ViewChild(AspnetroleViewSformComponent) childForm!: AspnetroleViewSformComponent;
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'dloltnmaspnetroleView': null}}], {relativeTo: this.route}).then(()=>{
                this.router.navigate([{outlets: { 'dloltnmaspnetroleView': null}}], {relativeTo: this.route});
            });
        } else {
            this.router.navigate([{outlets: { 'dloltnmaspnetroleView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'dloltnmaspnetroleView2' + this.selectedDetail!.vw ); 
                    qp.push(JSON.stringify(this.frmRootSrv.getHiddenFilterByRow(this.childForm.currentRow, this.selectedDetail!.nv)));
                    this.router.navigate([{ outlets: { 'dloltnmaspnetroleView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
...
*/
    ...
    onView(e: IaspnetroleView) {
        let locdata: IaspnetroleViewVdlg = {
            title: this.frases['view'],
            hiddenFilter: this.hiddenFilter,
            eformControlModel: e,
            eformNewControlModel: null
        };
        let w: string = this.appGlblSettings.getDialogWidth('aspnetroleView');
        let mw: string = this.appGlblSettings.getDialogMaxWidth('aspnetroleView');
        let dialogRef = this.dialog.open(AspnetroleViewVdlgComponent, {
              data: locdata,
              maxWidth: mw,
              width: w,
            });
    ...
    onAdd(sender: AspnetroleViewSformComponent) {
        let locdata: IaspnetroleViewAdlg = {
        ...
        let w: string = this.appGlblSettings.getDialogWidth('aspnetroleView');
        let mw: string = this.appGlblSettings.getDialogMaxWidth('aspnetroleView');
        let dialogRef = this.dialog.open(AspnetroleViewAdlgComponent, {
              data: locdata,
              maxWidth: mw,
              width: w,
            });
    ...
    onEdit(e: IaspnetroleView) {
        let locdata: IaspnetroleViewUdlg = {
        ...
        let w: string = this.appGlblSettings.getDialogWidth('aspnetroleView');
        let mw: string = this.appGlblSettings.getDialogMaxWidth('aspnetroleView');
        let dialogRef = this.dialog.open(AspnetroleViewUdlgComponent, {
              data: locdata,
              maxWidth: mw,
              width: w,
            });
    ...
    onDelete(e: IaspnetroleView , sender: AspnetroleViewSformComponent ) {
        let locdata: IaspnetroleViewDdlg = {
        ...
        let w: string = this.appGlblSettings.getDialogWidth('aspnetroleView');
        let mw: string = this.appGlblSettings.getDialogMaxWidth('aspnetroleView');
        let dialogRef = this.dialog.open(AspnetroleViewDdlgComponent, {
              data: locdata,
              maxWidth: mw,
              width: w,
            });
    ...
    onDeleteMultiple(sender: AspnetroleViewSformComponent) {
        ...
        dialogRef.afterClosed().subscribe(async rslt => {
            ...
            if (rslt === null) return;
            let currRowsCp: Array<IaspnetroleView> = sender.currentMultiRow.slice();
            let deletedRows: Array<IaspnetroleView> = [];
            for(let row of currRowsCp) {
                try {
                    // let data: IaspnetroleView  =  
                    await firstValueFrom<IaspnetroleView>(this.frmRootSrv.deleteone(  row.id ));
            ...
            deletedRows.forEach((r: IaspnetroleView) => {
                let i: number = sender.dataSource.indexOf(r); 
                if(i > -1) sender.dataSource.splice(i, 1);
            });
            sender.dataSource = sender.dataSource.slice(0);
            if ((deletedRows.indexOf( sender.currentRow as IaspnetroleView) > -1) && (sender.dataSource.length > 0)) sender.onSelectRow(sender.dataSource[0]);
        });
    }
