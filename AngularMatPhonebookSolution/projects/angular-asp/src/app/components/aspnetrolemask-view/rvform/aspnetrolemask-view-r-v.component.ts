...
import { AspnetrolemaskViewVformComponent } from 'asp-updforms';

@Component({
  selector: 'app-aspnetrolemask-view-r-v',
  templateUrl: './aspnetrolemask-view-r-v.component.html',
  styleUrls: ['./aspnetrolemask-view-r-v.component.css']
})

export class AspnetrolemaskViewRVComponent implements OnInit, AfterViewInit, IEventEmitterPub {
    frases: {[key:string]: string}  = {
        'title': $localize`:View Role Mask@@AspnetrolemaskViewRVComponent.ViewaspnetrolemaskView:View Role Mask`,
        'NavBackToMaster': $localize`:Navigate back to master@@AspnetrolemaskViewRVComponent.NavBackToMaster:Navigate back to master`,
        'Hide-details': $localize`:Hide details@@AspnetrolemaskViewRVComponent.Hide-details:Hide details`,
    ...
    eformControlModel: IaspnetrolemaskView | null = null;
    constructor(protected route: ActivatedRoute, protected router: Router, protected appGlblSettings: AppGlblSettingsService, protected frmRootSrv: AspnetrolemaskViewService) { }
    isdtl: boolean = false;
    ngOnInit() {
...
*/
    ...
    @ViewChild(AspnetrolemaskViewVformComponent) childForm!: AspnetrolemaskViewVformComponent;
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'oltnmaspnetrolemaskView': null}}], {relativeTo: this.route});
        } else {
            this.router.navigate([{outlets: { 'oltnmaspnetrolemaskView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'oltnmaspnetrolemaskView2' + this.selectedDetail!.vw );
                    ...
                    this.router.navigate([{ outlets: { 'oltnmaspnetrolemaskView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
