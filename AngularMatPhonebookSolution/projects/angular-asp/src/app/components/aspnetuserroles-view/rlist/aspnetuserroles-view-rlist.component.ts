...
import { AspnetuserrolesViewSformComponent } from 'asp-sforms';


@Component({
  selector: 'app-aspnetuserroles-view-rlist',
  templateUrl: './aspnetuserroles-view-rlist.component.html',
  styleUrls: ['./aspnetuserroles-view-rlist.component.css']
})
export class AspnetuserrolesViewRlistComponent implements OnInit, OnDestroy, IEventEmitterPub, IItemHeightData {
    frases: {[key:string]: string}  = {
        'Hide-details': $localize`:Hide details@@AspnetuserrolesViewRlistComponent.Hide-details:Hide details`,
        'nvm2mntallwd':  $localize`:Navigating to detail form is not allowed in One-to-Many mode@@AspnetuserrolesViewRlistComponent.nvm2mntallwd:Navigating to detail form is not allowed in One-to-Many mode`,
        'caption': $localize`:User Roles@@AspnetuserrolesViewRlistComponent.caption:User Roles`,
        'update': $localize`:Modify Item@@AspnetuserrolesViewRlistComponent.update:Modify Item`,
        'delete': $localize`:Delete Item@@AspnetuserrolesViewRlistComponent.delete:Delete Item`,
        'view': $localize`:View Item@@AspnetuserrolesViewRlistComponent.view:View Item`,
        'add': $localize`:Add Item@@AspnetuserrolesViewRlistComponent.add:Add Item`,
        'deletesel': $localize`:Delete Selected@@AspnetuserrolesViewRlistComponent.deletesel:Delete Selected`,
        'permission': $localize`:Permission@@AspnetuserrolesViewRlistComponent.permission:Permission`,
        'accessdenied': $localize`:Access denied@@AspnetuserrolesViewRlistComponent.accessdenied:Access denied`,
    ...
    onMultiSelectedRow(e: Array<IaspnetuserrolesView>): void {
    ...
    constructor(protected route: ActivatedRoute, protected router: Router, protected  frmRootSrv: AspnetuserrolesViewService, protected appGlblSettings: AppGlblSettingsService, 
    ...
    ngOnInit() {
        ...
        let luid = 'AspnetuserrolesViewRlistComponent';
        this.permMask = this.appGlblSettings.getViewModelMask('aspnetuserrolesView');
...
*/
    ...
    @Output('selected-row') selectedRow: EventEmitter<IaspnetuserrolesView> = new EventEmitter<IaspnetuserrolesView>();
    onSelectRow(e: IaspnetuserrolesView|any) {
        this.toDetail();
        this.selectedRow.emit(e);
    }
    @ViewChild(AspnetuserrolesViewSformComponent) childForm!: AspnetuserrolesViewSformComponent;
...
/*
        if(isNtDef) {
            this.router.navigate([{outlets: { 'loltnmaspnetuserrolesView': null}}], {relativeTo: this.route}).then(()=>{
                this.router.navigate([{outlets: { 'loltnmaspnetuserrolesView': null}}], {relativeTo: this.route});
            });
        } else {
            this.router.navigate([{outlets: { 'loltnmaspnetuserrolesView': null}}], {relativeTo: this.route}).then(
                () => {
                    let qp: string[] = [];
                    qp.push( 'loltnmaspnetuserrolesView2' + this.selectedDetail!.vw );
                    qp.push(JSON.stringify(this.frmRootSrv.getHiddenFilterByRow(this.childForm.currentRow, this.selectedDetail!.nv)));
                    this.router.navigate([{ outlets: { 'loltnmaspnetuserrolesView' : qp } }], {relativeTo: this.route});
                }
            );
        }
*/
    ...
    onDeleteMultiple(sender: AspnetuserrolesViewSformComponent) {
        ...
        dialogRef.afterClosed().subscribe(async rslt => {
            ...
            if (rslt === null) return;
            let currRowsCp: Array<IaspnetuserrolesView> = sender.currentMultiRow.slice();
            let deletedRows: Array<IaspnetuserrolesView> = [];
            for(let row of currRowsCp) {
                try {
                    // let data: IaspnetuserrolesView  =  
                    await firstValueFrom<IaspnetuserrolesView>(this.frmRootSrv.deleteone(  row.userId ,   row.roleId ));
            ...
            deletedRows.forEach((r: IaspnetuserrolesView) => {
                let i: number = sender.dataSource.indexOf(r); 
                if(i > -1) sender.dataSource.splice(i, 1);
            });
            sender.dataSource = sender.dataSource.slice(0);
            if ((deletedRows.indexOf( sender.currentRow  as IaspnetuserrolesView) > -1) && (sender.dataSource.length > 0)) sender.onSelectRow(sender.dataSource[0]);
        });
    }
    ...
    rowCommand(v: IEventEmitterData) {
        ...
        let flt: any = {
          ...
          roleId: v.value.roleId,
        };
        if(v.id === 'view') {
            qp.push( 'View' + 'aspnetuserrolesView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
            ...
            qp.push( 'Upd' + 'aspnetuserrolesView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
                qp.push(this.route.snapshot.params[this.hf]);
            } else {
                qp.push(JSON.stringify({}));
            ...
            qp.push( 'Del' + 'aspnetuserrolesView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
                qp.push(this.route.snapshot.params[this.hf]);
            } else {
                qp.push(JSON.stringify({}));
...
*/
    ...
    tableCommand(v: IEventEmitterData) {
        ...
        if(v.id === 'deletesel') {
            ...
            let qp: string[] = [];
            qp.push( 'Add' + 'aspnetuserrolesView' );
            if (!(typeof this.route.snapshot.params[this.hf] === 'undefined')) {
                qp.push(this.route.snapshot.params[this.hf]);
            } else {
                qp.push(JSON.stringify({}));
...
*/
    ...
    applyFilter(frm: AspnetuserrolesViewSformComponent | any) {
        ...
        let flt: IaspnetuserrolesView | any = JSON.parse(this.route.snapshot.queryParams['item']);
        if(typeof flt === 'undefined') return;
        if(flt === null) return;

        if ((act === 'Add') ||
            (act === 'Upd')) {
            if(typeof flt['userId'] === 'undefined') {
                return;
            }
            let pkpUserId: any = flt['userId'];
            if(typeof flt['roleId'] === 'undefined') {
                return;
            }
            let pkpRoleId: any = flt['roleId'];
            this.frmRootSrv.getone(
                  pkpUserId
                , pkpRoleId
 
            ).subscribe({
                   next: (data: IaspnetuserrolesView ) => { // success path
                        ...
                        let i: number = frm.dataSource.findIndex((e:any) => {
                            return  (e.userId === data.userId) &&  (e.roleId === data.roleId)                        });
                        if(i < 0) {
