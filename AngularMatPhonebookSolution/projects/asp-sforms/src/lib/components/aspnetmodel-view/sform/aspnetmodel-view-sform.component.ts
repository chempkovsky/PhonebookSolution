...
/*
class aspnetmodelViewMtPgntrIntl implements MatPaginatorIntl {
    changes = new Subject<void>();
    firstPageLabel = $localize`First page@@AspnetmodelViewSformComponent.firstPageLabel:First page`;
    itemsPerPageLabel = $localize`Items per page@@AspnetmodelViewSformComponent.itemsPerPageLabel:Items per page`;
    lastPageLabel = $localize`Last page@@AspnetmodelViewSformComponent.lastPageLabel:Last page`;
    nextPageLabel = $localize`Next page@@AspnetmodelViewSformComponent.nextPageLabel:Next page`;
    previousPageLabel = $localize`Previous page@@AspnetmodelViewSformComponent.previousPageLabel:Previous page`;
    ofLabel = $localize`of@@AspnetmodelViewSformComponent.ofLabel:of`;
...
@Component({
  selector: 'app-aspnetmodel-view-sform',
  templateUrl: './aspnetmodel-view-sform.component.html',
  styleUrls: ['./aspnetmodel-view-sform.component.css'],
//  providers: [{ provide: MatPaginatorIntl, useClass: aspnetmodelViewMtPgntrIntl }]
})
export class AspnetmodelViewSformComponent implements OnInit, AfterViewInit,  IEventEmitterPub, IItemHeightData {
    ...
    clmnCptnsaspnetmodelView: {[key:string]: string}  = {
        ...
        'modelDescription': $localize`:Model Description@@aspnetmodelView.modelDescription-Name:Model Description`,
    };


    menuCptns: {[key:string]: string}  = {
        'fullscann': $localize`:full scan@@AspnetmodelViewSformComponent.fullscann-menu:full scan`,
        'scannByPrimary': $localize`:filter by Primary@@AspnetmodelViewSformComponent.scannByPrimary-menu:filter by Primary`,
    };
    ...
    onMultiSelectRow(e: Array<IaspnetmodelView>) {
    ...
    ngOnInit() {
        this.multSelection.changed.subscribe({next: 
            (e: SelectionChange<IaspnetmodelView>) => {
...
//===============
    ...
    public dataSource: Array<IaspnetmodelView> = [];
    matPaginatorLen: number = 0;
    matPaginatorPageSize: number = 10;
    matPaginatorPageSizeOptions: Array<number> = [10, 25, 50, 100];
    displayedColumns:  Array<string> = ['selectAction', 'selectMultAction', 'modelPk', 'modelName', 'modelDescription', 'menuAction'];
          ...
          if(this.hiddenFilterEx.length === reqFkPrps.length) {
              ...
              if(insmi) {
                rslt.push(
                  ...
                  {fltrName: 'modelPk', fltrCaption: this.clmnCptnsaspnetmodelView['modelPk'], fltrDataType: 'int32', fltrMaxLen: null, fltrMin: null, fltrMax: null, fltrFlx:100, fltrMd: (100 / 1) - 1, fltrSm: (100 / 1) - 1, fltrXs: 100, fltrSrv: this.frmRootSrv},
                ]});
              }
            }
        }
        if(hasNtExternal && this.cnFllscn) {
          rslt.push(
            {id: 'fullscann', caption: this.menuCptns['fullscann'], iconName: 'search', iconColor: 'primary', enabled: true, data: [
              {fltrName: 'modelPk', fltrCaption: this.clmnCptnsaspnetmodelView['modelPk'], fltrDataType: 'int32', fltrMaxLen: null, fltrMin: null, fltrMax: null, fltrFlx:100, fltrMd:24, fltrSm:49, fltrXs: 100, fltrSrv: this.frmRootSrv },
              {fltrName: 'modelName', fltrCaption: this.clmnCptnsaspnetmodelView['modelName'], fltrDataType: 'string', fltrMaxLen: 50, fltrMin: null, fltrMax: null, fltrFlx:100, fltrMd:24, fltrSm:49, fltrXs: 100, fltrSrv: this.frmRootSrv },
          ]});
        }
      return rslt;
    }
    ...
    resetexternalFilter() {
        ...
        switch(ef.id) {
            default:
                let flt: IaspnetmodelViewFilter|any = JSON.parse(ef.f['data']);
                if(flt) {
    ...
    public currentRow: IaspnetmodelView | null = null;
    @Output('selected-row') selectedRow: EventEmitter<IaspnetmodelView|null> = new EventEmitter<IaspnetmodelView|null>();
    @Output('apply-filter') applyFilter: EventEmitter<AspnetmodelViewSformComponent|null> = new EventEmitter<AspnetmodelViewSformComponent|null>();

    cnFllscn: boolean = false;
    constructor(public  frmRootSrv: AspnetmodelViewService, public appGlblSettings: AppGlblSettingsService, 
    
            protected dialog: MatDialog, private cd: ChangeDetectorRef) {
            this.cnFllscn = (appGlblSettings.getViewModelMask('aspnetmodelView') & 16) === 16;
        this.redefTableIndexMenuItemsData();
    }
    ...
    onBeforeSquery(flt: IaspnetmodelViewFilter|any) {
    ...
    onFilter() {
        if(typeof this.curIndexMenuItemsData === 'undefined') return;
        if(this.curIndexMenuItemsData === null) return;
        let flt: IaspnetmodelViewFilter|any = { page: this.currentPageIndex, pagesize: this.currentPageSize };
        ...
        this.inQuery = true;
        let mc: Observable<IaspnetmodelViewPage> | any;
        switch (this.curIndexMenuItemsData.id) {
        ...
        mc.subscribe({
                next: (v: IaspnetmodelViewPage) =>{
                    ...
                    let currow: IaspnetmodelView | null = null;
    ...
    rowCommand(e: IaspnetmodelView, id: string) {
    ...
    onSettings() {
        let locdata: Array<IColumnSelectorItem>=[
            { name: 'modelPk', caption: this.clmnCptnsaspnetmodelView['modelPk'], checked: false },
            { name: 'modelName', caption: this.clmnCptnsaspnetmodelView['modelName'], checked: false },
            { name: 'modelDescription', caption: this.clmnCptnsaspnetmodelView['modelDescription'], checked: false },
        ];
...
/////////////////////////////////////////
    ...
    srchSlctRwPrimary: IaspnetmodelView | any = null;
    afterObjSelPrimary(e:{v: any, i: number}) {
        if(typeof e === 'undefined') return;
        if(e === null) return;
        if(e.v) {
            this.srchSlctRwPrimary = e.v as IaspnetmodelView;
        } else {
    ...
    tpAheadFncPrimary(srv: any, wsfd: Array<IUniqServiceFilter>, value: any, i: number): Observable<Array<any>> {
        let fltr: IaspnetmodelViewFilter | any = { page: 0, pagesize: 15 };
        if(i > 0) { 
            fltr.modelPk = [wsfd[0].fltrValue.value]; 
            fltr.modelPkOprtr = ['eq']
         } else if(i === 0) { 
            fltr.modelPk = [value]; 
            fltr.modelPkOprtr = ['lk']
         }
        return srv.getmanybyrepprim(fltr)
            .pipe(
                switchMap((rslt: IaspnetmodelViewPage) => {
                    if (!(typeof rslt === 'undefined')) {
                    if (!(rslt === null)) {
    ...
    tpAheadCptnPrimary(v: any, i: number): string {
        let retStr = '';
        if(v) {
            if (!(typeof v.modelPk === 'undefined')) {
                if(!(v.modelPk === null)) {
                    retStr = retStr + ' ' + v.modelPk;
                }
            }
        } 
        return retStr;
