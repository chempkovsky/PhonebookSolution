...
/*
class aspnetuserrolesViewMtPgntrIntl implements MatPaginatorIntl {
    changes = new Subject<void>();
    firstPageLabel = $localize`First page@@AspnetuserrolesViewSformComponent.firstPageLabel:First page`;
    itemsPerPageLabel = $localize`Items per page@@AspnetuserrolesViewSformComponent.itemsPerPageLabel:Items per page`;
    lastPageLabel = $localize`Last page@@AspnetuserrolesViewSformComponent.lastPageLabel:Last page`;
    nextPageLabel = $localize`Next page@@AspnetuserrolesViewSformComponent.nextPageLabel:Next page`;
    previousPageLabel = $localize`Previous page@@AspnetuserrolesViewSformComponent.previousPageLabel:Previous page`;
    ofLabel = $localize`of@@AspnetuserrolesViewSformComponent.ofLabel:of`;
...
@Component({
  selector: 'app-aspnetuserroles-view-sform',
  templateUrl: './aspnetuserroles-view-sform.component.html',
  styleUrls: ['./aspnetuserroles-view-sform.component.css'],
//  providers: [{ provide: MatPaginatorIntl, useClass: aspnetuserrolesViewMtPgntrIntl }]
})
export class AspnetuserrolesViewSformComponent implements OnInit, AfterViewInit,  IEventEmitterPub, IItemHeightData {
    ...
    clmnCptnsaspnetuserrolesView: {[key:string]: string}  = {
        ...
        'uLockoutEnd': $localize`:Lockout End@@aspnetuserrolesView.uLockoutEnd-Name:Lockout End`,
        'uUserName': $localize`:User Name@@aspnetuserrolesView.uUserName-Name:User Name`,
        'rName': $localize`:Role Name@@aspnetuserrolesView.rName-Name:Role Name`,
    };


    menuCptns: {[key:string]: string}  = {
        'fullscann': $localize`:full scan@@AspnetuserrolesViewSformComponent.fullscann-menu:full scan`,
        'scannByPrimary': $localize`:filter by Primary@@AspnetuserrolesViewSformComponent.scannByPrimary-menu:filter by Primary`,
    };
    ...
    public currentMultiRow: Array<IaspnetuserrolesView> = [];
    @Output('multi-selected-row') multiSelectedRow: EventEmitter<Array<IaspnetuserrolesView>> = new EventEmitter<Array<IaspnetuserrolesView>>();
    public multSelection: SelectionModel<IaspnetuserrolesView> = new SelectionModel<IaspnetuserrolesView>(true, []);
    selectRows(e: Array<IaspnetuserrolesView>) {
        if((typeof e === 'undefined') || (typeof this.dataSource  === 'undefined')) return;
        if((e === null) || (this.dataSource  === null)) return;
        if(!Array.isArray(e)) return;
        let rslt: Array<IaspnetuserrolesView>=[];
        e.forEach(row => { if(this.dataSource.indexOf(row) > -1) rslt.push(row) });
        this.multSelection.select(...rslt);
    }
    deselectRows(e: Array<IaspnetuserrolesView>) {
    ...
    onMultiSelectRow(e: Array<IaspnetuserrolesView>) {
    ...
    ngOnInit() {
        this.multSelection.changed.subscribe({next: 
            (e: SelectionChange<IaspnetuserrolesView>) => {
...
//===============
    ...
    public dataSource: Array<IaspnetuserrolesView> = [];
    matPaginatorLen: number = 0;
    matPaginatorPageSize: number = 10;
    matPaginatorPageSizeOptions: Array<number> = [10, 25, 50, 100];
    displayedColumns:  Array<string> = ['selectAction', 'selectMultAction', 'uUserName', 'rName', 'menuAction'];
        ...
        if(hasNtExternal && this.cnFllscn) {
          rslt.push(
            {id: 'fullscann', caption: this.menuCptns['fullscann'], iconName: 'search', iconColor: 'primary', enabled: true, data: [
              {fltrName: 'userId', fltrCaption: this.clmnCptnsaspnetuserrolesView['userId'], fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null, fltrFlx:100, fltrMd:24, fltrSm:49, fltrXs: 100, fltrSrv: this.frmRootSrv },
              {fltrName: 'roleId', fltrCaption: this.clmnCptnsaspnetuserrolesView['roleId'], fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null, fltrFlx:100, fltrMd:24, fltrSm:49, fltrXs: 100, fltrSrv: this.frmRootSrv },
              {fltrName: 'uUserName', fltrCaption: this.clmnCptnsaspnetuserrolesView['uUserName'], fltrDataType: 'string', fltrMaxLen: 256, fltrMin: null, fltrMax: null, fltrFlx:100, fltrMd:24, fltrSm:49, fltrXs: 100, fltrSrv: this.frmRootSrv },
              {fltrName: 'rName', fltrCaption: this.clmnCptnsaspnetuserrolesView['rName'], fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null, fltrFlx:100, fltrMd:24, fltrSm:49, fltrXs: 100, fltrSrv: this.frmRootSrv },
          ]});
        }
      return rslt;
    }
    ...
    resetexternalFilter() {
        ...
        switch(ef.id) {
            default:
                let flt: IaspnetuserrolesViewFilter|any = JSON.parse(ef.f['data']);
                if(flt) {
    ...
    public currentRow: IaspnetuserrolesView | null = null;
    @Output('selected-row') selectedRow: EventEmitter<IaspnetuserrolesView|null> = new EventEmitter<IaspnetuserrolesView|null>();
    @Output('apply-filter') applyFilter: EventEmitter<AspnetuserrolesViewSformComponent|null> = new EventEmitter<AspnetuserrolesViewSformComponent|null>();

    cnFllscn: boolean = false;
    constructor(public  frmRootSrv: AspnetuserrolesViewService, public appGlblSettings: AppGlblSettingsService, 
    
            protected dialog: MatDialog, private cd: ChangeDetectorRef) {
            this.cnFllscn = (appGlblSettings.getViewModelMask('aspnetuserrolesView') & 16) === 16;
        this.redefTableIndexMenuItemsData();
    }
    ...
    onBeforeSquery(flt: IaspnetuserrolesViewFilter|any) {
    ...
    onFilter() {
        if(typeof this.curIndexMenuItemsData === 'undefined') return;
        if(this.curIndexMenuItemsData === null) return;
        let flt: IaspnetuserrolesViewFilter|any = { page: this.currentPageIndex, pagesize: this.currentPageSize };
        ...
        this.inQuery = true;
        let mc: Observable<IaspnetuserrolesViewPage> | any;
        switch (this.curIndexMenuItemsData.id) {
        ...
        mc.subscribe({
                next: (v: IaspnetuserrolesViewPage) =>{
                    this.inQuery = false;
                    let pl: number = 0;
                    ...
                    let currow: IaspnetuserrolesView | null = null;
    ...
    onSelectRow(e: IaspnetuserrolesView|any) {
        if (typeof e === 'undefined') {
    ...
    rowCommand(e: IaspnetuserrolesView, id: string) {
    ...
    onSettings() {
        let locdata: Array<IColumnSelectorItem>=[
            { name: 'uUserName', caption: this.clmnCptnsaspnetuserrolesView['uUserName'], checked: false },
            { name: 'rName', caption: this.clmnCptnsaspnetuserrolesView['rName'], checked: false },
            { name: 'roleId', caption: this.clmnCptnsaspnetuserrolesView['roleId'], checked: false },
            { name: 'userId', caption: this.clmnCptnsaspnetuserrolesView['userId'], checked: false },
            { name: 'uLockoutEnd', caption: this.clmnCptnsaspnetuserrolesView['uLockoutEnd'], checked: false },
        ];
...
/////////////////////////////////////////
    ...
    srchSlctRwPrimary: IaspnetuserrolesView | any = null;
    afterObjSelPrimary(e:{v: any, i: number}) {
        if(typeof e === 'undefined') return;
        if(e === null) return;
        if(e.v) {
            this.srchSlctRwPrimary = e.v as IaspnetuserrolesView;
        } else {
    ...
    tpAheadFncPrimary(srv: any, wsfd: Array<IUniqServiceFilter>, value: any, i: number): Observable<Array<any>> {
        let fltr: IaspnetuserrolesViewFilter | any = { page: 0, pagesize: 15 };
        return srv.getmanybyrepprim(fltr)
            .pipe(
                switchMap((rslt: IaspnetuserrolesViewPage) => {
                    if (!(typeof rslt === 'undefined')) {
                    if (!(rslt === null)) {
